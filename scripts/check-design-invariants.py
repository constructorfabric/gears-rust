...
from orders_doc_layout import slice_sources, source_path, read_source, migrated, section_map
...
def resolve_doc(token: str, docs: Path) -> Path | None:
    ...
    if token in DOC_ALIASES:
        return source_path(docs, DOC_ALIASES[token])
    if m := re.fullmatch(r"(\d{2})", token):
        # Guarded by the caller: bare digits are only a slice in document position.
        matches = sorted(slice_sources(docs, f"{m.group(1)}-*.md"))
        return matches[0] if matches else None
    if token.endswith(".md"):
        candidate = source_path(docs, token)
...
def check_counts(docs: Path, res: Result) -> None:
    ...
    decisions = read(docs / "DECISIONS.md")
    foundation = next(iter(sorted(slice_sources(docs, "01-*.md"))), None)
...
def check_reasons(docs: Path, res: Result) -> None:
    ...
    for path in sorted(slice_sources(docs, "*.md")):
        text = read(path)
        for m in re.finditer(
...
def check_step_citations(docs: Path, res: Result) -> None:
    ...
    sources += sorted(slice_sources(docs, "*.md"))
...
def check_enum_membership(docs: Path, res: Result) -> None:
    ...
    files = sorted(slice_sources(docs, "*.md")) + [docs / "DESIGN.md"]
    for path in files:
        text = read(path)
...
def check_no_pre_engine_refusals(docs: Path, res: Result) -> None:
    ...
    for path in sorted(slice_sources(docs, "0[2-8]*.md")):
        text = read(path)
        blocks = list(re.finditer(r"^\*\*Algorithm:\s*(.+?)\*\*\s*$", text, re.M))
        for i, blk in enumerate(blocks):
            end = blocks[i + 1].start() if i + 1 < len(blocks) else len(text)
...
def check_prose_counts(docs: Path, res: Result) -> None:
    ...
    """
    core = next(iter(sorted(slice_sources(docs, "01-*.md"))), None)
    gate = next(iter(sorted(slice_sources(docs, "03-*.md"))), None)
    authz = next(iter(sorted(slice_sources(docs, "08-*.md"))), None)
    seam = next(iter(sorted(slice_sources(docs, "06-*.md"))), None)
    ...
    def artifacts() -> int:
        ...
        return (
            ...
            + len(list(slice_sources(docs, "*.md")))
        )
...
def check_adr_reciprocity(docs: Path, res: Result) -> None:
    ...
    for adr in sorted(adr_dir.glob("*.md")):
        ...
        if trace:
            ...
            if migrated(docs):
                named.update("contract:" + number for number in re.findall(r"\b(0[1-8])(?:\s+§|-[a-z])", trace.group(1)))
            for rel in sorted(named):
                target = resolve_doc(rel.split(":", 1)[1], docs) if rel.startswith("contract:") else source_path(docs, rel)
...
def check_version_vocabulary(docs: Path, res: Result) -> None:
    ...
    if not actual:
        return
    files = sorted(slice_sources(docs, "*.md")) + [docs / "DECISIONS.md"]
    for path in files:
        t = read(path).replace("**", "")
        for m in re.finditer(
...
def transition_rows(docs: Path) -> dict[int, tuple[set[str], set[str], str | None, str]]:
    ...
    """
    text = read(source_path(docs, "design/01-foundation.md"))
...
def check_retention_executor(docs: Path, res: Result) -> None:
    ...
    body = "\n".join(
        read(p) for p in [docs / "DESIGN.md"] + sorted(slice_sources(docs, "*.md"))
    )
...
def check_settlement_claim(docs: Path, res: Result) -> None:
    ...
    """
    flat = re.sub(r"\s+", " ", read(source_path(docs, "design/01-foundation.md")))
    m = re.search(
...
def check_migrated_layout(docs: Path, res: Result) -> bool:
    """Fail closed when relocation drops a section or its stable definitions."""
    if not migrated(docs):
        return True
    from collections import Counter
    import json
    from orders_doc_layout import MAP_PATH

    paths = [docs / "DESIGN.md", *sorted((docs / "features").glob("*.md"))]
    contents = {str(p.relative_to(docs)): p.read_text() for p in paths}
    definitions = Counter(re.findall(r"\*\*ID\*\*:\s*`(cpt-[^`]+)`", "\n".join(contents.values())))
    for x in section_map():
        marker = f'<!-- contract:{x["slice"]}:{x["section"]} -->'
        body = contents.get(x["target"], "")
        res.check(sum(t.count(marker) for t in contents.values()) == 1,
                  f'contract {x["slice"]}:{x["section"]} must exist exactly once')
        match = re.search(re.escape(marker) + r"(.*?)<!-- /contract -->", body, re.S)
        res.check(match is not None, f'contract {x["slice"]}:{x["section"]} missing from {x["target"]}')
        if match:
            for cid in x["ids"]:
                res.check(cid in match[1] and definitions[cid] == 1,
                          f'contract definition {cid} must remain unique in {x["target"]}')
    for cid in json.loads(MAP_PATH.read_text())["root_ids"].values():
        res.check(definitions[cid] == 1, f'architecture identity {cid} must remain unique')
...
def run(docs: Path) -> Result:
    ...
    if not supported(docs):
        return res
    if not check_migrated_layout(docs, res):
        return res
