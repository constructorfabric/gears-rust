...
from orders_doc_layout import source_path, slice_sources, migrated, normalize_references
...
def mutate(case: Case, root: Path) -> bool:
    ...
    target = source_path(root, case.path)
    ...
    if case.family in {"gts-ids", "gts-registry"}:
        ...
        hit = False
        for path in [root / "DESIGN.md"] + sorted(slice_sources(root)):
            body = path.read_text(encoding="utf-8")
    ...
    if case.family == "prose":
        ...
        find, replacement = case.find, case.replace
        if case.family == "steps" and find not in text and migrated(root):
            address = "[01 §3.6](features/01-foundation.md#contract-01-3-6)"
            find = find.replace("`01 §3.6`", address)
            replacement = replacement.replace("`01 §3.6`", address)
        if find not in text and migrated(root):
            # Fixture addresses retain logical contract numbers across layouts.
            text = normalize_references(text)
            find = re.sub(r"`(0[1-8] §[^`]+)`", r"\1", normalize_references(find))
            replacement = re.sub(r"`(0[1-8] §[^`]+)`", r"\1", normalize_references(replacement))
        if find not in text:
            return False
...
def main(argv: list[str]) -> int:
    ...
    if migrated(docs):
        cases.extend([
            Case("layout", "a canonical behavior section disappears", "features/01-foundation.md",
                 "<!-- contract:01-foundation:3.6 -->", "<!-- missing-contract -->", "missing from"),
            Case("layout", "a relocated schema loses its stable ID", "DESIGN.md",
                 "**ID**: `cpt-cf-bss-orders-lifecycle-dbtable-order-version`",
                 "**ID**: `cpt-cf-bss-orders-lifecycle-dbtable-removed-version`", "must remain unique"),
            Case("layout", "an architecture identity is duplicated", "DESIGN.md",
                 "**ID**: `cpt-cf-bss-orders-lifecycle-design-foundation`",
                 "**ID**: `cpt-cf-bss-orders-lifecycle-design-foundation`\n**ID**: `cpt-cf-bss-orders-lifecycle-design-foundation`",
                 "must remain unique"),
        ])
    failures = 0
