...
Every entry has a key on the REST, SDK, and Plugin SPI contracts, because a
measurement and an invalidation ride the same ingestion path. Who supplies it
depends on the entry. A measurement carries the caller's key, and the gear
rejects a keyless measurement with a deterministic error. An invalidation carries
no caller key: the gear derives one as `inv:` followed by the identifier of the
entry it withdraws, and rejects an invalidation that supplies a key of its own.
The prefix is reserved, so the gear also rejects a measurement key that begins
with `inv:`, and the two key spaces never meet.

Deriving the invalidation's key is what places at most one invalidation per
entry inside this decision. An invalidation copies its target's tenant, type,
and covered period, so every withdrawal of one entry has one dedup identity.
Two of them then resolve by the same-key outcomes below rather than by a
store-side rule of their own
...
bounds of the covered period. The active plugin enforces it at the storage
layer, under the guarantee levels below, and the gear keeps no dedup table of
its own. Two submissions under one key that cover different periods are
therefore distinct entries, not a conflict.
...
neither the dedup identity nor the identifier derivation. A measurement and an
invalidation cannot collide at all, because only an invalidation key carries the
reserved prefix.
...
- Every entry carries an idempotency key on every surface. A caller cannot omit
  it from a measurement, and cannot supply it on an invalidation, whose key the
  gear derives from the target.
...
- At most one invalidation per entry needs no store-side rule of its own. Two
  withdrawals of one entry share a derived identity, so the floor and the
...
- A caller chooses its own measurement keys. The contract bounds the key's
  length, forbids control characters so that the identifier derivation stays
  injective, and reserves the `inv:` prefix for derived invalidation keys. No
  token shape is enforced otherwise. A caller that generates an opaque key, such
  as a ULID or a UUIDv7, must therefore store it before the first send.
...
- Ingestion contract tests that reject, on every surface, a keyless measurement,
  an invalidation carrying a key, and a measurement key beginning with `inv:`.
...
Every measurement carries a caller-supplied key, and every invalidation a key
derived from its target. The plugin enforces the dedup identity at the storage
...
- `cpt-cf-usage-collector-adr-append-only-invalidation` — the invalidation entry
  whose reason code joins the canonical-field set, and whose derived key bounds
  withdrawal to one per entry.
...
- `cpt-cf-usage-collector-fr-record-invalidation` — the invalidation entry whose
  key the gear derives, and whose at-most-one rule follows from it.
