...
**Availability / Fallback**: Storage extension resolution is deferred to the first persistence or query call rather than performed at startup, so a registry or orchestration outage during startup does not withhold gear readiness. Until an extension resolves, every persistence and query call is rejected as unavailable; resolution is reattempted on the next call, so the gear recovers without a restart once the dependency returns.
