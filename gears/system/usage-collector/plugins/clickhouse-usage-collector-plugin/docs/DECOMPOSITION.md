...
- **Scope**:
  ...
  - Batch insert as one multi-row `INSERT`, per-record results in input order, using a single batched pre-check `SELECT`.
  ...
  - Batch-write-path throughput allocation (one multi-row `INSERT` per batch, no per-row round-trip).
