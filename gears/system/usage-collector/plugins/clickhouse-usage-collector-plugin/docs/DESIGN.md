...
| `cpt-cf-usage-collector-nfr-throughput` (≥10,000 records/sec) | Batch writes as a single multi-row `INSERT` per batch (ClickHouse's write path strongly favors large parts over many small ones); no per-row round-trip. **Lock-bound per-`gts_id` ceiling**: every create acquires the *same* per-`gts_id` exclusive coordination lock ([§3.5](#35-external-dependencies)), so creates for one `gts_id` execute one critical section at a time (lock acquire → catalog check → dedup pre-check → `INSERT`). Distinct `gts_id`s do not contend, and within one multi-`gts_id` batch their read phases overlap ([§3.6](#36-interactions--sequences) Batch Ingest step 4), so this envelope is met by batching and/or by spreading load across `gts_id`s; a workload concentrated on a single `gts_id` is capped by that serialized critical-section rate, not by ClickHouse's write throughput, and single-record `create_usage_record` calls against one hot `gts_id` are the worst case. **Shared-pool exposure**: the same client/pool serves the query path, so query bursts can consume capacity this NFR depends on (see the workload-isolation row). |
...
The `usage_records` TTL clause is baked into the `CREATE TABLE IF NOT EXISTS` DDL at first provisioning only; changing `retention_period_secs` after the table exists has no effect without a manual `ALTER TABLE … MODIFY TTL`. This is a consequence of ClickHouse's DDL model (`cpt-cf-uc-ch-plugin-constraint-no-in-place-update`).
...
| --- | --- |
| Plugin Module (`gear.rs`) | `init()`: load/validate config, build the ClickHouse client (via `build_client` — parses `database_url` into a bare base URL plus separate user/password/database, applied via `with_user`/`with_password`/`with_database` so the `clickhouse::Client` URL path and userinfo are never mixed), build the Coordination Lock Manager's cluster lock facade, run initial schema provisioning (baking the configured `retention_period_secs` value directly into the DDL via `{retention_period_secs}` placeholder substitution), perform the four-step GTS/`types-registry`/`ClientHub` registration handshake. |
| Schema Migration (`infra/storage/pool.rs`, `migrations/0001_init.sql`) | A single embedded SQL file executed as a sequence of idempotent `CREATE TABLE IF NOT EXISTS` DDL statements at `init`. The `{retention_period_secs}` placeholder is substituted with the configured value before execution, baking the operator-chosen retention window directly into the `TTL` clause. Comment lines (`--`) are stripped before statement splitting to avoid false statement boundaries from semicolons in prose comments; statements are split while respecting single-quoted string literals. No `schema_migrations` tracking table, no external framework. Owns **initial** schema creation only (see [§4](#4-additional-context) Deferred). |
...
Schema Migration (`migrations/0001_init.sql` + runner): idempotent DDL runner with `{retention_period_secs}` placeholder substitution.
...
4. `SELECT ... FINAL WHERE tenant_id = ? AND gts_id = ? AND created_at = ? AND id = ?` — a point lookup against the `usage_records` `ReplacingMergeTree` table, using the full `ORDER BY` key prefix (not a bare `id` lookup) so the read is a genuine point lookup against the table's sort key rather than a filtered scan (see [§3.7](#37-database-schema--tables)). Because the coordination lock is a **single exclusive mutex** per `gts_id` (not a reader/writer lock), concurrent creates for the same `gts_id` serialize at step 2 — only one create for a given `gts_id` executes this check at a time.
...
1. **Partition the batch by `gts_id`**, preserving each record's input position so outcomes can be written back positionally, then **sort the partition keys**. The sort is not cosmetic: acquiring per-`gts_id` locks in hash-map iteration order would let two concurrent multi-type batches take the same pair of locks in opposite orders and deadlock until both leases lapse. Sorting gives every caller one global acquisition order, so the acquisitions can only queue, never cycle.
2. **Acquire every partition's exclusive `gts_id` coordination lock up front, sequentially, in that sorted order** (same lock name as the single-record path, the catalog-create path, and the delete path). On failure for one partition, write that partition's error to every one of its records' positions and continue with the other partitions — no whole-batch abort.
...
4. **Per partition: one batched dedup pre-check** — a single `SELECT ... FINAL WHERE (tenant_id, gts_id, created_at, id) IN (...)` over that partition's records. Because every partition already holds its own lock (step 2), these read phases are independent and are issued **concurrently** across partitions rather than one partition at a time; only their ClickHouse round-trips overlap, not their critical sections' mutual exclusion.
5. **Per partition: `ensure_still_held()` (lease renew)** after the partition's reads and before any write, so a lease lost during the ClickHouse round-trips aborts the partition with `Transient` (incrementing `uc_clickhouse_lock_manager_unavailable_total{mode="create"}`) instead of writing under an expired lock — the same cluster ADR-002 deviation the single-record path applies ([§3.5](#35-external-dependencies)). An expired partition's composed rows are dropped from the pending insert set before the write.
6. **Resolve per-record outcomes in input order**, partition by partition and sequentially (so `version` offsets and within-batch dedup stay deterministic even though step 4 ran concurrently) — existing row with equal canonical fields → silent absorb; existing row with differing fields → `IdempotencyConflict`; a second row for the same dedup key inside this same batch is absorbed only if canonically identical, otherwise it is likewise a conflict; anything else joins the pending insert set with a monotonic `version`.
7. **One multi-row `INSERT` for the whole call**, pooling every passing partition's non-duplicate rows: a batch spanning `N` distinct `gts_id`s issues `N` lock acquisitions but exactly one ClickHouse `INSERT`.
8. **Release every partition's lock** — each is held until after the combined `INSERT` completes (including on the insert-failure path), not released the moment its own partition's rows are resolved.
9. **Return one outcome per input record in input order**, mirroring the reference plugin's positionally-aligned batch contract. A failed combined `INSERT` is reported per affected record slot rather than as a top-level error, so outcomes already decided for absorbed rows survive.
...
`SELECT ... FROM usage_records FINAL WHERE gts_id = ? [AND <filter>] [AND <metadata>] [AND <keyset>] ORDER BY <order> LIMIT <n+1>`, reusing the reference plugin's look-ahead-row-then-truncate keyset pagination pattern, translated to ClickHouse's parameter-binding syntax. The wire-level cap (≤ 1,000 records) bounds `<n>`, independent of the aggregation bucket cap above.
...
- **Replicated deployment: bounded by ClickHouse replication lag, not by the plugin.** A read hitting a different replica than the one that served the write is bounded by ClickHouse's own replication lag (via ClickHouse Keeper), typically sub-second under healthy operation. **Measurement method and owner**: the Foundation feature (DECOMPOSITION.md §2.1) exposes ClickHouse's own `system.replicas.absolute_delay` as a scraped operational signal in the plugin's deployment README (not a plugin-emitted OTLP metric, since it is not this plugin's own state); operators running a replicated deployment MUST monitor it and MUST configure ClickHouse's `insert_quorum` (a native ClickHouse write-quorum setting) if they require strict read-your-writes across replicas, at a documented write-latency cost proportional to the quorum size. This plugin's default configuration does not set `insert_quorum` (single-node/no-quorum default), and the plugin's own documentation MUST NOT claim a stronger cross-replica bound than "typically sub-second, operator-monitored" without quorum writes enabled.
...
- TLS is enforced, not merely advisory: `ClickHousePluginConfig::validate` rejects a plaintext `http://` `database_url` at startup unless the config explicitly sets `allow_insecure_http = true`, so a misconfigured DSN fails closed before any connection carrying credentials is attempted, rather than merely warning and proceeding. The override exists only for non-TLS development/test connections (e.g. a local Docker `ClickHouse` container) and is additionally logged via `tracing::warn!` on every connection it permits — mirroring the reference plugin's `sslmode` upgrade-with-warning pattern, but backed by a config-level gate rather than a log line alone.
