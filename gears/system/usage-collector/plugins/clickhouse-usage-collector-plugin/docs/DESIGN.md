...
4. **Per partition: one batched dedup pre-check** — a single `SELECT ... FINAL WHERE (tenant_id, gts_id, created_at, idempotency_key) IN (...)` over that partition's records. Because every partition already holds its own lock (step 2), these read phases are independent and are issued **concurrently** across partitions rather than one partition at a time; only their ClickHouse round-trips overlap, not their critical sections' mutual exclusion.
