...
| Feed position       | The opaque point in feed order a feed cursor carries, issued and interpreted by the plugin alone. Its age is the acceptance instant of the oldest entry after it, and a position with nothing after it is current (DESIGN §3.1, §3.6). |
...
| Retention floor     | The deployment's backfill window plus its operational replay horizon (`cpt-cf-usage-collector-fr-billing-retention-floor`), supplied to the plugin as `feed_retention_floor_secs`: the age past which the feed refuses a position (`cpt-cf-uc-plugin-fr-usage-feed`). This plugin requires more retention (`cpt-cf-uc-plugin-fr-per-type-retention`).                                                               |
...
- **a first read within the horizon**: a first read without a position starts inside the replay horizon by the acceptance-order slack, so every position it issues is within the horizon when issued; entries accepted earlier are reachable through the raw query, not the feed;
...
- **retention refusal**: a position older than the deployment's retention floor, or after which retention has removed an entry of a subscribed type, **MUST** be refused with the cursor-beyond-retention error rather than served as a silently truncated range; a position within the replay horizon **MUST** be served; and a current position, with nothing after it, is never refused on the retention floor. The known shortfalls of the removed-entry check are listed in [§13](#13-open-questions).
...
- **Invalid config / unreachable database / missing TimescaleDB extension / a failed startup check** (an unsafe durability setting): startup fails fast; the plugin does not register and the host does not bind it.
...
1. The core requests a feed page for a subscription under a compiled scope, after a position, optionally bounded by a later position, with a page limit.
2. The plugin reads the settled, in-scope entries of the subscribed types after that position, in feed order, up to the limit.
...
- **First read**: no position is given — the plugin starts inside the replay horizon as `cpt-cf-uc-plugin-fr-usage-feed` states.
...
- A consumer resumes from a position older than the deployment's retention floor, or after which retention has removed an entry of a subscribed type.

**Main Flow**:

1. The core requests a feed page after that position.
2. The plugin finds a retention mark of a subscribed type above the position, or finds the first entry after the position too old for the retention floor (DESIGN §3.6).
...
- **Within the replay horizon**: the position is served normally, except in the known shortfalls of [§13](#13-open-questions).
- **Nothing after the position**: the position is served and a head position returned; it is never refused on the retention floor.
...
- [ ] Feed pages omit every entry outside the host-supplied scope, are complete behind every returned position under concurrent writers, snapshot-consistent, place an invalidation after its target, return a head position on a quiet subscription, replay identically when bounded, serve a position within the replay horizon, and refuse a position older than the retention floor or after which retention has removed an entry of a subscribed type.
...
- The deployment supplies its replay horizon (`feed_replay_horizon_secs`) and its retention floor (`feed_retention_floor_secs`) to the plugin as configuration, because the SPI carries neither. The retention rule of `cpt-cf-uc-plugin-fr-per-type-retention` adds the acceptance-order slack to the gear's retention floor, and a first read starts the acceptance-order slack inside the horizon rather than at the oldest retained entry, so a new consumer's feed starts that much short of the horizon. The gear states no first-read start, so this is an open question for the gateway.
...
1. Whether the replay horizon and the retention floor should reach the plugin through the SPI rather than configuration ([§11](#11-assumptions)).
2. Where a first read without a cursor starts (`cpt-cf-uc-plugin-fr-usage-feed`).
3. The raw-list SPI return type: the gear's trait returns an `ODataPage` while its raw-query sequence returns a keyset (`cpt-cf-uc-plugin-fr-raw-query`).

Known shortfalls against the gateway's cursor zones (`cpt-cf-usage-collector-fr-billing-retention-floor`), argued in DESIGN §3.6 Retention refusal:

1. The plugin tracks retention deletions with one mark per GTS type, which ignores the compiled scope. It can therefore refuse a cursor whose own scope lost nothing, including a cursor within the replay horizon and one the gateway calls current (`cpt-cf-uc-plugin-fr-usage-feed`).
2. Only a transaction that holds an id open for a long time can cause these cases. A cursor polled at the head can be refused by a retention mark only while such a transaction has been open for at least the replay horizon, less the time between the consumer's pages. The floor-related cases need one open for longer than roughly the retention floor less twice the acceptance-order slack, less the time between the consumer's pages: a polled cursor refused on the retention floor, and a cursor older than the retention floor served an empty page rather than refused. That empty page departs from the gateway MUST that a cursor older than the retention floor is refused. Nothing is silently truncated, because a retention mark still refuses the cursor after any deletion. The horizon-lag gauge surfaces such a transaction (DESIGN §3.6 Retention refusal, §4.3).
