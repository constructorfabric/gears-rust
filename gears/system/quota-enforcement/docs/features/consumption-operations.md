...
**Steps**:
1. [x] - `p1` - Caller sends `POST /v1/quota-enforcement/operations/rollback` with a `RollbackRequest` carrying the original debit's own attribution (tenant, subjects, metric, metadata, optional resource) and both idempotency keys; the caller may not supply a subject key, so the server re-authorizes that attribution and recomputes the key the debit was recorded under - `inst-rlb-request`
...
3. [x] - `p1` - DB: look up the original committed debit by its full scope (tenant, the recomputed subject key, the `debit` operation type, and the original key) and require its recorded authorized attribution to equal this request's, so a caller admitted for another metric or resource over the same subjects cannot reverse it; lease-commit-derived debits are addressable through the commit call's idempotency key exactly like direct debits (`cpt-cf-quota-enforcement-fr-rollback`) - `inst-rlb-lookup`
