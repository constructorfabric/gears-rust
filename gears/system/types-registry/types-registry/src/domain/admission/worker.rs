...
//! [`ItemFailure`]: an **outcome** recorded on the operation item, not a fault of
//! the worker. Retrying it would produce the same answer forever and burn the
//! outbox's attempt budget on a decision that is already final. So the two travel
//! in different positions: `Err(WorkerError)` versus `Ok(_)` with a failed item.
//!
//! # P0 scope
//!
...
pub enum WorkerError {
    ...
    /// Not a fault, and never reaches a caller: [`run_operation`] catches it and
    /// reports the outcome the other pass recorded. It exists as an error because
    /// rolling the commit transaction back is the only way to *not* write an entity
    /// behind an item that is already terminal.
...
pub struct ItemFailure {
    ...
    /// and there is no borrow of it here. Making the field owned-or-borrowed keeps
    /// [`Self::from_payload`] able to return the real reason instead of a placeholder
    /// — every constructor at a failure site still passes a `&'static str`.
...
impl ItemFailure {
    ...
    ///
    /// Without this, a redelivery and a first pass reported *different shapes of the
    /// same fact*: the first returned `{reason, message}`, the replay returned
    /// `reason: "recorded"` with the whole JSON object stuffed into `message`.
    /// Invisible on the wire today — REST reads `error_payload` from the row — but
    /// T16 counts refusals by `reason`, and a metric that reads `recorded` for every
    /// redelivered item counts nothing.
    ///
