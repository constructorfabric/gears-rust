...
- [x] These routes are the **platform-plane** API for global entities: they keep the authentication they have, stay `.exposed()`, and no handler assumes a tenant scope. `.anonymous()` is **not** used. Ceiling C8 is commented where the routes are registered
