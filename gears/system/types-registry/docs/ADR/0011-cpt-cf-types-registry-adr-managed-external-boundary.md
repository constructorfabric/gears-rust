...
Live reverse-impact query is retained, but closing the boundary hollows out most of what it was for, and saying so is more useful than leaving the old description standing. It can no longer report anything about a Managed Entity: no externally managed entity may depend on one, so the answer is always empty. What it can still report is external dependents of an *externally managed* entity — a question entirely inside the source's own universe, which the source's own tooling generally answers better. It therefore informs no platform decision at all and survives as an optional pass-through diagnostic, which is why ADR-0007 keeps it out of the mandatory profile.
