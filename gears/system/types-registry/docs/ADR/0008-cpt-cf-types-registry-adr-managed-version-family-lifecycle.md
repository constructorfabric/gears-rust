...
Without the one-usable-member invariant there is nothing to serialize: concurrent admissions into one family are independent, and nothing has to be demoted atomically with anything else. The family-scoped compare-and-swap that invariant would require is therefore unnecessary.
...
* There is no one-usable-member invariant, and therefore no family-scoped serialization point, no deletion-ordering rule to protect it, and no rejection rule for out-of-order admission. None of the three would describe anything a user wants; each exists only to keep a derived value truthful.
* Concurrent admissions into one family do not contend, so no family-wide compare-and-swap sits on a write path that every gear uses at startup.
