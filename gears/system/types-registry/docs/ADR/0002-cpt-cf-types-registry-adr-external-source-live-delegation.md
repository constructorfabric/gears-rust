...
ADR-0011 closed the managed–external boundary in both directions. That is what lets the persistence prohibition above stand with **no** exception: the one exception with a real argument behind it — an external identifier held as the label on a dependency edge toward a Managed Entity — records a relationship the closed boundary makes unrepresentable.
