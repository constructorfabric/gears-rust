...
That dry run is not a facility of this job. It is the general dry-run mode of ADR-0012 applied to `kind = purge`, and it inherits that mode's rules: it runs the same check sequence as the real purge, commits nothing, participates in the request fingerprint so a dry run and the real purge under one key stay distinct requests, and proves nothing about a purge submitted later, since an entity's eligibility can change in between.
...
Retired Source Claim reservations from ADR-0011 are released by the same exception. A Registry Source Plugin is a registered Instance, so purging it removes its reservations along with its identity, and the claimed identifier space becomes registrable again. Placing those reservations out of scope, on the ground that only a claim takeover should release them, would carve out an identical hazard for different treatment, since a released claim space rebinds a persisted reference exactly as a released identifier does. Takeover is the safe path, transferring a claim with no unreserved interval and available in every deployment; purge is the deliberate one.
