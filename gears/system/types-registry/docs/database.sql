...
-- and not three more values in that enumeration.
--
...
-- the plugin columns are cleared. A retired reservation therefore still
-- names the plugin it belonged to, which is what a successor's continuity
-- assertion in a claim takeover is about. After a takeover the row names the
-- successor and the predecessor lives on in the operation history; a second row
-- is not possible, since the pattern is unique.
--
