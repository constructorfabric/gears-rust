...
The managed identifier profile is narrower than the GTS grammar in two ways, adopted for the same reason as the dialect restriction below: each keeps a platform guarantee decidable. A managed GTS Identifier **MUST NOT** carry a minor version, so it names one mutable logical entity within one major rather than an immutable snapshot (ADR-0004). A managed GTS Identifier **MUST NOT** carry an explicit UUID tail, because the Registry Reference derivation passes such a tail through unchanged and two different identifiers embedding the same tail would resolve to one reference; the invariant that different identifiers resolve to different references is therefore imposed on what may be admitted rather than guaranteed by the derivation (ADR-0001). Neither restriction reaches an Externally Managed Entity, whose identifiers its source owns.
...
Decisions that are deliberately not settled by this PRD. Each is recorded in a design note or ADR; this table exists so that a reader of the PRD alone does not mistake the surrounding requirements for a closed set. Entry numbers are stable and are never reused. A question leaves this table once its answer has a home — in a requirement above, in DESIGN, or in an ADR — so a gap in the numbering marks a closed question rather than a missing one.

| # | Question | Affects | Recorded in |
|---|----------|---------|-------------|
| 4 | When deprecation is introduced after P1, whether it lands as a third Lifecycle Status or as an annotation orthogonal to lifecycle, and whether the managed and source-asserted halves take the same shape — P1 defers both, so the answer now covers relaying a source assertion as well as authoring one | `cpt-cf-types-registry-fr-lifecycle` | ADR-0008 |
...
| 9 | Which platform-defined control-plane types exist, and the concrete invariants their built-in validators enforce — the federation half is settled in DESIGN: one type derived from `gts.cf.toolkit.plugins.plugin.v1~`, carrying a plugin's Source Claims, served entity kinds, and capability declarations. The P2 Validation Hook declaration remains open | `cpt-cf-types-registry-fr-registry-source-routing` — partly closed | ADR-0012, DESIGN |
...
| 14 | The concrete value of the query expansion maximum, and whether the resulting Concrete Reference Set carries a staleness contract. The set is now narrowed to what is available to the requesting tenant, so a staleness answer has to cover a verdict changing with no mutation to any entity in the set (ADR-0010) as well as ordinary registry change | `cpt-cf-types-registry-fr-type-query-assistance` | ADR-0001 |
...
| 17 | How a tenant moving within the hierarchy is reconciled with the visible audience of the entities it owns and can see. Out of scope for P1 per ADR-0009, and load-bearing as such: it is the only state in which a dependency could make a visible entity unavailable, so bringing it into scope reopens open question 1 and requires traversing dependency edges during availability evaluation | `cpt-cf-types-registry-fr-tenant-ownership` | ADR-0009, ADR-0010 |
...
| 25 | How the entities of an offboarded tenant are retired. Ordinary deletion belongs to the owning tenant and ADR-0013 requires an entity to be `DELETED` before it can be purged, so once a tenant is removed nobody can delete its entries and the platform plane cannot author in its place. The answer also has to decide what happens to dependents in other tenants that reference the departing tenant's contracts | `cpt-cf-types-registry-fr-tenant-ownership`, `cpt-cf-types-registry-fr-lifecycle` | not yet recorded; adjacent to entry 17 |
