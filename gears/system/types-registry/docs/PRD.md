...
This does not make SDK caching P1. What is P1 is the server-side capability and the metadata; whether the SDK stores, validates, and evicts on the caller's behalf remains `cpt-cf-types-registry-fr-client-cache`. A P1 caller that holds resolved content in process memory can keep it correct by hand with these two facilities.
...
Caching itself is a P2 optimization: a client that never caches and always resolves is correct in P1. What P1 requires is the pair of server-side facilities that make caching affordable and correct — the emitted validator and the conditional read that honours it — both specified above. What remains for P2 is the SDK doing it on the caller's behalf: where entries live, when they are evicted, how a cold start behaves, and how a batch poll is scheduled. A P1 caller may use both facilities directly.
...
- [ ] Independent candidates that pass every check are admitted even when another branch of the same batch fails; a candidate whose selected in-batch dependency failed is reported as blocked and creates nothing; a cyclic dependency component is admitted atomically or not at all; and every failure identifies the offending members with diagnostics sufficient for correction and retry.
...
Decisions that are deliberately not settled by this PRD. Each is recorded in a design note or ADR; this table exists so that a reader of the PRD alone does not mistake the surrounding requirements for a closed set. A question leaves this table once its answer has a home — in a requirement above, in DESIGN, or in an ADR.

Entries were renumbered from 1 once, after the first round of closures left the sequence mostly gaps. From here numbers are stable and are never reused, so a gap marks a closed question rather than a missing one, and a later closure renumbers nothing.
...
|---|----------|---------|-------------|
| 1 | Deprecation is deferred beyond P2 and is not scheduled. When it is introduced, whether it lands as a third Lifecycle Status or as an annotation orthogonal to lifecycle, and whether the managed and source-asserted halves take the same shape — P1 defers both, so the answer now covers relaying a source assertion as well as authoring one | `cpt-cf-types-registry-fr-lifecycle` | ADR-0008 |
| 5 | The concrete deadline after which a mutation returns an operation reference instead of a result, and operation retention and cancellation authority | `cpt-cf-types-registry-interface-sdk`, `cpt-cf-types-registry-interface-rest` | ADR-0012 |
| 7 | Where the registrant-side retry and readiness-gating obligation is documented for gear authors, since it moves out of Types Registry | `cpt-cf-types-registry-fr-two-phase-init` | ADR-0012 |
| 8 | Quotas and limits: entities per tenant, schema size, derivation depth, dependency fan-out, retained revisions | `cpt-cf-types-registry-fr-register-schemas` | not yet recorded |
| 10 | Observability and audit requirements for a fail-closed federated control plane | all federation requirements | [registry-federation-external-sources.md](./design-notes/registry-federation-external-sources.md) |
| 11 | The admissible transition set for casting, given that it lies outside GTS OP#9 | `cpt-cf-types-registry-fr-casting` | this PRD, section 5.3 |
| 12 | How a tenant moving within the hierarchy is reconciled with the visible audience of the entities it owns and can see. Out of scope for P1 per ADR-0009, and load-bearing as such: it is the only state in which a dependency could make a visible entity unavailable, so bringing it into scope reopens the closed question of dependency-closure caching and requires traversing dependency edges during availability evaluation | `cpt-cf-types-registry-fr-tenant-ownership` | ADR-0009, ADR-0010 |
| 15 | Concrete validator representation for a resolution result, given that it must cover entity revision, availability inputs, and plugin configuration rather than the entity alone. Narrowed in DESIGN: for an Externally Managed Entity the validator must be *recoverable* rather than merely comparable, because Types Registry holds no copy of an `external_revision` and a conditional read can only obtain the source's token from the validator the caller presents. A digest is therefore admissible for the managed components and not for the external ones | `cpt-cf-types-registry-fr-cache-freshness-metadata` | ADR-0010, [managed-entity-resource-versioning.md](./design-notes/managed-entity-resource-versioning.md) |
| 16 | How a registration grant encodes the identifier region it covers, and the action vocabulary it is granted against | `cpt-cf-types-registry-fr-registration-authority` | not yet recorded; see [PERMISSION_GTS_TYPE.md](../../../../docs/arch/authorization/PERMISSION_GTS_TYPE.md) |
| 17 | How the entities of an offboarded tenant are retired. Ordinary deletion belongs to the owning tenant and ADR-0013 requires an entity to be `DELETED` before it can be purged, so once a tenant is removed nobody can delete its entries and the platform plane cannot author in its place. The answer also has to decide what happens to dependents in other tenants that reference the departing tenant's contracts | `cpt-cf-types-registry-fr-tenant-ownership`, `cpt-cf-types-registry-fr-lifecycle` | not yet recorded; adjacent to entry 12 |
