...
| 26 | How discovery excludes unstable Type Schemas. A GTS wildcard has no negation and `GET /entities` has no stability filter, so a catalogue view that wants published contracts only cannot express it today. The answer also has to decide whether the filter is a distinct parameter or a value of an existing one, and whether it applies to Externally Managed Entities, whose majors the platform does not interpret | `cpt-cf-types-registry-fr-type-query-assistance`, `cpt-cf-types-registry-fr-validate-schema-compat` | ADR-0015 |
