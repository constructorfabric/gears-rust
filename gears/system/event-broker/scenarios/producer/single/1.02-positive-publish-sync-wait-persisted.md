...
Opting into synchronous persistence makes the broker hold the response until the storage backend has durably persisted the event, returning `201` instead of the default `202`. Synchronous persistence is requested with the standard `Prefer: wait` header (RFC 7240); `Prefer: respond-async` names the default async behaviour.
