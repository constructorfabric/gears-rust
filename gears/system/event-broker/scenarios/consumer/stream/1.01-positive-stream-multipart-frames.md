...
- The session cursor auto-advances with delivery — as events are emitted, `evbk_group_offsets(group, "acme.orders.v1", 0)` follows the stream. The consumer does not SEEK mid-stream to record progress (SEEK is pre-stream-only); durable progress is the consumer's own (ADR-0006).
