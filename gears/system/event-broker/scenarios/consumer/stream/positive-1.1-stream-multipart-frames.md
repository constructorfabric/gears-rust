...
- Run [Pre-stream SEEK to earliest](../positions/positive-1.1-seek-earliest.md) — cursor seeded at `99`, so emission begins at offset `100`.
...
- Streaming does not itself advance the cursor — `evbk_group_offsets(group, "acme.orders.v1", 0)` stays at `99` until the consumer acks via [positions](../positions/positive-1.8-forward-seek-while-streaming.md).
