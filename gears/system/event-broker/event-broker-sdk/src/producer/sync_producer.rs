...
impl SyncProducer {
    ...
    pub(crate) async fn build(
        ...
        let producer_id = match builder.chain_mode {
            ...
            mode => {
                let client_agent = match builder.client_agent.as_deref() {
                    Some(client_agent) => client_agent,
                    None => EventBrokerSdk::default_client_agent(),
                };
                if let Some(reuse_id) = builder.reuse {
                    ...
                    Some(backend.register_producer(ctx, mode, client_agent).await?)
                }
            }
        };
    ...
    pub async fn reset_chain(
        ...
        match scope {
            ...
            ResetScope::Partition { topic, partition } => {
                self.chain_state.reset(&(pid, topic.to_owned(), partition));
            }
            ResetScope::Topic(_topic) => {
    ...
    fn build_meta(&self, topic: &str, partition: u32) -> Option<ProducerMeta> {
        match (self.chain_mode, self.producer_id) {
            ...
            (ProducerMode::Monotonic, Some(pid)) => {
