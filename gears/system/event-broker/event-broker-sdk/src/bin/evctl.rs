...
#[derive(Parser)]
#[command(name = "cf-evctl", about = "Demo event-broker CLI over the SDK REST transport")]
struct Cli {
    /// Broker base URL, e.g. https://host:8080
    #[arg(long, global = true, default_value = "http://127.0.0.1:8080")]
    broker: String,
    /// Caller tenant id. Defaults to the standalone demo tenant.
    #[arg(long, global = true, default_value = "11111111-1111-1111-1111-111111111111")]
    tenant: Uuid,
...
struct ProduceArgs {
    ...
    #[arg(long = "type", default_value = "gts.cf.core.events.event.v1~cf.core.demo.spread.v1~")]
    type_id: String,
...
struct ConsumeArgs {
    ...
    #[arg(long, default_value = "gts.cf.core.events.topic.v1~cf.core.demo.topic.v1")]
    topic: String,
...
fn read_stored_group(conn: &rusqlite::Connection) -> Result<Option<String>, String> {
    ...
    conn.query_row("SELECT group_id FROM group_singleton WHERE k = 1", [], |row| {
        row.get::<_, String>(0)
    })
    .optional()
...
async fn resolve_shared_group(
    ...
    let created = broker
        .create_consumer_group(
            ctx,
            CreateConsumerGroupRequest { client_agent: "cf-evctl".to_owned(), description: None },
        )
        .await
        .map_err(|e| e.to_string())?;
    let gts = created.id.to_gts();
    match conn.execute("INSERT INTO group_singleton (k, group_id) VALUES (1, ?1)", [&gts]) {
        Ok(_) => {
            ...
            if e.code == rusqlite::ErrorCode::ConstraintViolation =>
        {
            for _ in 0..10 {
                if let Some(id) = read_stored_group(&conn)? {
                    eprintln!("joined shared group {id} (a peer won the claim; our {gts} is orphaned)");
                    return parse_stored_group(&id);
                }
...
async fn reset_group_store(
    ...
    if !store.exists() {
        eprintln!("group store {} does not exist; nothing to reset", store.display());
        return Ok(());
    }
    let conn = rusqlite::Connection::open(store)
        .map_err(|e| format!("open group store {}: {e}", store.display()))?;
    if let Some(id) = read_stored_group(&conn).unwrap_or(None) {
        if let Some(group) = ConsumerGroupId::try_from_gts(&id) {
...
fn summarize(frame: &WireFrame) -> String {
    match frame {
        ...
        WireFrame::Control {
            ...
            reason,
            ..
        } => format!("control {code:?} reason={reason:?}"),
    }
}
...
async fn run() -> Result<(), String> {
    ...
    if cli.debug {
        ...
        tracing_subscriber::fmt()
            .with_env_filter(tracing_subscriber::EnvFilter::new("event_broker_sdk::rest=debug"))
