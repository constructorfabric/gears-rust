...
impl PositionViolation {
    ...
    /// The problem-body entry for this violation. The field names the `positions`
    /// map entry by its topic key; the partition (a validated index) rides in the
    /// description. Neither reproduces the submitted position value - the range
    /// it names is broker state.
    #[must_use]
    pub fn as_field_violation(&self) -> FieldViolation {
        FieldViolation::new(
            format!("positions.{}", self.topic),
            format!(
                "partition {}: the position is {} the valid range [{}, {}]",
                self.partition,
            ...
            CanonicalError::NotFound {
                ...
                match resource_type.as_deref() {
                    ...
                    Some(resources::PARTITION) => {
                        Self::StorageBackend(StorageBackendError::PartitionNotFound {
                            detail,
                            instance,
                        })
                    }
            ...
            CanonicalError::FailedPrecondition {
                ...
                match (resource_type.as_deref(), reason) {
                    ...
                    (_, Some(reasons::IN_TX_OFFSETS_NOT_SUPPORTED)) => {
                        Self::OffsetManager(OffsetManagerError::InTxNotSupported {
                            detail,
                            instance,
                        })
                    }
