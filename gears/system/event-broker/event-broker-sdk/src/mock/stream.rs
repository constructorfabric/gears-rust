...
pub(super) fn open_stream(broker: MockBroker, sub_id: SubscriptionId) -> FrameStream {
    let stream = async_stream::try_stream! {
        ...
        // ── 0. Emit initial Topology frame ────────────────────────────────────
        {
            ...
            let offsets: Vec<(PartitionSlot, i64)> = group.map(|g| {
                sub.assigned.iter().map(|(topic, partition)| {
                    let off = g.cursor.get(&(topic.clone(), *partition)).map(|c| c.offset).unwrap_or(0);
                    (PartitionSlot { topic_ix: 0, partition: *partition }, off)
                }).collect()
            }).unwrap_or_default();
            let last_examined: Vec<(PartitionSlot, i64)> = sub.assigned.iter().map(|(topic, partition)| {
                let le = core.topics.get(topic.as_str())
                    .and_then(|t| t.next_offset.get(partition).copied())
                    .unwrap_or(0)
                    .saturating_sub(1);
                (PartitionSlot { topic_ix: 0, partition: *partition }, le)
            }).collect();
            yield WireFrame::Topology { topology_version, assigned, offsets, last_examined };
        }
        ...
        loop {
            ...
            // Check for topology change (rebalance during active stream).
            {
                ...
                if current_tv > sub.topology_version {
                    ...
                    let group_state = core.groups.get(&sub.group);
                    let new_offsets: Vec<(PartitionSlot, i64)> = group_state.map(|g| {
                        sub.assigned.iter().map(|(topic, partition)| {
                            let off = g.cursor.get(&(topic.clone(), *partition)).map(|c| c.offset).unwrap_or(0);
                            (PartitionSlot { topic_ix: 0, partition: *partition }, off)
                        }).collect()
                    }).unwrap_or_default();
                    let new_last_examined: Vec<(PartitionSlot, i64)> = sub.assigned.iter().map(|(topic, partition)| {
                        let le = core.topics.get(topic.as_str())
                            .and_then(|t| t.next_offset.get(partition).copied())
                            .unwrap_or(0)
                            .saturating_sub(1);
                    ...
                    yield WireFrame::Topology {
                        topology_version: current_tv,
                        assigned: new_assigned,
                        offsets: new_offsets,
                        last_examined: new_last_examined,
                    };
