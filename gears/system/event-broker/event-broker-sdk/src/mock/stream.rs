...
pub(super) fn open_stream(broker: MockBroker, sub_id: SubscriptionId) -> FrameStream {
    // A2: clear the `streaming` marker when this stream ends or is dropped, so a
    // later stream()/seek() on the same subscription is allowed again. A sync Drop
    // guard (the marker set is a std::sync::Mutex) - runs on generator completion
    // or drop.
    struct StreamGuard {
        set: std::sync::Arc<std::sync::Mutex<std::collections::HashSet<SubscriptionId>>>,
        id: SubscriptionId,
    }
    ...
    let stream = async_stream::try_stream! {
        ...
        let _stream_guard = StreamGuard { set: broker.streaming.clone(), id: sub_id };
        ...
        let baseline: Result<(i64, Vec<(String, u32)>, Vec<PartitionPosition>), EventBrokerError> = {
            ...
            match core.subscriptions.get(&sub_id) {
                ...
                Some(sub) => {
                    ...
                    Ok((topology_version, assigned, positions))
                }
            }
        };
        let (baseline_tv, mut current, baseline_positions) = baseline?;
        ...
        loop {
            ...
            // Detect a rebalance (topology change) on this subscription.
            {
                ...
                if current_tv > observed_tv {
                    ...
                    if gained || lose_all {
                        ...
                        let group_id = core.subscriptions.get(&sub_id).map(|s| s.group.clone());
            ...
            let mut progress_positions: Vec<PartitionPosition> = Vec::new();
            {
                ...
                for (topic, partition) in &assigned {
                    ...
                    let group_id = core.subscriptions.get(&sub_id).map(|s| s.group.clone());
                    if let Some(gid) = group_id {
                        if let Some(g) = core.groups.get_mut(&gid) {
