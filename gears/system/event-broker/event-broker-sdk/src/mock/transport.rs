...
fn resolve_at_timestamp(core: &Core, topic: &str, partition: u32, ts: &str) -> i64 {
    ...
    if let Some(first) = log.first() {
        if target <= first.event.occurred_at {
...
impl EventBroker for MockBroker {
    ...
    async fn stream(
        ...
        // the terminal control frame).
        {
            ...
            if sub.map(|s| s.terminated).unwrap_or(false) {
            ...
            if let Some(sub) = sub {
                ...
                if !unseeded.is_empty() {
                    return Err(EventBrokerError::PositionsNotSet {
                        unseeded,
                        detail: "SEEK every assigned partition before opening the stream"
                            .to_owned(),
    ...
    async fn seek(
        ...
        let group_id = core.subscriptions.get(&id).map(|s| s.group.clone());
