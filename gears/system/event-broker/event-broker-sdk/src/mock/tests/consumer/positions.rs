...
/// expects `409 StreamingInProgress` when a SEEK arrives while a stream is open.
/// The mock does not track an "open stream" precondition on `seek` - a `FrameStream`
/// is an independent in-process object and `seek` always succeeds. The mock's only
/// SEEK movement guard is the forward-only MAX rule on the GROUP cursor (verified
/// here): a backward SEEK is silently clamped, not rejected. The 409 is HTTP-only.
#[tokio::test]
