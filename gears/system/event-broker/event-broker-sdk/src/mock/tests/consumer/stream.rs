...
async fn next_frame(
    ...
    match tokio::time::timeout(Duration::from_millis(200), s.next()).await {
        Ok(item) => item,
        Err(_) => None,
    }
}
...
///
/// Opening `:stream` with a subscription id that never existed errors. (Divergence:
/// the scenario maps this to `404 Not Found`; the in-process mock surfaces an
/// `EventBrokerError::Internal` carrying "not found" from `open_stream`'s lookup -
/// there is no HTTP status. The asserted contract is "unknown id → stream error".)
#[tokio::test]
async fn s1_05_negative_stream_unknown_subscription() {
    ...
    // `stream()` itself returns Ok (it only guards `terminated`); the lookup failure
    // surfaces as the first polled frame being an error.
    let mut s = broker.stream(&c, unknown).await.unwrap();
    assert!(
        matches!(next_frame(&mut s).await, Some(Err(_))),
        "an unknown subscription id yields a stream error on first poll"
    );
}
...
///
/// DIVERGENCE (StreamingInProgress is unrepresentable in the mock): the scenario
/// expects `409 StreamingInProgress` for a second concurrent `:stream` (or any
/// non-DELETE call) on a subscription with an open stream. The mock does not track a
/// single-stream-per-subscription invariant - each `stream()` returns an independent
/// `FrameStream` object and a second open succeeds. Closest real assertion: opening a
/// second stream on the same id succeeds (the single-stream guard is HTTP-only).
#[tokio::test]
        ...
        .join(
            ...
            JoinRequest {
                ...
                interests: vec![SubscriptionInterest {
                    ...
                    types: vec![
                        "gts.cf.core.events.event_type.v1~test.mock.broker.NOMATCH.v1".to_owned(),
                    ],
                    filter: None,
                }],
