...
async fn s1_01_positive_problem_details_envelope() {
    ...
    let c = ctx();
    let missing = ConsumerGroupId::from_gts(
        "gts.cf.core.events.consumer_group.v1~deadbeef-0000-0000-0000-000000000000",
    );
...
/// The mock's `stream()` is lazy (errors surface on poll), so this asserts the
/// eager not-found path via `get_subscription` on an unknown id.
///
/// DIVERGENCE (partial): the mock returns `EventBrokerError::Internal("...not
/// found")` for an unknown subscription rather than a dedicated
/// `SubscriptionNotFound`; the message still carries the not-found semantics the
/// `404` envelope conveys.
#[tokio::test]
async fn s1_04_negative_404_not_found() {
    ...
    let err = broker.get_subscription(&c, unknown).await.unwrap_err();
    let msg = format!("{err:?}");
    assert!(
        matches!(err, EventBrokerError::SubscriptionNotFound { .. }) || msg.contains("not found"),
        "unknown subscription must surface a not-found error: {msg}"
    );
}
