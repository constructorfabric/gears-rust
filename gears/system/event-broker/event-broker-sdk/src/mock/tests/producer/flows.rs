...
async fn s1_08_unknown_producer() {
    ...
    assert!(
        matches!(
            err,
            crate::error::EventBrokerError::InvalidProducerOptions { .. }
        ),
        "expected InvalidProducerOptions for an unknown producer, got {err:?}"
    );
}
...
// -- Test-local helper ---------------------------------------------------------
// Find a partition_key string that hashes (ADR-0002 murmur3) to `target`
