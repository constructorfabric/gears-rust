...
pub(super) fn run_rebalance(group_id: &ConsumerGroupId, core: &mut Core) {
    ...
    for topic in &all_topics {
        ...
        if s >= n {
            ...
            for i in 0..n {
                new_assignments.insert((topic.clone(), i as u32), eligible[i]);
            }
        } else {
    ...
    // Re-initialize cursor entries (create-if-absent, sticky).
    let group = core.groups.get_mut(group_id).unwrap();
    for (topic_partition, _) in &new_assignments {
        group.cursor.entry(topic_partition.clone()).or_default();
    }
