...
pub enum ResolvedPosition {
    ...
    Latest,
    /// Broker-resolved: seek to the first offset at or after the given ISO-8601 timestamp.
    /// The mock resolves this to `Earliest` (offset 0) since it does not implement
    /// true timestamp-indexed lookup.
    AtTimestamp(String),
}
...
impl LocalDbOffsetManager {
    pub fn new(db: toolkit_db::Db, table: impl Into<String>, fallback: Fallback) -> Self {
        Self {
            _db: db,
            _table: table.into(),
            fallback,
