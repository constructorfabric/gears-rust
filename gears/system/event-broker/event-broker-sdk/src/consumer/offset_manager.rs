...
// ---- Traits ------------------------------------------------------------------
//
// Offsets are a *client-side* concern: the broker does not store group offsets
// (SEEK is the only cursor mechanism), so the consumer owns its cursor. Reading
...
pub struct LocalDbOffsetManager {
    db: toolkit_db::Db,
    fallback: Fallback,
    overrides: HashMap<(TopicId, u32), i64>,
}
...
impl LocalDbOffsetManager {
    ...
    pub fn with_overrides(
        mut self,
        overrides: impl IntoIterator<Item = ((TopicId, u32), i64)>,
    ) -> Self {
...
pub struct InMemoryOffsetManager {
    inner: Mutex<HashMap<(ConsumerGroupId, TopicId, u32), i64>>, // (group, topic, partition) → last-processed offset
    fallback: Fallback,
    overrides: HashMap<(TopicId, u32), i64>,
}
