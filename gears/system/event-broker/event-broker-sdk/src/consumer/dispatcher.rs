...
impl PartitionSlowState {
    ...
    #[cfg(test)]
    pub(crate) fn consecutive_slow_handlers(&self) -> u16 {
        self.consecutive_slow_handlers
    }

    #[cfg(test)]
    pub(crate) fn is_slow_detected(&self) -> bool {
        self.slow_detected
    }
...
pub(crate) async fn enqueue_partition_batch(
    buffers: &Arc<RwLock<HashMap<TopicPartitionKey, PartitionEventBuffer>>>,
...
pub(crate) fn processed_count_from_outcome(
    ...
    match outcome {
        ...
        BatchHandlerOutcome::AdvanceThrough { offset } => batch_events
            .iter()
            .position(|event| event.offset == *offset)
            .map(|idx| Some(idx + 1))
            .ok_or_else(|| EventBrokerError::InvalidConsumerOptions {
                detail: format!(
                    "batch progress offset {offset} is not present in delivered offsets {:?}",
                    batch_events
                        .iter()
...
pub(crate) fn processed_count_from_delivered_offset(
    ...
    batch_events
        ...
        .ok_or_else(|| EventBrokerError::InvalidConsumerOptions {
            detail: format!(
                "committed offset {offset} is not present in delivered offsets {:?}",
                batch_events
                    .iter()
                    .map(|event| event.offset)
                    .collect::<Vec<_>>()
    ...
    OM: CommitOffsetInTx + 'static,
{
    pub async fn run(
        ...
        let cursors: Arc<RwLock<HashMap<TopicPartitionKey, PartitionCursor>>> =
            Arc::new(RwLock::new(HashMap::new()));
        let buffers: Arc<RwLock<HashMap<TopicPartitionKey, PartitionEventBuffer>>> =
            Arc::new(RwLock::new(HashMap::new()));
        let slow_states: Arc<RwLock<HashMap<TopicPartitionKey, PartitionSlowState>>> =
            Arc::new(RwLock::new(HashMap::new()));
        ...
        'outer: loop {
            ...
            let mut stream = match self.broker.stream(&ctx, sub_id).await {
                ...
                Err(e) => {
                    warn!(error = %e, "transactional stream open failed; re-joining");
                    assignment = self
                ...
                match frame_result {
                    ...
                    Ok(WireFrame::Control {
                        ...
                        if code == ControlCode::Terminal {
                            trace!(?reason, "transactional terminal control frame; re-JOIN");
                            break;
                        }
                    }
                    ...
                    Err(e) => {
                        warn!(error = %e, "transactional stream frame error; ending stream");
                        break;
                    }
                }
            }
    ...
    async fn dispatch_event(
        ...
        cursors: &Arc<RwLock<HashMap<TopicPartitionKey, PartitionCursor>>>,
        buffers: &Arc<RwLock<HashMap<TopicPartitionKey, PartitionEventBuffer>>>,
        slow_states: &Arc<RwLock<HashMap<TopicPartitionKey, PartitionSlowState>>>,
        ...
        let batch_events = drain_partition_batch(buffers, &key, self.batch_max_events).await;
        self.process_batch(
            group_id,
            cursors,
            slow_states,
    ...
    async fn drain_subscription_buffers(
        ...
        loop {
            let Some((key, batch_events)) =
                next_buffered_partition_batch(buffers, self.batch_max_events).await
            else {
                break;
            };
            let _ = self
    ...
    async fn observe_enqueue_slow_signal(
        ...
        let mut guard = slow_states.write().await;
        ...
        if let Some(signal) = signal {
            ...
            self.emit_slow_consumer_events(group_id, sub_id, key, signal, affected)
                .await;
            return true;
        }
        false
                                    ...
                                    if seek_om.commit(
                                    ...
                                    ).await.is_ok() {
                                        {
                                            ...
                                            if let Some(cursor) = guard.get_mut(&key) {
                                                if cursor.latest_offset() >= latest_offset {
