...
/// Tracks the contiguous processed frontier for one topic partition.
///
/// Out-of-order processed offsets are remembered, but the frontier only advances
/// through offsets that form a contiguous run after the last committed offset.
#[derive(Debug, Clone)]
...
impl PartitionFrontier {
    ...
    pub(crate) fn mark_acked(&mut self, offset: i64) -> i64 {
    ...
    fn advance(&mut self) -> i64 {
        loop {
            let next = self.committed.saturating_add(1);
            if !self.processed.remove(&next) {
                break;
            }
            self.committed = next;
        }
