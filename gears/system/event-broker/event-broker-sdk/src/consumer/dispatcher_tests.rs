...
use super::consumer::RoutedBatchHandler;
use super::dispatcher::{
    PartitionEventBuffer, PartitionSlowState, SlowConsumerReason, TopicPartitionKey,
    enqueue_partition_batch,
};
use super::{
    BatchHandlerOutcome, CommitOffset, ConnectionDropReason, ConsumerBuffering, ConsumerBuilder,
    ConsumerCommitMode, ConsumerGroupRef, ConsumerHandler, ConsumerListenerSettings, ConsumerRetry,
...
struct AckAllBatchHandler {
    calls: Arc<Mutex<Vec<(String, u32, i64)>>>,
}
...
struct RecordingCommitOffsetManager {
    commits: Arc<Mutex<Vec<(ConsumerGroupId, TopicId, u32, i64)>>>,
}
...
struct BatchScopeRecorder {
    scopes: Arc<Mutex<Vec<(String, u32, usize)>>>,
...
struct TimelineRuntimeListener {
    timeline: Arc<Mutex<Vec<&'static str>>>,
}

#[async_trait::async_trait]
impl ConsumerRuntimeListener for TimelineRuntimeListener {
    async fn on_consumer_event(&self, event: &ConsumerRuntimeEvent) -> Result<(), ConsumerError> {
        if matches!(
            event,
            ConsumerRuntimeEvent::ProgressAdvanced { progress, .. }
                if progress.iter().any(|progress| progress.offset >= 0)
        ) {
            self.timeline.lock().unwrap().push("progress");
        }
        Ok(())
    }
}
...
fn slow_state_detects_handler_latency_strikes_and_resets_on_fast_completion() {
    ...
    assert_eq!(state.consecutive_slow_handlers(), 1);
    assert!(
        state
            .observe_handler_completion(Duration::from_millis(10), threshold, 2, 21)
            .is_none()
    );
    assert_eq!(state.consecutive_slow_handlers(), 0);
...
async fn partition_buffer_capacity_is_isolated_per_topic_partition() {
    let buffers: Arc<RwLock<HashMap<TopicPartitionKey, PartitionEventBuffer>>> =
        Arc::new(RwLock::new(HashMap::new()));
    let topic = "gts.cf.core.events.topic.v1~test.dispatcher.buffer.v1";
...
async fn runtime_dispatch_never_mixes_topics_or_partitions_in_handler_batches() {
    ...
    const ORDERS_TOPIC: &str = "gts.cf.core.events.topic.v1~test.mock.broker.orders.v1";
    const PAYMENTS_TOPIC: &str = "gts.cf.core.events.topic.v1~test.mock.broker.payments.v1";
    const ORDERS_EVENT: &str = "gts.cf.core.events.event_type.v1~test.mock.broker.order.v1";
    const PAYMENTS_EVENT: &str = "gts.cf.core.events.event_type.v1~test.mock.broker.payment.v1";
...
async fn parallelism_creates_independent_slots_with_shared_group_and_interests() {
    ...
    const TOPIC: &str = "gts.cf.core.events.topic.v1~test.mock.broker.parallel.v1";
    const GROUP: &str = "gts.cf.core.events.consumer_group.v1~test.mock.consumer.parallel.v1";
...
async fn wait_for_parallel_assignments(
    handle: &super::consumer::ConsumerHandle,
...
async fn slow_detection_emits_listener_events_and_drops_subscription_stream() {
    ...
    const TOPIC: &str = "gts.cf.core.events.topic.v1~test.mock.broker.slow.v1";
    const EVENT_TYPE: &str = "gts.cf.core.events.event_type.v1~test.mock.broker.slow.v1";
...
async fn slow_drop_reports_other_assignments_owned_by_same_subscription_slot() {
    ...
    const TOPIC: &str = "gts.cf.core.events.topic.v1~test.mock.broker.affected.v1";
    const EVENT_TYPE: &str = "gts.cf.core.events.event_type.v1~test.mock.broker.affected.v1";
...
async fn wait_for_slow_drop_affected_assignments(
    recorded: &Arc<Mutex<Vec<ConsumerRuntimeEvent>>>,
) -> BTreeSet<(String, u32)> {
    for _ in 0..100 {
        let events = recorded.lock().unwrap();
...
async fn runtime_listener_observes_representative_non_dlq_event_variants() {
    ...
    const TOPIC: &str = "gts.cf.core.events.topic.v1~test.mock.broker.listener_all.v1";
    const EVENT_TYPE: &str = "gts.cf.core.events.event_type.v1~test.mock.broker.listener_all.v1";
    ...
    let expected_after_stop = expected_before_stop
        ...
        .collect::<BTreeSet<_>>();
    let observed = wait_for_runtime_event_kinds(&recorded, &expected_after_stop).await;
...
async fn listener_failure_does_not_commit_drop_or_stop_consumer_events() {
    ...
    const TOPIC: &str = "gts.cf.core.events.topic.v1~test.mock.broker.listener_failure.v1";
    const EVENT_TYPE: &str =
        "gts.cf.core.events.event_type.v1~test.mock.broker.listener_failure.v1";
...
#[cfg(feature = "mock")]
async fn wait_for_timeline_entries(
    timeline: &Arc<Mutex<Vec<&'static str>>>,
    expected: &[&'static str],
) -> Vec<&'static str> {
    for _ in 0..100 {
        ...
        if expected
            .iter()
            .all(|expected| observed.iter().any(|entry| entry == expected))
        {
            return observed;
        }
        drop(observed);
    ...
    panic!(
        "timeline did not contain expected entries {expected:?}; observed={:?}",
...
async fn slow_listener_timeout_does_not_block_runtime_delivery_or_handler_processing() {
    ...
    const TOPIC: &str = "gts.cf.core.events.topic.v1~test.mock.broker.listener_timeout.v1";
    const EVENT_TYPE: &str =
        "gts.cf.core.events.event_type.v1~test.mock.broker.listener_timeout.v1";
...
async fn handler_latency_strikes_emit_listener_events_and_drop_subscription_stream() {
    ...
    const TOPIC: &str = "gts.cf.core.events.topic.v1~test.mock.broker.latency.v1";
    const EVENT_TYPE: &str = "gts.cf.core.events.event_type.v1~test.mock.broker.latency.v1";
...
async fn slow_drop_drains_buffer_before_rejoin_load_position() {
    ...
    const TOPIC: &str = "gts.cf.core.events.topic.v1~test.mock.broker.drain_rejoin.v1";
    const EVENT_TYPE: &str = "gts.cf.core.events.event_type.v1~test.mock.broker.drain_rejoin.v1";
...
#[cfg(feature = "mock")]
async fn wait_for_drain_rejoin_timeline(
    timeline: &Arc<Mutex<Vec<&'static str>>>,
) -> Vec<&'static str> {
    for _ in 0..100 {
        let observed = timeline.lock().unwrap().clone();
...
async fn async_auto_commit_uses_resolved_group_topic_partition_and_frontier_offset() {
    ...
    const TOPIC: &str = "gts.cf.core.events.topic.v1~test.mock.broker.audit.v1";
    const EVENT_TYPE: &str = "gts.cf.core.events.event_type.v1~test.mock.broker.event.v1";
