...
const TOPIC: &str = "gts.cf.core.events.topic.v1~orders.v1";
const EVENT_TYPE: &str = "gts.cf.core.events.event.v1~orders.rejected.v1";
...
fn dead_letter_record(offset: i64) -> DeadLetterRecord {
    DeadLetterRecord::builder(&raw_event(offset), "permanent failure")
        .group_id(ConsumerGroupId::from_gts(
            "gts.cf.core.events.group.v1~orders-projector",
        ))
        .attempts(3)
        .build()
}
