...
```rust
use event_broker_sdk::{CommitHandle, ConsumerGroupRef, EventHandler, Fallback,
                       HandlerOutcome, InMemoryOffsetManager, RawEvent};

struct BillingProjector;

#[async_trait]
impl EventHandler<CommitHandle, HandlerOutcome> for BillingProjector {
    async fn handle(&self, event: RawEvent, attempts: u16, commit: CommitHandle)
...
```rust
use event_broker_sdk::{LocalDbOffsetManager, RejectableOutcome, TxCommitHandle};

#[async_trait]
impl EventHandler<TxCommitHandle<LocalDbOffsetManager>, RejectableOutcome> for MyHandler {
    async fn handle(
        &self,
        event: RawEvent,
        attempts: u16,
        ...
        -> Result<RejectableOutcome, ConsumerError>
    {
        ...
        commit.commit_in_tx(&txn).await?;   // offset + business state atomic
        txn.commit().await?;
...
let handle = broker
    ...
    .tx_offset_manager(LocalDbOffsetManager::new(db.clone(), Fallback::Earliest))
