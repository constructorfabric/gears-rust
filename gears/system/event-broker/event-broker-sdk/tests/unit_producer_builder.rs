...
use event_broker_sdk::{EventBrokerError, ProducerBuilder, ProducerMode, ValidationTiming};
use uuid::Uuid;
...
fn stateless_plus_reuse_fails() {
    ...
    let b = base().chain_mode(ProducerMode::Stateless).reuse(pid);
    let err = b.validate_pub().unwrap_err();
    assert!(matches!(
        err,
        EventBrokerError::InvalidProducerOptions { .. }
...
fn default_chain_mode_is_chained() {
    let b = ProducerBuilder::new_unbound();
    assert_eq!(b.chain_mode, ProducerMode::Chained);
}
