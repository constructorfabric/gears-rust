...
#[derive(Debug, Clone, Deserialize)]
#[serde(deny_unknown_fields)]
// The shared `_interval_secs` suffix is deliberate: each field name is also its
// config-file key (deny_unknown_fields, no serde rename), and the suffix states
// the unit. Dropping it to satisfy the lint would rename the config keys and
// lose that, so the postfix stays.
...
/// A whole-second cadence that must be at least 1. Zero would make the schedule
/// permanently due, so the delivery session loop would emit that frame every
/// pass and never park - the failure documented for both the heartbeat and the
/// progress timer in `domain::streaming::session`. Rejecting it at deserialization
/// keeps that footgun out of any accepted config.
fn deserialize_nonzero_secs<'de, D>(deserializer: D) -> Result<u32, D::Error>
where
    D: serde::Deserializer<'de>,
{
    ...
    if secs == 0 {
        return Err(serde::de::Error::custom(
            "must be at least 1 second; 0 would spin the delivery session loop",
        ));
    }
    Ok(secs)
}
...
/// A topic's own settings entry and the instance-less entry for its type,
/// either of which may be absent. Returned by [`EventBrokerConfig::entries_for`].
type TopicEntryPair<'a> = (
    Option<&'a TopicSettingsEntry>,
    Option<&'a TopicSettingsEntry>,
);
...
impl EventBrokerConfig {
    ...
    ) -> Result<TopicEntryPair<'_>, TopicSettingsError> {
