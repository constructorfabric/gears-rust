...
/// outlives the set. This replaces the old per-worker `JoinHandle` fields and
/// `stop_*` methods: a `start_*` failing midway tears down whatever already
/// started (see [`Supervisor::shutdown`]) instead of leaking it.
struct Supervisor {
    cancel: CancellationToken,
