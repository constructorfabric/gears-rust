...
/// `(new_member_assigned, topology_version, sibling_updates)` returned by `join`.
type JoinOutcome = (Vec<Assignment>, i64, Vec<(Uuid, Vec<Assignment>)>);
...
impl ConsumerGroupCoordinator {
    ...
    pub fn join(
        ...
        for (&id, assigned) in &new_assignments {
            if let Some(m) = group.members.get_mut(&id) {
                m.assigned.clone_from(assigned);
            }
        }
    ...
    pub fn subscribe(
        ...
        let receiver = {
            let state = this.state.lock().unwrap_or_else(PoisonError::into_inner);
