...
pub enum DomainError {
    ...
    /// A publish named a `producer_id` (the `meta.producer_id` body field) that
    /// is not registered - never registered, or reaped after the
    /// producer-registration inactivity TTL. `api/rest/error.rs` lifts this to
    /// `400` with a `meta.producer_id` `context.field_violations[]` entry. `topic`
    /// is the stream the publish targeted, named in `context.resource_name` - the
    /// common resource every publish error shares. `producer_id` is a validated
    /// UUID by the time existence is checked, so the violation names it (it is a
    /// derived, already-validated fact, not raw submitted content).
    #[error("unknown producer {producer_id} on topic {topic}")]
    UnknownProducer {
        topic: String,
