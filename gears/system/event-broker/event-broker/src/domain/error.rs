...
pub enum DomainError {
    ...
    /// A publish named a `producer_id` (in its `meta` block) that is not
    /// registered - never registered, or reaped after the producer-registration
    /// inactivity TTL. `api/rest/error.rs` lifts this to `400` with a
    /// `Producer-Id` `context.field_violations[]` entry. `topic` is the stream
    /// the publish targeted, named in `context.resource_name` - the common
    /// resource every publish error shares (the submitted `producer_id` is not
    /// echoed).
    #[error("unknown producer on topic {topic}")]
    UnknownProducer { topic: String },
