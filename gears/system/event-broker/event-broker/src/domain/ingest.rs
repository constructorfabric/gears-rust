    ...
    R: crate::domain::idempotency::IdempotencyGuard + ProducerRegistry + Send + Sync + 'static,
{
    async fn publish_event(
        ...
        // A chained/monotonic publish names a registered producer. Verify it
        // exists before running the chain check so an unknown `producer_id` -
        // never registered, or aged out - is reported as the producer's own
        // `400 UnknownProducer` to fix, not as the `503` a downstream
        // foreign-key failure in `check_and_enqueue` would otherwise surface
        // (`DESIGN.md:1163`). A stateless publish carries a nil `producer_id`
        // and is not subject to this.
        if let Some(meta) = &request.meta {
            if !meta.producer_id.is_nil() && self.repo.owner(meta.producer_id).await?.is_none() {
                return Err(DomainError::UnknownProducer {
