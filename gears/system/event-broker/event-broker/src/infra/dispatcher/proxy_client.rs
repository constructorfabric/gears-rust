...
pub enum ProxyError {
    ...
    /// The proxied request body exceeded the caller-supplied limit
    /// (defaulting to [`crate::config::MAX_REQUEST_BODY_BYTES`]).
...
pub async fn proxy(
    ...
    // Bound the TCP connect: `PeerOptions` defaults leave every timeout `None`,
    // so without this an upstream that never completes the handshake hangs the
    // handler forever. `connection_timeout`/`total_connection_timeout` are the
    // two the connector honours on this HTTP/1 path.
    ...
    // The connector builds the session with `new_with_options`, which does not
    // copy `PeerOptions`' read/write timeouts, so they must be set on the
    // session directly. Both are per-operation, not total: reusing `idle_timeout`
    // bounds the header phase and each body read without capping a long-lived
    // SSE/long-poll stream (the outer body-read timeout below then double-covers).
    session.read_timeout = Some(idle_timeout);
    session.write_timeout = Some(idle_timeout);
