...
//! one.
//!
//! Small and shared on purpose: "does this range serve a reader standing
//! here" must be answered identically by the wake path, reclamation's
//! retained windows and the byte-pressure victim filter. Three predicates
//! that merely agree would break liveness silently the moment one changed -
//! a reader woken for a span reclaimed in the same breath parks with nothing
//! left to wake it.
...
impl AccountedSpan {
    ...
    pub fn serves(self, position: Sequence) -> bool {
        self.contains(Sequence::assigned(position.as_i64().saturating_add(1)))
    }
    ...
    pub fn is_adjacent_to(self, next: Self) -> bool {
        Sequence::assigned(self.through.as_i64().saturating_add(1)) == next.from
    }
}
