...
impl SegmentDemand {
    ...
    pub fn builder(key: PartitionKey) -> SegmentDemandBuilder {
        SegmentDemandBuilder {
            key,
            segment_from: Sequence::assigned(0),
            readers: 0,
...
impl Allocation {
    ...
    /// Total committed bytes for `demand`. Each demand is matched to its grant by
    /// key (`runway_for`), not by slice position, so the caller can pass `demand`
    /// in any order; a demand with no matching grant contributes nothing.
    #[must_use]
    pub fn committed_bytes(&self, demand: &[SegmentDemand]) -> usize {
        demand
            ...
            .map(|want| {
                want.bytes_for(
                    self.runway_for(want.key(), want.segment_from())
                        .unwrap_or(0),
                )
            })
