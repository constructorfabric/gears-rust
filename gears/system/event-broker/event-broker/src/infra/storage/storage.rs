...
use crate::domain::ingest::{
    ProducerCursors, ProducerMode, ProducerPartitionCursor, ProducerRegistration, ProducerRegistry,
    ProducerResetScope, ProducerTopicCursors,
};
                    ...
                    let outcome = match &chain {
                        ...
                        Some(c) => {
                            ...
                            let outcome = match existing {
                                ...
                                // Idempotent retry of a lost ack: the producer
                                // re-sends the exact step that established the
                                // current head, so `sequence` equals
                                // `last_sequence` and `previous` still links to
                                // the step before it. Not re-admitted; state
                                // unchanged. Any other `sequence <=
                                // last_sequence` means the producer's chain
                                // pointer is behind the broker's (a
                                // crash/restore from a stale checkpoint) and it
                                // must re-read the cursor - a desync, not a
                                // duplicate.
                                Some(row)
