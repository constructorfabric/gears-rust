...
impl IdempotencyGuard for Storage {
    async fn check_and_enqueue(
        ...
        let outbox = Arc::clone(self.outbox.get().ok_or_else(|| {
            DomainError::StorageUnavailable(
                "the ingest outbox pipeline is not running yet".to_owned(),
            )
        })?);
        let PublishEnqueue {
            chain,
                ...
                Box::pin(async move {
                    let outcome = match &chain {
                        ...
                        Some(c) => {
                            ...
                            match existing {
                                // First publish for this (producer, topic,
                                // partition) in either mode: seed
                                // `last_sequence` to the submitted value and
                                // admit. A monotonic first publish may seed any
                                // value; a chained one conventionally starts at
                                // `1`, but the seed is the submitted `sequence`
                                // regardless.
                                None => {
                                ...
                                Some(row) => {
                                    // Whether this publish advances the stored
                                    // head. Chained requires the exact next
                                    // step and a `previous` that links to the
                                    // current head; monotonic admits any
                                    // strictly greater `sequence` and ignores
                                    // `previous`, so a gap over skipped values
                                    // is legal and consumes them for good.
                                    let advances = match c.mode {
                                        ProducerMode::Chained => {
                                            c.sequence == row.last_sequence + 1
                                                && c.previous == row.last_sequence
                                        }
                                        ProducerMode::Monotonic => c.sequence > row.last_sequence,
                                    };
                                    ...
                                    if advances {
                                        ...
                                        match c.mode {
                                            // Chained: an idempotent retry of a
                                            // lost ack re-sends the exact step
                                            // that established the current head
                                            // (`sequence` equals `last_sequence`
                                            // and `previous` links to the step
                                            // before it). Not re-admitted; state
                                            // unchanged. Any other
                                            // `sequence <= last_sequence` means
                                            // the producer's chain pointer is
                                            // behind the broker's (a
                                            // crash/restore from a stale
                                            // checkpoint) and it must re-read the
                                            // cursor - a desync, not a duplicate.
                                            ProducerMode::Chained => {
                                                if c.sequence == row.last_sequence
                                                    && c.previous == row.last_sequence - 1
                                                {
                                                    ProducerIdempotencyOutcome::DuplicateIgnore
                                                } else {
                                                    ProducerIdempotencyOutcome::SequenceViolation {
                                                        last_sequence: row.last_sequence,
                                                    }
                                                }
                                            }
                                            // Monotonic: a `sequence` at or below
                                            // the head is a re-send, not a chain
                                            // break - deduplicated with the head
                                            // unmoved, never a sequence violation
                                            // (that outcome belongs to chained
                                            // mode).
...
mod tests {
    ...
    async fn publish_before_the_pipeline_starts_is_reported_as_unavailable() {
        ...
        assert!(
            matches!(&err, DomainError::StorageUnavailable(detail)
                if detail == "the ingest outbox pipeline is not running yet"),
