//! Permissive-by-default `AuthZResolverClient` double for
...
//! Tenant scope is enforced via the same `AuthZResolverClient`/
...
#[async_trait]
impl<F> AuthZResolverClient for DenyingAuthZ<F>
where
