...
pub fn register_ingest_routes(mut router: Router, openapi: &dyn OpenApiRegistry) -> Router {
    ...
    // NOTE: revisit route tags on the next axum update.
...
pub fn register_delivery_routes(mut router: Router, openapi: &dyn OpenApiRegistry) -> Router {
    ...
    router = OperationBuilder::post("/event-broker/v1/subscriptions/{id}")
        ...
        .json_response_with_schema::<delivery_dto::SeekResponse>(
            openapi,
            StatusCode::OK,
...
/// is separate and unaffected by this function's shape. Gated
/// `any(test, feature = "test-utils")` like `oagw`, since the SDK's own
/// integration tests reach the harness (which holds this router) through the
/// `test-utils` feature, not only this crate's unit tests.
