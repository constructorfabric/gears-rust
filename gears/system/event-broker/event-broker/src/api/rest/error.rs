...
impl From<DomainError> for CanonicalError {
    fn from(err: DomainError) -> Self {
        match err {
            ...
            // it is a `400` naming the `meta.producer_id` body field (scenario
            ...
            // resource). The id is a validated UUID by the time existence is
            // checked, so the description names it.
            DomainError::UnknownProducer { topic, producer_id } => {
                TopicResourceError::invalid_argument()
                    .with_field_violation(
                        "meta.producer_id",
                        format!(
                            "producer {producer_id} is not registered or registration has expired"
                        ),
                        "unknown_producer",
                    )
                    .with_resource(topic)
