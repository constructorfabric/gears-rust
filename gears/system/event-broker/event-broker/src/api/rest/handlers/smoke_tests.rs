...
#[tokio::test]
#[ignore = "uses a named-shaped group id; named consumer groups not implemented yet (returns 501, not 404); re-enable when named-group support lands"]
...
#[tokio::test]
async fn events_publish_requires_a_registered_event_type() {
    let harness = EventBrokerHarness::builder().build().await;
    let resp = harness
        .api_v1()
        .post_events()
