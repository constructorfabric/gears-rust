...
/// Parse the stream query ourselves rather than through axum's `Query`
/// extractor: `StreamQuery` is `#[serde(deny_unknown_fields)]`, so a legacy
/// `timeout`/`collect` (or any unrecognised) parameter must be rejected, and
/// this maps that to a canonical `400 InvalidQuery` `Problem` rather than the
/// plain-text body axum's `QueryRejection` would emit (`scenarios/consumer/
/// stream/1.10`). The parameter name, not any value, appears in the message.
fn parse_stream_query(raw: Option<String>) -> Result<StreamQuery, CanonicalError> {
    serde_urlencoded::from_str::<StreamQuery>(raw.as_deref().unwrap_or("")).map_err(|err| {
        EventBrokerResourceError::invalid_argument()
            .with_format(format!("InvalidQuery: {err}"))
            .create()
    })
}
...
pub async fn stream_events(
    ...
    let query = parse_stream_query(raw_query)?;
