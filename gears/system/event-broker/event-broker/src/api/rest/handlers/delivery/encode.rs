...
pub fn to_multipart_part(boundary: &str, frame: Frame) -> Bytes {
    ...
    let json = serde_json::to_vec(&dto).unwrap_or_else(|err| {
        // Near-unreachable: `FrameDto` is plain structs plus a JSON payload. Log
        // it rather than let the empty part be silent - the framing survives, but
        // the operator should see that a frame was dropped.
        tracing::error!(%err, frame_kind = kind, "multipart frame serialization failed; emitting an empty part");
        Vec::new()
    });
    Bytes::from(
        [
...
pub fn to_sse_event(frame: Frame) -> SseEvent {
    ...
    SseEvent::default()
        ...
        .json_data(dto)
        .unwrap_or_else(|err| {
            // Near-unreachable, same as the multipart path. Log the drop rather
            // than let the substituted heartbeat hide it.
            tracing::error!(%err, frame_kind = kind, "SSE frame serialization failed; substituting an empty heartbeat");
