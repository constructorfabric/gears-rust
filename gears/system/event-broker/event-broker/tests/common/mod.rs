...
fn config_yaml(home_dir: &Path, port: u16, entities: &[Value]) -> String {
    let entities_json: Vec<String> = entities.iter().map(std::string::ToString::to_string).collect();
    format!(
        r#"
server:
  home_dir: "{home}"
...
fn spawn_server(home_dir: &Path, port: u16, entities: &[Value]) -> (Child, Arc<Mutex<Vec<String>>>) {
...
async fn wait_for_healthy(child: &mut Child, base_url: &str, logs: &Arc<Mutex<Vec<String>>>) {
    ...
    loop {
        if let Ok(Some(status)) = child.try_wait() {
            ...
            panic!("server exited early ({status}) before becoming healthy; captured output:\n{captured}");
        }
...
pub struct SseFrameReader {
    stream: std::pin::Pin<Box<dyn tokio_stream::Stream<Item = reqwest::Result<bytes::Bytes>> + Send>>,
    buf: String,
}
