...
///
/// The plain `topology_frame_json` reports `offset: 0`, which is correct only
/// for a session that has not delivered anything yet. The session builds these
/// positions from the read set it actually holds, so a stream that has
/// consumed `at` events on a partition reports `at` - not the `Assignment`'s
/// own `offset`/`last_examined`, which the coordinator sets to 0 and never
/// updates.
    ...
    let resp = client
        ...
        .json(&json!({
            ...
            "positions": {
                topic: partitions
                    .iter()
                    .map(|p| json!({ "partition": p, "value": "earliest" }))
                    .collect::<Vec<_>>(),
            },
        }))
        .send()
...
async fn two_subscribers_baseline_then_third_joins_mid_stream() {
    ...
    seek_all(&client, &server, a_id, topic, &a_assigned_v1, a_v1).await;
    ...
    seek_all(&client, &server, b_id, topic, &b_steady, b_v).await;
    ...
    seek_all(&client, &server, c_id, topic, &three_way[&c_id], c_v).await;
...
async fn two_way_steady_state(f: &TestFixture) -> (Member, Member) {
    ...
    seek_all(&f.client, &f.server, b_id, &f.topic, &two_way[&b_id], b_v).await;
