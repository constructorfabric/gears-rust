...
async fn publish_then_consume_happy_path() {
    ...
    let event_type = "gts.cf.core.events.event_type.v1~x.eb.t10.foo.v1";
    let subject_type = "gts.x.eb.t10.subject.v1~";
    let server = TestServer::start(vec![
        json!({ "id": topic, "partitions": 1, "created_at": "2026-01-01T00:00:00Z" }),
        json!({
            "id": event_type,
            "topic_id": topic,
    ...
    let group_id = group_resp.json::<serde_json::Value>().await.expect("group json")["id"]
        .as_str()
        .expect("group id")
        .to_owned();
    ...
    let sub_id = sub_resp.json::<serde_json::Value>().await.expect("sub json")["id"]
        .as_str()
        .expect("sub id")
        .to_owned();
...
async fn restart_preserves_events_and_consumer_groups_but_not_subscriptions() {
    ...
    let event_type = "gts.cf.core.events.event_type.v1~x.eb.t10r.foo.v1";
    ...
    let group_resp = client
        ...
        .expect("create consumer group");
    assert_eq!(group_resp.status(), 201);
    let group_body_before_restart = group_resp.json::<serde_json::Value>().await.expect("group json");
    let group_id = group_body_before_restart["id"].as_str().expect("group id").to_owned();
    ...
    let sub1_id = sub1_resp.json::<serde_json::Value>().await.expect("sub1 json")["id"]
        .as_str()
        .expect("sub1 id")
        .to_owned();
    ...
    let sub2_id = sub2_resp.json::<serde_json::Value>().await.expect("sub2 json")["id"]
        .as_str()
        .expect("sub2 id")
        .to_owned();
...
async fn producer_chain_sequence_resubmission_is_deduped_not_double_persisted() {
    ...
    let event_type = "gts.cf.core.events.event_type.v1~x.eb.t10p.foo.v1";
    ...
    let producer_id = producer_resp.json::<serde_json::Value>().await.expect("producer json")["id"]
        .as_str()
        .expect("producer id")
        .to_owned();
...
async fn consumer_group_listing_paginates_against_the_real_sqlite_backend() {
    ...
    assert_eq!(items1.len(), 2, "page 1 must carry exactly `limit` items: {body1}");
    ...
    assert_eq!(items2.len(), 1, "page 2 must carry the one remaining item: {body2}");
    ...
    for item in items1.iter().chain(items2.iter()) {
        ...
        assert!(seen.insert(id.to_owned()), "'{id}' appeared on more than one page");
    }
