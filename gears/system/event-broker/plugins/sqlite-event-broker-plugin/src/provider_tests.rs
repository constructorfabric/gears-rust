...
use event_broker_sdk::{EventBrokerBackendProvider, GtsTypeId, StorageBackendError};
...
fn event() -> Event {
    Event {
        id: Uuid::now_v7(),
        type_id: GtsTypeId::new("gts.cf.core.events.event.v1~x.eb.provider.foo.v1"),
        tenant_id: Uuid::now_v7(),
        source: "provider-tests".to_owned(),
        subject: "s".to_owned(),
        subject_type: GtsTypeId::new("gts.x.eb.provider.subject.v1~"),
...
async fn a_configured_path_is_the_file_the_events_land_in() {
    ...
    let stored = reopened
        .read(&SecurityContext::anonymous(), TOPIC, 0, Sequence::NONE, 10)
        .await
        .expect("read the reopened event log back");
    assert_eq!(
        stored
            .iter()
            .map(|e| e.sequence.map(Sequence::as_i64))
            .collect::<Vec<_>>(),
        vec![Some(1)],
