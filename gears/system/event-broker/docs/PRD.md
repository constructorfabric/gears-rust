...
| Event | An immutable record in a `(topic, partition)` log, GTS-typed via its `event_type`. Carries `subject` + `subject_type` to identify the entity it concerns; its partition is derived from `tenant` by default. See [DESIGN.md §3.1 Event Schema](DESIGN.md) and [ADR-0003 Event Schema](ADR/0003-event-schema.md). |
