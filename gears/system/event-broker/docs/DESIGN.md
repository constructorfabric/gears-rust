...
- `cpt-cf-evbk-adr-partition-selection` — Partition selection algorithm: MurmurHash3-32 over `partition_key` with producer override and broker-side range validation. See [`ADR/0002-partition-selection.md`](ADR/0002-partition-selection.md).
...
Cursors live in the **ClusterCapabilities-backed cache** (NOT a SQL table). Keyed by `(consumer_group, topic, partition)`. Stores `offset` (session cursor set by SEEK; broker emits from offset+1) and `last_examined` (highest offset the broker has scanned for this group on that `(topic, partition)`, used by the offset-adviser feature — see R57). Persistence properties match the cache provider: in standalone (in-process cache) cursors die with the process; in cluster with a persistent provider (Redis-with-disk, etcd) cursors survive shard failover. Consumers that need stronger durability use the future `/v1/groups/{group}/offsets` KV endpoint (§4.8) or track offsets in their own DB and seek on JOIN via `POST /v1/subscriptions/{id}:seek`.
...
- Derive `partition` from the publish body: hash `partition_key` if present, else `tenant_id`. Reject any top-level `partition` field on publish input with `400 BadRequest`. Reject `meta.partition_hint` mismatches with `400 PartitionHashMismatch`. See [`ADR/0002-partition-selection.md`](ADR/0002-partition-selection.md)
...
POST   /v1/events                                # PUBLISH (with dedup)
       ...
       Returns 412: SequenceViolation            # chain broken; see RECOVERY below
...
- `"at:<ISO-8601>"` → cursor set to the offset of the first event whose `occurred_at ≥ timestamp`. Timestamp before retention floor → retention floor offset; timestamp beyond HWM → HWM (consumer will wait for future events). Malformed timestamp → `400 InvalidTimestamp`.
- Response body returns the resolved integer offsets per partition for audit.

**`:stream` defensive backstop**: `GET /v1/events:stream` rejects with `409 PositionsNotSet { unseeded: [...], recovery_hint }` when any assigned partition has no committed cursor in the subscription's group. A well-behaved SDK seeds positions before opening the stream via `POST /v1/subscriptions/{id}:seek` and never observes this on the happy path; the SDK recovers from it by re-resolving via `position()` and re-SEEKing (sharing the `SubscriptionRecoveryExhausted` budget).

**Topology-frame re-SEEK**: when a `WireFrame::Topology { topology_version, assigned }` arrives mid-stream, the dispatcher diffs `prev_assignments` vs `new_assignments`. For partitions in `new \ prev`, the same `position() → POST /v1/subscriptions/{id}:seek` flow runs before the broker may deliver events for them. Continuing partitions keep their cursor; lost partitions follow the existing drop-in-flight behavior.

**Forward-only enforcement during streaming**: while a subscription's `:stream` is open, SEEK requests via `POST /v1/subscriptions/{id}:seek` are forward-only (`MAX(stored, requested)` per partition; backward moves rejected with `409 SeekBackwardNotAllowed`). Pre-stream SEEK (before the first `:stream` after JOIN, or after a partition's cursor was admin-cleared) accepts any value in the valid range — this is how `Earliest`/`Latest` reseeds and `Custom` replay overrides work.
...
POST   /v1/subscriptions/{id}:seek               # SEEK (pre-stream, required before first stream open)
       ...
       # If omitted, broker returns 409 PositionsNotSet on the first stream open.
       # Mid-stream re-SEEK (after topology change for newly assigned partitions) uses the same
       # endpoint but enforces forward-only MAX rule while the stream is open.
...
GET    /v1/events:stream?subscription_id={uuid}  # STREAM — multipart/mixed streaming delivery
       Returns 200:  long-lived multipart/mixed stream; each part carries one frame:
                       ...
                       kind: advisory   → broker hint (rate-limit, deprecation, etc.)
                       kind: topology   → assignment update (topology_version + assigned[])
       Returns 410:  SubscriptionTerminated — re-JOIN required
       Returns 404:  SubscriptionNotFound — subscription_id unknown or expired
       Returns 409:  PositionsNotSet — SEEK was not called for one or more assigned partitions
...
**Explicit SEEK only**: All cursor advancement is the consumer's explicit responsibility. There is no auto-commit option in the Event Broker protocol — cursor advancement happens solely via `POST /v1/subscriptions/{id}:seek` (client-driven, forward-only MAX rule during streaming). The at-least-once delivery guarantee holds as long as consumers SEEK only after successfully processing events.

#### Error Response Format

**ID**: `cpt-cf-evbk-interface-error-codes`

All errors follow RFC 9457 Problem Details (`application/problem+json`) using the canonical `toolkit-canonical-errors` categories. The `type` field is always a canonical GTS category URI of the form `gts://gts.cf.core.errors.err.v1~cf.core.err.<category>.v1~`. Domain identity (which specific resource was not found, which field failed, etc.) is expressed via `context.resource_type` / `context.resource_name` or structured violation arrays — not via a domain-specific `type` URI.
...
| ValidationError | 422 | `InvalidArgument` | `invalid_argument` | No | `field_violations` — one per validator diagnostic |
...
| SequenceConflict | 409 | `FailedPrecondition` | `failed_precondition` | Yes | `violations: [{type: "sequence_conflict", subject: "(producer)", description: <detail>}]` |
| ConsumerGroupAlreadyExists | 409 | `FailedPrecondition` | `failed_precondition` | No | `violations: [{type: "already_exists", subject: <group_id>, description: <detail>}]` |
| ConsumerGroupHasActiveMembers | 409 | `FailedPrecondition` | `failed_precondition` | No | `violations: [{type: "has_active_members", subject: <group_id>, description: <detail>}]` |
| PositionsNotSet | 409 | `FailedPrecondition` | `failed_precondition` | No | `violations`: one `{type: "cursor_missing", subject: "<topic>:<partition>", description: "no committed cursor"}` per unseeded partition |
| SequenceViolation | 412 | `FailedPrecondition` | `failed_precondition` | No | `violations: [{type: "sequence_mismatch", subject: "(producer)", description: "expected_previous=<n>"}]` |
...
- `instance`: URI reference identifying the specific occurrence (middleware-injected)
...
- Cursor positions satisfy: `0 ≤ offset ≤ backend.MAX(sequence) for that (topic, partition)`. `sent` is per-subscription advisory state in cache (not on the cursor); `received`/`max_sequence` are derivable via `backend.segments()` or `backend.query()`.
