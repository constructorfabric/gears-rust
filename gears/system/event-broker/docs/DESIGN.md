...
was a dead, never-called `todo!()` stub and has been deleted - those tickets never actually tracked
this work). `TopicRepo`/`EventRepo` (event/cursor storage, a separate concern from topic/event-type
*metadata*) remain `InMemoryDomainRepo`-backed, pending `#4347`/`#4348`'s real storage-backend work.
...
Errors (every publish-path error names the targeted stream in `context.resource_name`; see §3.3 "Error Response Format"):
- `400` - Read-only/unknown field on the body, an unknown producer, or a chained/monotonic mode invariant violated
- `403` - Caller's principal doesn't match the registered owner of the producer
- `404` - Topic or event type not registered
...
| UnknownProducer | 400 | `InvalidArgument` | `invalid_argument` | No | `field_violations: [{field: "meta.producer_id", reason: "unknown_producer", description: "producer <id> is not registered or registration has expired"}]`, `resource_type: "gts.cf.core.events.topic.v1~"`, `resource_name: <topic>`. The description names the validated producer id |
