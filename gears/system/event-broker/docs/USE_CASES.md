...
| **C2** | Single, broker-backed, solo | Joins group, polls, broker tracks all cursor state | On crash: `session_timeout` expires; new instance joins → gets all partitions → reads from `cursor.offset` → re-receives un-sought events |
