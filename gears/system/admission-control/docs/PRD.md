...
Every refusal **MUST** carry a machine-readable cause drawn from the platform's error type system, distinguishing at minimum: refused by a built-in policy, refused by policy, refused because the operation awaits approval per `cpt-cf-admission-control-fr-deferral-relay`, and refused because a check could not run. A refusal by a built-in policy **MUST** identify that policy; a refusal by policy **MUST** carry the reason the engine supplied.
