...
| `toolkit-canonical-errors` | Library | The gear's error family and the four refusal causes of `cpt-cf-admission-control-fr-refusal-cause`: refused by a built-in policy, refused by policy, awaiting approval, and could-not-run |
...
| Verdicts by cause | Counter | Verdict builder | Distinguishes admissions and every refusal cause of `cpt-cf-admission-control-fr-refusal-cause` in aggregate: built-in-policy, policy, awaiting-approval, and could-not-run. Awaiting-approval is its own label so a deferral is never counted as an outage |
