...
impl DirectoryServiceImpl {
    ...
    async fn authenticate_peer(
        ...
        match authenticator.authenticate(token.expose_secret()).await {
            ...
            Err(_) => Err(Status::internal("internal authentication failure")),
        }
    }
}
...
mod tests {
    ...
    /// its REST endpoint + `OpenAPI` spec — the edge-gateway discovery path.
    #[tokio::test]
    async fn list_all_instances_returns_all_gears_with_specs() {
        let svc = service();
        ...
        for inst in &all {
            ...
            let spec = inst.openapi_spec.as_deref().unwrap_or_default();
            assert!(spec.contains(&inst.gear_name), "spec should carry gear tag");
        }
