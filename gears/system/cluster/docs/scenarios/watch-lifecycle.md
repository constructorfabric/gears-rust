...
> both watches; per-primitive instances live in [cache.md](./cache.md) (SC-CACHE-016/017).
> The recovery action differs per
...
- *Steps:* type-level / structural check of `CacheWatchEvent` and `LeaderWatchEvent`.
- *Expected:* each carries exactly `Event`/`Status`, `Lagged { dropped }`, `Reset`, `Closed(ClusterError)`; all `#[non_exhaustive]`; none expose a fallible `changed()`.
- *Done-when:* asserts both enums share the variant set (a compile-time/structural test, no backend needed).
...
- *Done-when:* asserts `Reset` on each watch and that the per-primitive recovery restores correct state. *(Generalizes SC-CACHE-017.)*
