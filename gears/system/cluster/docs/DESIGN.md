...
| **I11** | **The plugin-facing `*Backend` traits stay stable.** Extensions are defaulted and dyn-safe (`probe()`, `migrate()`), never required methods — **except when a whole operating profile depends on the extension.** The store-owned-lease methods (`acquire`/`acquire_waiting`/`renew`/`release` on `DistributedLockBackend`; `join`/`renew`/`resign` on `LeaderElectionBackend`) are **required**: Profile 3 serves every lock/election RPC through them, so a missing implementation is a whole-profile failure and is made a build error rather than a silent per-call `Unsupported`. A deliberate, SemVer-breaking reversal for these methods only, recorded in ADR-012's 2026-09-18 amendment | §3.17.3, §3.17.8, ADR-012, `cpt-cf-clst-nfr-plugin-stability` |
