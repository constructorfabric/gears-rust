...
- `CacheBasedServiceDiscoveryBackend` — `put({name}/{instance_id}, metadata, ttl)` for registration, `watch_prefix({name}/)` for topology change events, background TTL renewal for heartbeat. Metadata filtering is client-side; `features()` returns `ServiceDiscoveryFeatures { metadata_pushdown: false }`.
