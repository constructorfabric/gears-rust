...
**Scope note.** This decomposition was authored when the lifecycle wiring and a production backend were follow-ups. They have since been **brought into this change**: the lifecycle owner collapsed into the `cf-gears-cluster` gear crate, and one production-shaped in-process backend (the standalone plugin) ships here. The elements below are updated to reflect that; only external per-backend plugins and per-primitive native routing remain genuine follow-ups.

- **Component `cpt-cf-clst-component-wiring`** — the lifecycle wiring (`ClusterWiring::builder(...).build_and_start() -> ClusterHandle`) is **implemented in this change**, in the `cf-gears-cluster` gear crate rather than a separate wiring crate (host collapsed in).
- **Component `cpt-cf-clst-component-plugins`** — the in-process **standalone** plugin is implemented in this change; the remaining per-backend plugin crates (Postgres, K8s, Redis, NATS, etcd) are still separate follow-up changes.
- **FRs realized by the wiring / operator config / lifecycle owner**: `cpt-cf-clst-fr-routing-omit-default` (wiring auto-wrap), `cpt-cf-clst-fr-lifecycle-owner` (gear crate owns `ClusterHandle`), `cpt-cf-clst-fr-shutdown-revoke` (leader revocation on `stop`) and `cpt-cf-clst-fr-shutdown-ttl-cleanup` (no remote cleanup on `stop`) are **implemented in this change**. `cpt-cf-clst-fr-routing-per-primitive` remains a follow-up: binding a native non-cache backend per primitive is rejected at config time until those providers ship.
- **Sequences `cpt-cf-clst-seq-lifecycle-startup` and `cpt-cf-clst-seq-shutdown`** — now realized by the gear crate's `ClusterHandle` start/stop (leader revocation included); `cpt-cf-clst-seq-per-primitive-resolution` is realized by the SDK resolvers.

The remaining out-of-scope elements (external per-backend plugins, per-primitive native routing) will be decomposed in the follow-up changes that build against the contract frozen here.
...
- **Scope**:
  ...
  - `LockGuard` (extend/release; no-op Drop), lock error variants (`LockContended`/`LockTimeout`/`LockExpired`), `LockCapability`, `LockFeatures`.
...
- **API**:
  - Rust: `DistributedLockV1::resolver(hub)...resolve()`; `try_lock/lock`; `LockGuard::{extend,release}`.
