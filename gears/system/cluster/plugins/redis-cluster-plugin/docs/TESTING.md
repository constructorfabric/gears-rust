...
> **272 tests** — 202 unit, four conformance suites, and the 63 Layer 3 scenarios
...
|---|---|
| `config.rs` | serde round-trip for every field and every default (`pool_size` 4, `command_timeout_ms` 5000, `key_prefix` "cluster", `wait_timeout_ms` 1000, `watch_mode` `publish`, `manage_keyspace_notifications` false); `topology`/`durability`/`watch_mode` enum variants round-trip and an unknown variant is rejected; `deny_unknown_fields` turns an operator typo into `InvalidConfig`; `validate()` rejects each of the four zeros that would remove a bound rather than tighten it (DESIGN §8); `Debug` masks `url` on both types; `RedisLockConfig`'s accepted field set is exactly `RedisClusterConfig`'s minus the three cache-only fields, read back out of serde's own error rather than from a hand-written list (the drift guard the duplication needs, DESIGN §8) |
...
| `wait.rs`, `shutdown.rs` | The `WAIT` short-count arm as an error rather than a silent success; and the `Drop`-ordering rule — that the shutdown token is cancelled *before* the diagnosis is returned, which is why the diagnosis is returned rather than emitted in place (DESIGN §11) |
...
| `cache/mod.rs` | Key construction: `<prefix>:c:<key>` and the channel `<prefix>:e:c:<key>` for the same input; `Ttl::Of`→ ms, `Ttl::Indefinite` → the `-1` sentinel the scripts expect; `CacheEntry` decoding from an `HMGET` reply shape, including both-`nil` → `None` and a version string at `i64::MAX` decoding losslessly (the precision claim DESIGN §2.2 rests on) |
...
`leader_conformance` and `discovery_conformance` are wired the same way, over
`CasBasedLeaderElectionBackend` / `CacheBasedServiceDiscoveryBackend` on this
...
**`discovery_conformance` needs no polling-interval override**, unlike the Postgres
wiring's. This cache declares `prefix_watch: true`, so the SD default backend takes
its native-watch branch and there is no polyfill interval to tune. That is the only
place in the platform where that branch is taken, which makes this suite the sole
coverage of the remote-native-prefix-watch behaviour DESIGN §7 describes.

**Two conformance scenarios do not run here at all.** `TimeControl::Real` is
...
healthy server. The cost is that the shared runners skip `SC-LEAD-006` and
`SC-DISC-006`, both of which force a *missed* renewal that a healthy real backend
never exhibits by waiting. They map to fault injection, which §8 defers; §8.2
records them, because a green suite says nothing about a scenario it skipped.
...
- `watch_mode: disabled` or cluster mode: `prefix_watch == false` → `watch_prefix`
  returns `Unsupported` and the SD suite exercises `PollingPrefixWatch`.
- `LockFeatures::linearizable` tracks the cache declaration per fixture.
- `ServiceDiscoveryFeatures::metadata_pushdown == false` — confirmed by the
  discovery suite.
...
| `RD-WATCH-010` | `watch_mode: disabled` degrades honestly | `watch`/`watch_prefix` return `Unsupported`; `features().prefix_watch == false`; no `PUBLISH` is issued on the write path (asserted via `INFO commandstats`); the SD default backend still works over `PollingPrefixWatch`/`scan_prefix` |
...
|---|---|---|
| L1 unit tests | Every PR — `cargo test -p cf-redis-cluster-plugin --lib`, no Docker | 205 tests, sub-second |
| L2 conformance + L3 integration, over eight single-node shapes plus two multi-node topologies | Every PR touching Rust — `make test-cluster-redis`, in `ci.yml`'s `integration` job | 275 tests total in ~10 s of test time, ~35 s including the build |
...
The 275 are 205 unit tests, 66 Layer 3 test functions, and the four conformance
...
every topology, single-node and multi-node alike — and L2 with two scenarios
short, `SC-LEAD-006` and `SC-DISC-006`, which the shared runners skip under
`TimeControl::Real` and which therefore have no Redis coverage at all. "Three
layers delivered" is a statement about the layers, not a claim that every
scenario in them runs; the row below is where the two that do not are recorded.
...
| Gap | Severity | Tracking |
|---|---|---|
| The conformance runners skip `SC-LEAD-006` and `SC-DISC-006` under `TimeControl::Real` | Warning — both force a *missed* renewal to assert re-enrolment, which a healthy real backend never exhibits by waiting, so the shared runner skips them on a real clock. They map to fault injection, which §8.1 defers. Two conformance scenarios therefore have no Redis coverage at all, which the suite's green result does not say | Accepted — would need L4 |
