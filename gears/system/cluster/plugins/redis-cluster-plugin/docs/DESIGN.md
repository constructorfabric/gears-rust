...
- [7. Leader Election and Service Discovery](#7-leader-election-and-service-discovery)
...
release. Leader election and service discovery are derived from the SDK default
backends over the Redis cache — no additional keys or connections are required for
...
cache and lock and K8s Lease serves leader election and service discovery. ADR-001
...
- Implements `ClusterLockProvider` so the wiring crate can *independently*
  ...
  `cluster/src/wiring.rs`) dispatches `lock` against its own registry and needs
...
| `ServiceDiscoveryBackend` | SDK default `CacheBasedServiceDiscoveryBackend` over the Redis cache | — | `metadata_pushdown: false` |

`prefix_watch: true` (where it holds) means the SDK-default service-discovery backend
gets real reactive topology watches rather than the polling polyfill, and a consumer
may declare `CacheCapability::PrefixWatch` against this backend. That it is
*conditional* on topology and watch mode is the part that must not be glossed — see
§4.3.
...
(`cluster/src/wiring.rs`) rather than inventing a variation, so a reader who knows one
knows both.

`build_and_start`:

0. Validates the config values that can only fail at startup — a zero `pool_size`,
   `command_timeout_ms`, `wait_timeout_ms`, or `sd_poll_interval_ms` (§8) — before
...
holds it; an evicted leader-election key elects a second leader; an evicted
service registration drops an instance from discovery. No TTL has lapsed and no
consumer is told.
...
| `disabled` | No publish, no watcher registry, no cache subscriptions. `watch`/`watch_prefix` return `Unsupported`; the SD default backend falls back to `PollingPrefixWatch` over `scan_prefix`, its documented behaviour on any prefix-watch-incapable cache | `false` |
...
- **Cluster mode scans every primary** and concatenates, since `SCAN` is per-node.
  ...
  order is unspecified anyway (`cpt-cf-clst-fr-sd-discover` says as much), and the
...
## 7. Leader Election and Service Discovery

Both primitives use the SDK defaults over the Redis cache, and §1.2's table records a
...
**Service discovery** — `CacheBasedServiceDiscoveryBackend::new(cache)`. This works
in every configuration. In `watch_mode: publish` on a non-clustered deployment the
backend gets a real native prefix watch for topology changes; otherwise it
auto-selects `PollingPrefixWatch` over `scan_prefix`, with the operator-set
`sd_poll_interval_ms` threaded in by the wiring through
`cluster_sdk::provider::SD_POLL_INTERVAL_MS_OPTION` — the SDK's single-sourced
convention for that option key, so naming the config field to match is the whole
opt-in.
`ServiceDiscoveryFeatures::metadata_pushdown == false`.

This plugin is the **first remote backend in the platform to declare
`prefix_watch: true`**, which matters to the SDK backend rather than to this one.
`CacheBasedServiceDiscoveryBackend::discover` reconciles from backend truth on
every call precisely because a native prefix watch does not imply *in-process*
delivery: here delivery is a network round trip, so a `set_state` that has already
returned has published its event and not yet had it applied, and an event-maintained
view is not a sound source for a point query. The cluster gear's own
`docs/DESIGN.md` §3.11 carries the reasoning. Nothing in this plugin turns on it,
beyond the `scan_prefix` cost on the `discover` path being paid on every deployment
rather than only on the polling ones.
...
pub struct RedisClusterConfig {
    ...
    /// swallowed (§3.6), so the shorter bound loses no information.
    ...
    /// native prefix watch is unavailable. Default: 5s. Single-sourced with
    /// `cluster_sdk::provider::SD_POLL_INTERVAL_MS_OPTION` (§7).
...
`watch_mode`, `manage_keyspace_notifications`, and `sd_poll_interval_ms`, which have
...
the cluster type minus the three cache-only fields. Deriving the guard from serde
...
`command_timeout_ms`, `wait_timeout_ms`, or `sd_poll_interval_ms`. Each of those
...
which silently deletes the property §11 and §12 rest on; `wait_timeout_ms: 0` makes
`WAIT` block with no deadline; and `sd_poll_interval_ms: 0` panics
`tokio::time::interval` the moment a polling watch starts. `pool_size: 0` is refused
...
and service-discovery defaults riding on this cache (§7). At ~0.15 ms per read
...
| **Reserved `provider: default`** | **Chosen.** Per-binding as required, uniform across `leader_election`/`lock`/`service_discovery`, self-documenting in YAML, and typo-safe because `provider` stays required |
...
Three rules on the sentinel itself. It is **rejected on `cache`**, which is the
omit-default anchor and has no default to resolve to, with a naming error rather
than a confusing "unknown cache provider `default`". `allow_weak_consistency` is
**meaningful only on `leader_election` and `lock`** — on `service_discovery` it is
rejected, because that default has no consistency guard and accepting the flag there
would imply a protection that does not exist. And it is **intercepted before the
registry lookup**, so a plugin that registered a provider literally named `default`
cannot shadow it.
...
§"Honest backend declaration" forbids, and the SD default backend degrades cleanly to
`PollingPrefixWatch` in the meantime (§7).
...
reaching a clustered deployment, whose service discovery would otherwise start
