...
//! and service discovery are derived from the SDK default backends over the
//! Redis cache — no additional keys or connections are required for those two
//! primitives (DESIGN.md §7).
//!
//! This is the recommended cache and lock backend for the **K8s +
//! high-throughput cache** deployment shape (`../../../docs/DESIGN.md` §4.2),
//! where Redis serves cache and lock and K8s Lease serves leader election and
//! service discovery. ADR-001 puts Redis 10–100× above every other backend on
...
pub use config::{
    ...
    default_key_prefix, default_pool_size, default_sd_poll_interval, default_wait_timeout,
};
