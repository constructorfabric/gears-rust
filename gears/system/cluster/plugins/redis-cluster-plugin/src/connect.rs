...
/// connect closes the pool on the way out, so a half-open startup leaves
/// nothing behind.
pub async fn connect(spec: ConnectSpec<'_>) -> Result<Connected, ClusterError> {
    ...
    let client = match subscriber_builder.build_subscriber_client() {
        Ok(client) => client,
        Err(err) => {
            close_pool(&pool).await;
            return Err(map_redis_error(err));
    ...
    let subscriber = match tokio::time::timeout(CONNECT_TIMEOUT, client.init()).await {
        ...
        Ok(Ok(connection)) => (client, connection),
        Ok(Err(err)) => {
            close_pool(&pool).await;
            return Err(map_redis_error(err));
        }
