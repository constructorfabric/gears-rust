...
/// come from a lucky jitter draw, and far enough above a publish round trip that a
/// loaded CI container does not make it flaky.
///
/// A wake at roughly a heartbeat would mean the notification was *missed* — a
/// correctness bug in the subscribe path (this is the scenario
/// the `PING` barrier of DESIGN.md §3.2 step 4 exists for) — rather than merely slow.
fn assert_woken_by_publish(released_at: Instant, acquired_at: Instant) {
    let latency = acquired_at.saturating_duration_since(released_at);
    assert!(
        latency < Duration::from_millis(60),
        "the wake must be publish-driven: a wake at or near the 250 ms heartbeat means the release \
         notification was missed rather than merely slow. Release to acquire took {latency:?}"
    );
}
...
async fn rd_lock_003_a_blocked_lock_wakes_on_the_release_publish() {
    ...
    let waiter = spawn_waiter(&lock, "res");

    // Let the waiter reach its first failed `SET NX` and register its interest.
    // The registration happens *before* each attempt (DESIGN.md §5.3), so a
    ...
    assert_woken_by_publish(released_at, acquired_at);
...
async fn rd_lock_009_the_standalone_lock_needs_no_keyspace_notifications() {
    ...
    let (acquired, acquired_at) = waiter.await.expect("the waiter task does not panic");
    let guard = acquired.expect("the blocked acquire succeeds");
...
async fn rd_lock_011_two_instances_cannot_hold_the_same_lock() {
    ...
    let (acquired, acquired_at) = waiter.await.expect("B's waiter task does not panic");
    ...
    // replica is fast when the holder is in another.
    assert_woken_by_publish(released_at, acquired_at);
