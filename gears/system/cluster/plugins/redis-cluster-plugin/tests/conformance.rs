...
//! | discovery | `start_redis` | The SDK default over this plugin's cache, which is how the wiring resolves it (DESIGN.md §7) |
//!
...
/// The `SC-DISC-*` scenarios against `CacheBasedServiceDiscoveryBackend` over this
/// plugin's cache — again the SDK default, since `metadata_pushdown` is `false`
/// and there is nothing Redis-native to push down to (DESIGN.md §7).
///
/// This runs on the **stock** fixture rather than the durable one, and the
/// difference from `leader_conformance` is the point: the SD default has no
/// consistency guard, so it constructs happily over an `EventuallyConsistent`
/// cache. Nothing in `SC-DISC-*` asserts a linearizability-dependent property —
/// registrations are TTL'd keys and discovery is a prefix read — so running it on
/// the fixture an operator would actually deploy is the more useful coverage.
///
/// No `with_prefix_watch_polling` override, unlike the Postgres plugin's wiring,
/// and that too is a real difference rather than an omission: this cache declares
/// `prefix_watch: true` on this fixture (native `PSUBSCRIBE`, DESIGN.md §4.3), so
/// the SD backend uses the native watch and never installs the
/// `PollingPrefixWatch` polyfill there is an interval to tune. **This is the first
/// backend in the platform for which that branch is taken at all.** The polling
/// path is covered instead by `RD-WATCH-010`, under `watch_mode: disabled`.
#[tokio::test]
async fn discovery_conformance() {
    ...
    Box::pin(run_discovery_conformance(
        || {
            let url = url.clone();
            let index = scenario_index.fetch_add(1, Ordering::Relaxed);
            async move {
                ...
                let handle = RedisClusterPlugin::builder(config)
                    ...
                    .await
                    .expect("a fresh per-scenario instance starts against the test container");
                let discovery = Arc::new(CacheBasedServiceDiscoveryBackend::new(handle.cache()))
