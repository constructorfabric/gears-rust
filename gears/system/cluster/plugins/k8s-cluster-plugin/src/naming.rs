...
/// Whether `s` is a legal RFC 1123 *subdomain*: at most [`MAX_OBJECT_NAME_LEN`]
/// characters of dot-separated RFC 1123 labels. This is exactly the rule the API
/// server applies to a Lease `metadata.name` (`IsDNS1123Subdomain`), so an
/// operator-supplied literal object name (e.g. an `election_lease_names` override,
/// §14) must satisfy it.
///
/// Note: subdomain validation deliberately does **not** impose the per-label 63-char
/// cap that *label* validation (namespaces, `resolve_namespace`) does — a single
/// 64–253-char label is a legal Lease name and the API server accepts it (verified
/// against a live control plane), so this must not reject one.
#[must_use]
pub fn is_rfc1123_subdomain(s: &str) -> bool {
    !s.is_empty() && s.len() <= MAX_OBJECT_NAME_LEN && s.split('.').all(is_rfc1123_label)
}

#[cfg(test)]
mod tests {
    use super::{Seg, is_rfc1123_subdomain, lease_name, validate_lease_prefix};
    ...
    fn subdomain_validation() {
        assert!(is_rfc1123_subdomain("my-pinned-lease"));
        assert!(is_rfc1123_subdomain("a.b.c")); // dot-separated labels
        assert!(is_rfc1123_subdomain("svc"));
        assert!(!is_rfc1123_subdomain("")); // empty
        assert!(!is_rfc1123_subdomain("a/b")); // slash rewrites the path
        assert!(!is_rfc1123_subdomain("a,b")); // comma appends a selector term
        assert!(!is_rfc1123_subdomain("a b")); // whitespace
        assert!(!is_rfc1123_subdomain("Foo")); // uppercase
        assert!(!is_rfc1123_subdomain("-a")); // non-alphanumeric first
        assert!(!is_rfc1123_subdomain(&"a".repeat(254))); // over the 253 total budget
        // Subdomain validation (unlike label validation) imposes no per-label 63 cap:
        // a single 253-char label is a legal Lease name and the API server accepts it.
        assert!(is_rfc1123_subdomain(&"a".repeat(253))); // at the 253 limit, one label
    }
