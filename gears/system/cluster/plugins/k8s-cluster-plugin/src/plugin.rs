...
use crate::config::{
    K8sCacheConfig, K8sClusterConfig, K8sLeaderElectionConfig, K8sLockConfig, reject_zero,
};
...
async fn log_startup(resolved: &ResolvedClient, primitives: &[Primitive], budget: Duration) {
    let server_version = preflight::server_version(&resolved.client, budget).await;
    info!(
...
impl K8sClusterBuilder {
    ...
    pub async fn build_and_start(self) -> Result<K8sClusterHandle, ClusterError> {
        ...
        // Validate everything derivable from config *before* any request is issued
        // (§2.2, §3.4): the prefix is composed into object names and request paths (the
        // preflight canary included), and a zero interval/budget turns a bounded loop
        // into an unthrottled one or fails every request. The public constructors keep
        // their own prefix checks — that redundancy is defence, not duplication.
        ...
        if config.reaper {
            nonzero.push(("reaper_interval_ms", config.reaper_interval_ms));
        }
        if !config.cache_watch {
            nonzero.push(("cache_sweep_interval_ms", config.cache_sweep_interval_ms));
        }
        reject_zero(&nonzero)?;
        ...
        // Each preflight request runs under this per-request budget (§3.4).
        ...
        log_startup(&resolved, &primitives, budget).await;
        ...
        // `K8sCache::new` has already spawned its watcher/sweeper, which own their own
        // `Arc<CacheRuntime>` and stop only on the cache's cancellation token. A later
        // fallible constructor (e.g. `K8sLeaderElection::new` validating
        // `election_lease_names`) must therefore cancel the cache on error, or a `?` here
        // would return `Err` with no handle ever created and nothing left to tear the
        // tasks down — leaking a watch against the API server for the process lifetime.
        let leader = match K8sLeaderElection::new(&resolved, &leader_config, Arc::clone(&metrics)) {
        ...
        let lock = match K8sLock::new(&resolved, &lock_config, Arc::clone(&metrics)) {
...
impl K8sCacheBuilder {
    ...
    pub async fn build_and_start(self) -> Result<K8sCacheHandle, ClusterError> {
        ...
        // Validate config before any request (§2.2, §3.4) — see the combined builder.
        naming::validate_lease_prefix(&config.lease_prefix)?;
        let mut nonzero = vec![
            ("request_timeout_ms", config.request_timeout_ms),
            ("max_acquire_backoff_ms", config.max_acquire_backoff_ms),
...
impl K8sLeaderElectionBuilder {
    ...
    pub async fn build_and_start(self) -> Result<K8sLeaderElectionHandle, ClusterError> {
        ...
        // Validate config before any request (§2.2, §3.4) — see the combined builder.
        // `K8sLeaderElection::new` additionally validates the `election_lease_names`
        // overrides; the prefix/zero checks are hoisted here to fail before I/O.
        naming::validate_lease_prefix(&config.lease_prefix)?;
        reject_zero(&[
            ("request_timeout_ms", config.request_timeout_ms),
            ("max_acquire_backoff_ms", config.max_acquire_backoff_ms),
...
impl K8sLockBuilder {
    ...
    pub async fn build_and_start(self) -> Result<K8sLockHandle, ClusterError> {
        ...
        let metrics = self.metrics.unwrap_or_else(metrics_sink);
        let lock = Arc::new(K8sLock::new(&resolved, &config, metrics)?);
