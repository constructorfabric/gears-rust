...
/// Maximum resolved-namespace length — Kubernetes' own namespace constraint is an
/// RFC 1123 label capped at 63 characters.
...
///
/// The resolved value is validated as a lowercase RFC 1123 label of at most
/// [`MAX_NAMESPACE_LEN`] characters — Kubernetes' own namespace rule. Every source is
/// operator-controlled, so this is defence-in-depth and a fail-fast diagnostic: an
/// unvalidated value containing `/`, `..`, `?` or `#` would otherwise be interpolated
/// straight into every API URL path by `Api::namespaced`, the same way `lease_prefix`
/// is validated before it reaches an object name.
///
/// # Errors
///
/// [`ClusterError::InvalidConfig`] when no source yields a non-blank namespace (naming
/// the four sources so an operator knows what to wire), or when the resolved value is
/// not a legal Kubernetes namespace.
pub fn resolve_namespace(sources: &NamespaceSources) -> Result<String, ClusterError> {
    ...
    if resolved.len() > MAX_NAMESPACE_LEN || !naming::is_rfc1123_label(resolved) {
        return Err(ClusterError::InvalidConfig {
            reason: format!(
                "namespace `{resolved}` is not a legal Kubernetes namespace (a lowercase RFC 1123 \
                 label of at most {MAX_NAMESPACE_LEN} characters: a-z, 0-9, '-', alphanumeric \
                 first and last)"
            ),
        });
    }
...
mod tests {
    ...
    #[test]
    fn illegal_resolved_namespace_is_rejected() {
        // A value that would rewrite the request path or query, plus uppercase and an
        // over-length one — each must fail-fast rather than reach `Api::namespaced`.
        for bad in ["a/b", "..", "a?b", "a#b", "Gears", &"n".repeat(64)] {
            let sources = NamespaceSources {
                config: some(bad),
                ..NamespaceSources::default()
            };
            let err = resolve_namespace(&sources).unwrap_err();
            match err {
                cluster_sdk::ClusterError::InvalidConfig { reason } => {
                    assert!(
                        reason.contains("legal Kubernetes namespace"),
                        "for {bad:?}: {reason}"
                    );
                }
        ...
        // Exactly at the 63-char limit, and a legal label, are accepted.
        let ok = NamespaceSources {
            config: some(&"n".repeat(63)),
            ..NamespaceSources::default()
        };
        assert_eq!(resolve_namespace(&ok).unwrap(), "n".repeat(63));
    }
