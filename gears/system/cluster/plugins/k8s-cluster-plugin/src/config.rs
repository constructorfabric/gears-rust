...
#[derive(Debug, Clone, PartialEq, Eq, Deserialize, toolkit_macros::ExpandVars)]
...
// ── `Default` impls ──────────────────────────────────────────────────────────
//
// Each is hand-written and sources every field from the shared `default_*`
// functions, so the two default paths (serde and `Default`) cannot drift — asserted
// by `tests::default_matches_serde_defaults`. A `#[derive(Default)]` would be wrong:
// it would give `lease_prefix: ""`, `request_timeout_ms: 0`, `reaper: false`, none of
// which are the intended defaults. These exist so an out-of-crate caller can build a
// config to hand to a `builder(...)` — `#[non_exhaustive]` blocks a struct expression
// from another crate but not `Default::default()` followed by public-field assignment.

impl Default for K8sClusterConfig {
    fn default() -> Self {
        Self {
            namespace: None,
            identity: None,
            lease_prefix: default_lease_prefix(),
            request_timeout_ms: default_request_timeout(),
            max_acquire_backoff_ms: default_max_acquire_backoff(),
            skip_rbac_preflight: false,
            min_election_ttl_ms: default_min_election_ttl(),
            election_lease_names: BTreeMap::new(),
            reaper: default_true(),
            reaper_interval_ms: default_reaper_interval(),
            lock_object_retention_ms: default_lock_object_retention(),
            lock_name_cardinality_warn_threshold: default_lock_name_cardinality_warn(),
            cache_reads: ReadMode::default(),
            cache_watch: default_true(),
            cache_sweep_interval_ms: default_cache_sweep_interval(),
            max_value_bytes: default_max_value_bytes(),
            put_max_retries: default_put_max_retries(),
        }
    }
}
...
impl Default for K8sLockConfig {
    fn default() -> Self {
        Self {
            namespace: None,
            identity: None,
...
impl Default for K8sCacheConfig {
    fn default() -> Self {
        Self {
            namespace: None,
            identity: None,
...
// ── Startup range validation ───────────────────────────────────────────────────

/// Rejects a zero interval on any field the runtime uses as a sleep or a per-request
/// budget. Each such zero turns a bounded loop into an unthrottled one against the
/// API server (§12), or fails every request before it is issued (`request_timeout_ms:
/// 0` → an already-elapsed `timeout`). Called at the top of each `build_and_start`
/// before any I/O, sharing one implementation so the four shapes cannot drift.
///
/// # Errors
///
/// [`ClusterError::InvalidConfig`] naming the first field found to be zero.
pub fn reject_zero(fields: &[(&str, u64)]) -> Result<(), ClusterError> {
...
mod tests {
    use super::{
        ClusterError, K8sCacheConfig, K8sClusterConfig, K8sLeaderElectionConfig, K8sLockConfig,
        ReadMode, reject_zero,
    };
    use serde_json::json;
    ...
    /// The two default paths must not drift: a field added to `serde(default = …)`
    /// and forgotten in `impl Default` (or vice versa) is caught here.
    #[test]
    fn default_matches_serde_defaults() {
        assert_eq!(
            serde_json::from_value::<K8sClusterConfig>(json!({})).unwrap(),
            K8sClusterConfig::default()
        );
        assert_eq!(
            serde_json::from_value::<K8sLeaderElectionConfig>(json!({})).unwrap(),
            K8sLeaderElectionConfig::default()
        );
        assert_eq!(
            serde_json::from_value::<K8sLockConfig>(json!({})).unwrap(),
            K8sLockConfig::default()
        );
        assert_eq!(
            serde_json::from_value::<K8sCacheConfig>(json!({})).unwrap(),
            K8sCacheConfig::default()
        );
    }

    /// An out-of-crate caller can configure via public-field assignment on top of
    /// `Default` without a struct expression (`#[non_exhaustive]` forbids that from
    /// another crate) — this exercises the same path a real consumer takes, which is
    /// exactly why the `field_reassign_with_default` suggestion is not wanted here.
    #[test]
    #[expect(
        clippy::field_reassign_with_default,
        reason = "the Default-then-public-field-assign path is exactly what this test exercises: \
                  it must compile on a #[non_exhaustive] config and leave other fields at Default"
    )]
    fn default_then_field_assignment_configures() {
        let mut config = K8sCacheConfig::default();
        config.namespace = Some("gears".to_owned());
        config.cache_watch = false;
        // The real property: assigning some fields leaves the others at their Default
        // (a `#[derive(Default)]` would zero `lease_prefix` to "" instead of "cluster").
        assert_eq!(config.lease_prefix, "cluster");
    }

    #[test]
    fn reject_zero_flags_the_first_zero_field() {
        assert!(
            reject_zero(&[
                ("request_timeout_ms", 10_000),
                ("max_acquire_backoff_ms", 5_000)
            ])
            .is_ok()
        );
        let err = reject_zero(&[("request_timeout_ms", 0)]).unwrap_err();
        match err {
            ClusterError::InvalidConfig { reason } => {
                assert!(reason.contains("request_timeout_ms"));
                assert!(reason.contains("greater than zero"));
            }
            other => panic!("expected InvalidConfig, got {other:?}"),
        }
        // The named field is the offending one, not merely the first in the list.
        let err =
            reject_zero(&[("request_timeout_ms", 10_000), ("min_election_ttl_ms", 0)]).unwrap_err();
        match err {
            ClusterError::InvalidConfig { reason } => {
                assert!(reason.contains("min_election_ttl_ms"));
            }
            other => panic!("expected InvalidConfig, got {other:?}"),
        }
    }
