...
impl CacheRegistry {
    ...
    /// Fans a keyed [`CacheEvent`] out to every matching subscriber, pruning any whose
    /// consumer has dropped its watch — **whether or not** this event matched their
    /// interest. A subscriber whose buffer is momentarily full is kept and owed a
    /// [`CacheWatchEvent::Lagged`] (§6.3).
    ///
    /// The interest-independent prune is load-bearing: `unsubscribe` has no hot-path
    /// caller and a dropped [`CacheWatch`] does not deregister itself, so a `watch(key)`
    /// on a key that is never written would otherwise leave its [`Subscriber`] in the
    /// map for the process lifetime. Pruning is still event-driven, so a namespace with
    /// no cache writes at all reclaims nothing — acceptable, since unbounded growth
    /// needs unbounded `watch()` calls and any write to any key triggers a full sweep.
    pub fn dispatch(&self, event: &CacheEvent) {
        let key = event.key();
        let mut dead = Vec::new();
        for entry in &self.subscribers {
            let subscriber = entry.value();
            // Cheap relaxed atomic load first: reclaim any consumer that is gone,
            // regardless of interest, so an unmatched subscriber cannot leak.
    ...
    /// Delivers `Reset` to every subscriber after a re-list (§6.3), pruning any whose
    /// consumer has dropped its watch — the same interest-independent reclaim
    /// [`dispatch`](Self::dispatch) performs, so a `Reset` pass also bounds the map.
    pub fn broadcast_reset(&self) {
        let mut dead = Vec::new();
        for entry in &self.subscribers {
            let subscriber = entry.value();
            // Route through `deliver` (not a bare `try_send`) so a *full* buffer bumps
            // the subscriber's `dropped` count and surfaces later as `Lagged`, exactly
            // as an ordinary event does — otherwise a lagging consumer silently loses
            // the relist boundary. `deliver` returns `false` only on a closed sender,
            // which prunes it here just as `dispatch` does.
            if !deliver(subscriber, CacheWatchEvent::Reset) {
                dead.push(*entry.key());
            }
        }
        for id in dead {
...
mod tests {
    ...
    /// The leak the interest-independent prune fixes: a `watch(key)` on a key that is
    /// never written is dropped, and a single dispatch on an *unrelated* key reclaims
    /// it — the old interest-gated prune would have left it in the map forever.
    #[tokio::test]
    async fn a_dropped_unmatched_subscriber_is_pruned_by_an_unrelated_dispatch() {
        let registry = CacheRegistry::new();
        // 100 subscribers on keys that are never written, all dropped immediately.
        for i in 0..100 {
            let (tx, watch) = CacheWatch::channel(8);
            registry.subscribe_key(&format!("never-written/{i}"), tx);
            drop(watch);
        }
        // A live subscriber on the key we will actually dispatch.
        let (live_tx, _live) = CacheWatch::channel(8);
        registry.subscribe_key("hot", live_tx);
        assert_eq!(registry.len(), 101);

        // One dispatch on the hot key reclaims all 100 closed, unmatched subscribers.
        registry.dispatch(&CacheEvent::Changed {
            key: "hot".to_owned(),
        });
        assert_eq!(
            registry.len(),
            1,
            "closed subscribers are pruned regardless of interest match"
        );
    }

    #[tokio::test]
    async fn broadcast_reset_prunes_closed_subscribers() {
        let registry = CacheRegistry::new();
        let (dead_tx, dead) = CacheWatch::channel(8);
        let (live_tx, mut live) = CacheWatch::channel(8);
        registry.subscribe_prefix("p/", dead_tx);
        registry.subscribe_key("live", live_tx);
        drop(dead);
        assert_eq!(registry.len(), 2);

        registry.broadcast_reset();

        // The live subscriber got its Reset; the dropped one was reclaimed.
        assert!(matches!(live.recv().await, Some(CacheWatchEvent::Reset)));
        assert_eq!(registry.len(), 1, "a closed subscriber is pruned on reset");
    }
}
