...
impl ApiCounts {
    ...
    /// `PUT`s (guarded replaces) so far. Finer than [`mutating`](Self::mutating), which
    /// folds in creates/deletes: a scenario asserting "exactly the guarded-replace
    /// writes landed" (K8S-LEAD-004) reads this.
    pub fn updates(&self) -> u64 {
        self.updates.load(Ordering::Relaxed)
    }

    /// `PATCH`es so far — always zero, since the plugin only ever issues guarded PUTs.
    /// A non-zero value means a guarded `replace` was converted to an unconditional
    /// `patch` (the split-brain regression K8S-LEAD-004 guards against).
