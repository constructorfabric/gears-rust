...
/// defaults have that consistency guard, and rejected on `service_discovery`, whose
/// default has none. It defaults to `false`, so the loud startup failure stays the
/// default behaviour, and it does **not** launder capability validation — a consumer
...
/// intercepts it *before* any [`ProviderRegistry`](crate::provider::ProviderRegistry)
...
pub struct BackendBinding {
    ...
    ///
    /// [`DEFAULT_PROVIDER`] (`default`) is reserved: on `leader_election`, `lock`,
    /// or `service_discovery` it selects the SDK default backend over this
