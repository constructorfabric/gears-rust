...
#[test]
fn span_names_follow_dotted_lowercase_convention() {
    for name in ALL_SPANS {
        assert!(
            name.starts_with("cluster."),
            "span `{name}` must be under the `cluster.` namespace"
        );
        assert!(name.contains('.'), "span `{name}` must be dotted");
        assert_eq!(
            *name,
            name.to_lowercase(),
            "span `{name}` must be lowercase"
        );
    }
}

#[test]
fn metric_names_follow_underscored_lowercase_convention() {
    for name in ALL_METRICS {
        assert!(
            name.starts_with("cluster_"),
            "metric `{name}` must be prefixed `cluster_`"
        );
        assert!(
            !name.contains('.'),
            "metric `{name}` must not contain dots (Prometheus style)"
        );
        assert_eq!(
            *name,
            name.to_lowercase(),
            "metric `{name}` must be lowercase"
        );
    }
}
