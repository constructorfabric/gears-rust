...
use crate::cache::{CacheWatch, CacheWatchEvent, ClusterCacheBackend};
...
use crate::observability::{self, ClusterMetrics, NoopMetrics, logs, spans, transition};
...
// @cpt-dod:cpt-cf-clst-dod-sdk-default-backends-leader:p1
...
impl CasBasedLeaderElectionBackend {
    ...
    async fn join(&self, name: &str, config: ElectionConfig) -> Result<LeaderWatch, ClusterError> {
        ...
        let out = async {
            ...
            // @cpt-begin:cpt-cf-clst-algo-sdk-default-backends-cas-election:p1:inst-ce-watch
            let cache_watch = self.cache.watch(&key).await?;
            // @cpt-end:cpt-cf-clst-algo-sdk-default-backends-cas-election:p1:inst-ce-watch
            // @cpt-begin:cpt-cf-clst-algo-sdk-default-backends-cas-election:p1:inst-ce-claim
            let initial = match self
                .cache
                .put_if_absent(&key, node_id.as_bytes(), Some(config.ttl()))
                .await?
            {
                Some(_) => LeaderStatus::Leader,
                None => LeaderStatus::Follower,
            };
            // @cpt-end:cpt-cf-clst-algo-sdk-default-backends-cas-election:p1:inst-ce-claim
...
impl LeaderElectionBackend for CasBasedLeaderElectionBackend {
    fn features(&self) -> LeaderElectionFeatures {
        LeaderElectionFeatures::new(
            self.cache.consistency() == crate::cache::CacheConsistency::Linearizable,
        )
    }
...
impl ElectionTask {
    async fn run(
        ...
        // @cpt-begin:cpt-cf-clst-algo-leader-election-renewal:p1:inst-rn-interval
        let interval = self.config.renewal_interval();
        // @cpt-end:cpt-cf-clst-algo-leader-election-renewal:p1:inst-rn-interval
        ...
        loop {
            ...
            tokio::select! {
                ...
                // @cpt-begin:cpt-cf-clst-algo-leader-election-renewal:p1:inst-rn-tick
                // @cpt-begin:cpt-cf-clst-flow-leader-election-participate-observe:p1:inst-le-renew
                () = &mut tick => {
                ...
                // @cpt-end:cpt-cf-clst-flow-leader-election-participate-observe:p1:inst-le-renew
                // @cpt-end:cpt-cf-clst-algo-leader-election-renewal:p1:inst-rn-tick
    ...
    async fn renew_tick(&mut self) -> bool {
        ...
        async {
            ...
            // @cpt-begin:cpt-cf-clst-algo-leader-election-renewal:p1:inst-rn-attempt
            // @cpt-begin:cpt-cf-clst-algo-sdk-default-backends-cas-election:p1:inst-ce-renew
            match self
                .cache
                .compare_and_swap(
                    &self.key,
                ...
                .await
            {
                ...
                // @cpt-begin:cpt-cf-clst-algo-leader-election-renewal:p1:inst-rn-transient
                Err(err) if err.is_retryable() => self.on_transient().await,
                // @cpt-end:cpt-cf-clst-algo-leader-election-renewal:p1:inst-rn-transient
                Err(err) => self.close(err).await,
            }
            // @cpt-end:cpt-cf-clst-algo-sdk-default-backends-cas-election:p1:inst-ce-renew
            // @cpt-end:cpt-cf-clst-algo-leader-election-renewal:p1:inst-rn-attempt
        }
        .instrument(span)
        .await
    }
    ...
    async fn claim(&mut self) -> bool {
        match self
            .cache
            .put_if_absent(&self.key, self.node_id.as_bytes(), Some(self.config.ttl()))
            .await
        {
    ...
    async fn ensure_leader(&mut self) -> bool {
        ...
        // @cpt-begin:cpt-cf-clst-state-leader-election-leadership:p1:inst-st-acquire
        self.am_leader = true;
        self.record_transition(transition::ACQUIRED);
        self.sender.send_status(LeaderStatus::Leader).await.is_ok()
        // @cpt-end:cpt-cf-clst-state-leader-election-leadership:p1:inst-st-acquire
    }
    ...
    async fn lose_then_reclaim(&mut self) -> bool {
        ...
        self.missed = 0;
        self.record_transition(transition::LOST);
        // @cpt-begin:cpt-cf-clst-algo-sdk-default-backends-cas-election:p1:inst-ce-lost
        // @cpt-begin:cpt-cf-clst-algo-sdk-default-backends-cas-election:p1:inst-ce-reenroll
        // @cpt-begin:cpt-cf-clst-state-leader-election-leadership:p1:inst-st-lose
        // @cpt-begin:cpt-cf-clst-flow-leader-election-participate-observe:p1:inst-le-reenroll
        if self.sender.send_status(LeaderStatus::Lost).await.is_err() {
            return false;
        }
        // @cpt-end:cpt-cf-clst-state-leader-election-leadership:p1:inst-st-lose
        match self
            .cache
            .put_if_absent(&self.key, self.node_id.as_bytes(), Some(self.config.ttl()))
            .await
        {
            // @cpt-begin:cpt-cf-clst-state-leader-election-leadership:p1:inst-st-reacquire
            Ok(Some(_)) => {
                self.am_leader = true;
                self.record_transition(transition::ACQUIRED);
                self.sender.send_status(LeaderStatus::Leader).await.is_ok()
            }
            // @cpt-end:cpt-cf-clst-state-leader-election-leadership:p1:inst-st-reacquire
            // @cpt-begin:cpt-cf-clst-state-leader-election-leadership:p1:inst-st-follower
            Ok(None) => self
                .sender
                .send_status(LeaderStatus::Follower)
                .await
                .is_ok(),
            // @cpt-end:cpt-cf-clst-state-leader-election-leadership:p1:inst-st-follower
        ...
        // @cpt-end:cpt-cf-clst-flow-leader-election-participate-observe:p1:inst-le-reenroll
        // @cpt-end:cpt-cf-clst-algo-sdk-default-backends-cas-election:p1:inst-ce-reenroll
        // @cpt-end:cpt-cf-clst-algo-sdk-default-backends-cas-election:p1:inst-ce-lost
    }
    ...
    async fn on_transient(&mut self) -> bool {
        ...
        // @cpt-begin:cpt-cf-clst-algo-leader-election-renewal:p1:inst-rn-retry
        self.missed = self.missed.saturating_add(1);
        // @cpt-begin:cpt-cf-clst-algo-leader-election-renewal:p1:inst-rn-exceeded
        if self.missed > self.config.max_missed_renewals() {
            // @cpt-begin:cpt-cf-clst-algo-leader-election-renewal:p1:inst-rn-lost
            self.lose_then_reclaim().await
            // @cpt-end:cpt-cf-clst-algo-leader-election-renewal:p1:inst-rn-lost
        } else {
            true
        }
        // @cpt-end:cpt-cf-clst-algo-leader-election-renewal:p1:inst-rn-exceeded
        // @cpt-end:cpt-cf-clst-algo-leader-election-renewal:p1:inst-rn-retry
    }
