...
use crate::cache::{CacheEvent, CacheWatch, CacheWatchEvent, ClusterCacheBackend};
use crate::defaults::{ShutdownRevoke, identity};
use crate::discovery::{
...
/// Default heartbeat cadence (`ttl / 3`, mirroring the leader-election default).
const DEFAULT_RENEWAL: Duration = Duration::from_secs(10);
...
///
/// Each instance is stored under a per-instance key `"{name}/{instance_id}"`
...
/// lapses within the TTL. Discovery is served from a backend-maintained
/// membership view fed by a `watch_prefix("{name}/")` subscription, and
...
/// here.
// @cpt-dod:cpt-cf-clst-dod-sdk-default-backends-sd:p1
pub struct CacheBasedServiceDiscoveryBackend {
    ...
    /// translator to close its stream terminally, and every
    /// [`HeartbeatTask`] to stop renewing and exit. The store-maintainer task is
    /// left to lapse via TTL.
    shutdown: CancellationToken,
    /// Handles of the spawned [`WatchTranslator`] and [`HeartbeatTask`] tasks, so
    /// `revoke` can await their cancellation. Finished handles are pruned as new
    /// tasks are tracked.
...
impl CacheBasedServiceDiscoveryBackend {
    ...
    /// prefix watch.
    async fn ensure_maintainer(&self, name: &str) -> Result<(), ClusterError> {
        {
            let mut registry = self.shared.lock();
            if registry.maintained.contains(name) {
                return Ok(());
            }
            registry.maintained.insert(name.to_owned());
            registry.instances.entry(name.to_owned()).or_default();
        }
        let prefix = Self::prefix(name);
        // @cpt-begin:cpt-cf-clst-algo-sdk-default-backends-cache-sd:p1:inst-cs-watch
        let watch = match self.cache.watch_prefix(&prefix).await {
            // @cpt-end:cpt-cf-clst-algo-sdk-default-backends-cache-sd:p1:inst-cs-watch
            Ok(watch) => watch,
            Err(err) => {
...
impl ServiceDiscoveryBackend for CacheBasedServiceDiscoveryBackend {
    ...
    async fn register(&self, reg: ServiceRegistration) -> Result<ServiceHandle, ClusterError> {
        // @cpt-begin:cpt-cf-clst-flow-service-discovery-register:p1:inst-rg-noid
        // @cpt-begin:cpt-cf-clst-flow-service-discovery-register:p1:inst-rg-assign
        let instance_id = reg.instance_id.clone().unwrap_or_else(identity::fresh_id);
        // @cpt-end:cpt-cf-clst-flow-service-discovery-register:p1:inst-rg-assign
        // @cpt-end:cpt-cf-clst-flow-service-discovery-register:p1:inst-rg-noid
        ...
        let out = async {
            ...
            // @cpt-begin:cpt-cf-clst-flow-service-discovery-register:p1:inst-rg-register
            ...
            // @cpt-begin:cpt-cf-clst-algo-sdk-default-backends-cache-sd:p1:inst-cs-put
            self.cache
                .put(&key, &record.encode(), Some(self.ttl))
                .await?;
            // @cpt-end:cpt-cf-clst-algo-sdk-default-backends-cache-sd:p1:inst-cs-put
            // @cpt-end:cpt-cf-clst-flow-service-discovery-register:p1:inst-rg-register
            ...
            if self.ensure_maintainer(&reg.name).await.is_ok() {
            ...
            // Degraded mode (maintainer could not start): skip the pre-insert. With
            // nothing to reap it on TTL expiry it would otherwise read as live
            // forever, so `discover` stays best-effort empty until prefix watch is
            // available.
            ...
            // @cpt-begin:cpt-cf-clst-flow-service-discovery-register:p1:inst-rg-return
            Ok(handle)
            // @cpt-end:cpt-cf-clst-flow-service-discovery-register:p1:inst-rg-return
        }
        .instrument(span)
        .await;
        record_discovery(
    ...
    async fn discover(
        ...
        let out = async {
            ...
            // @cpt-begin:cpt-cf-clst-algo-sdk-default-backends-cache-sd:p1:inst-cs-filter
            // @cpt-begin:cpt-cf-clst-algo-service-discovery-filter:p1:inst-fl-foreach
            // @cpt-begin:cpt-cf-clst-flow-service-discovery-discover:p1:inst-ds-match
            ...
            // @cpt-end:cpt-cf-clst-flow-service-discovery-discover:p1:inst-ds-match
            // @cpt-end:cpt-cf-clst-algo-service-discovery-filter:p1:inst-fl-foreach
            // @cpt-end:cpt-cf-clst-algo-sdk-default-backends-cache-sd:p1:inst-cs-filter
            // @cpt-begin:cpt-cf-clst-algo-service-discovery-filter:p1:inst-fl-return
            // @cpt-begin:cpt-cf-clst-flow-service-discovery-discover:p1:inst-ds-return
            Ok(instances)
            // @cpt-end:cpt-cf-clst-flow-service-discovery-discover:p1:inst-ds-return
            // @cpt-end:cpt-cf-clst-algo-service-discovery-filter:p1:inst-fl-return
        }
        .instrument(span)
        .await;
...
impl ShutdownRevoke for CacheBasedServiceDiscoveryBackend {
    ...
    /// `ServiceWatchEvent::Closed(ClusterError::Shutdown)` and exits, and every
    /// heartbeat task stops renewing and exits — then awaits those tracked tasks,
    /// so an active watcher has observed the close before this returns. The store
    /// maintainer is not revoked; registered entries lapse via TTL
...
struct Registry {
    ...
    /// Service names with an active maintainer task.
...
/// `Weak<Shared>` no longer upgrades) or the cache watch ends.
...
impl HeartbeatTask {
    async fn run(mut self, mut receiver: ServiceCommandReceiver) {
        // @cpt-begin:cpt-cf-clst-algo-service-discovery-heartbeat:p1:inst-hb-renew
        let mut renewal =
        ...
        loop {
            tokio::select! {
                ...
                _ = renewal.tick() => {
                    ...
                    let _renewed = self.cache.put(&self.key, &self.record.encode(), Some(self.ttl)).await;
                }
                command = receiver.recv() => {
                    match command {
                        ...
                        Some(ServiceRequest::SetState { state, responder }) => {
                            // @cpt-begin:cpt-cf-clst-state-service-discovery-instance:p1:inst-is-disable
                            // @cpt-begin:cpt-cf-clst-state-service-discovery-instance:p1:inst-is-enable
                            self.record.state = state;
                            // @cpt-end:cpt-cf-clst-state-service-discovery-instance:p1:inst-is-enable
                            // @cpt-end:cpt-cf-clst-state-service-discovery-instance:p1:inst-is-disable
                            // @cpt-begin:cpt-cf-clst-flow-service-discovery-drain:p1:inst-dr-exclude
                            responder.respond(self.put_current().await);
                            // @cpt-end:cpt-cf-clst-flow-service-discovery-drain:p1:inst-dr-exclude
                        }
                        ...
                        // @cpt-begin:cpt-cf-clst-algo-service-discovery-heartbeat:p1:inst-hb-stop
                        // @cpt-begin:cpt-cf-clst-algo-service-discovery-heartbeat:p1:inst-hb-expire
                        None => return,
                        // @cpt-end:cpt-cf-clst-algo-service-discovery-heartbeat:p1:inst-hb-expire
                        // @cpt-end:cpt-cf-clst-algo-service-discovery-heartbeat:p1:inst-hb-stop
                    }
                }
            }
        }
        // @cpt-end:cpt-cf-clst-algo-service-discovery-heartbeat:p1:inst-hb-renew
    }

    async fn put_current(&self) -> Result<(), ClusterError> {
        self.cache
            .put(&self.key, &self.record.encode(), Some(self.ttl))
            .await
    }
}
