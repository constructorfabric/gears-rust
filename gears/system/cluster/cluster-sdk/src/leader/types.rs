...
// @cpt-dod:cpt-cf-clst-dod-leader-election-config:p1
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl ElectionConfig {
    ...
    pub fn new(ttl: Duration, max_missed_renewals: u8) -> Result<Self, ClusterError> {
        // @cpt-begin:cpt-cf-clst-algo-leader-election-config-validate:p1:inst-cv-check
        if ttl.is_zero() || max_missed_renewals == 0 {
            // @cpt-begin:cpt-cf-clst-algo-leader-election-config-validate:p1:inst-cv-reject
            ...
            // @cpt-end:cpt-cf-clst-algo-leader-election-config-validate:p1:inst-cv-reject
        }
        // @cpt-end:cpt-cf-clst-algo-leader-election-config-validate:p1:inst-cv-check
        let config = Self {
            ttl,
            max_missed_renewals,
        };
        ...
        // @cpt-begin:cpt-cf-clst-algo-leader-election-config-validate:p1:inst-cv-ok
        Ok(config)
        // @cpt-end:cpt-cf-clst-algo-leader-election-config-validate:p1:inst-cv-ok
    }
