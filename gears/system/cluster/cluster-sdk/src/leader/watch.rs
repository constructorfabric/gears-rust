...
// @cpt-dod:cpt-cf-clst-dod-leader-election-watch:p1
...
impl LeaderWatch {
    ...
    pub async fn resign(self) -> Result<(), ClusterError> {
        // @cpt-begin:cpt-cf-clst-flow-leader-election-step-down:p1:inst-sd-resign
        ...
        // @cpt-end:cpt-cf-clst-flow-leader-election-step-down:p1:inst-sd-resign
        // @cpt-begin:cpt-cf-clst-flow-leader-election-step-down:p1:inst-sd-release
        match reply_rx.await {
            // The backend completed the release and reported its real outcome.
            // @cpt-begin:cpt-cf-clst-flow-leader-election-step-down:p1:inst-sd-return
            Ok(result) => result,
            // @cpt-end:cpt-cf-clst-flow-leader-election-step-down:p1:inst-sd-return
        ...
        // @cpt-end:cpt-cf-clst-flow-leader-election-step-down:p1:inst-sd-release
    }
...
impl LeaderWatchSender {
    ...
    /// and a terminal `Status(Lost)` is best-effort enqueued. A terminal
    /// `Closed(Shutdown)` is then best-effort enqueued. The snapshot update is
    /// non-blocking and the event sends use `try_send`, so a backed-up event
    /// buffer cannot stall shutdown: the snapshot guard and `changed()`'s
    /// synthesized close keep the contract even if the events are dropped.
    pub(crate) fn revoke_for_shutdown(&self, was_leader: bool) {
        if was_leader {
            ...
            let _lost = self
                .events
                .try_send(LeaderWatchEvent::Status(LeaderStatus::Lost));
        }
        let _closed = self
            .events
            .try_send(LeaderWatchEvent::Closed(ClusterError::Shutdown));
    }
}
