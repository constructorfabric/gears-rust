...
#[tokio::test]
async fn extend_round_trips_backend_ok() {
    let (mut rx, guard) = LockGuard::channel("rate-limit".to_owned(), 1);
    let backend = tokio::spawn(async move {
        let Some(LockRequest::Extend {
            additional_ttl,
            responder,
        }) = rx.recv().await
        else {
            panic!("an extend request must arrive");
        };
        assert_eq!(additional_ttl, Duration::from_secs(5));
        responder.respond(Ok(()));
    });

    assert!(guard.extend(Duration::from_secs(5)).await.is_ok());
    assert!(backend.await.is_ok());
}

#[tokio::test]
async fn extend_is_repeatable_and_surfaces_backend_expired() {
    let (mut rx, guard) = LockGuard::channel("rate-limit".to_owned(), 1);
    let backend = tokio::spawn(async move {
        // First extension succeeds.
        let Some(LockRequest::Extend { responder, .. }) = rx.recv().await else {
            panic!("first extend request must arrive");
        };
        responder.respond(Ok(()));
        // Second extension is rejected — the TTL had elapsed.
        let Some(LockRequest::Extend { responder, .. }) = rx.recv().await else {
            panic!("second extend request must arrive");
        };
    ...
    assert!(guard.extend(Duration::from_secs(1)).await.is_ok());
    assert!(matches!(
        guard.extend(Duration::from_secs(1)).await,
...
#[tokio::test]
async fn extend_after_backend_gone_reports_expired() {
    let (rx, guard) = LockGuard::channel("rate-limit".to_owned(), 1);
    // Backend torn down before the consumer extends: nothing maintains the
    // claim, so an extension cannot keep it.
    drop(rx);
    assert!(matches!(
...
#[tokio::test]
async fn extend_errors_when_backend_drops_responder() {
    let (mut rx, guard) = LockGuard::channel("rate-limit".to_owned(), 1);
    let backend = tokio::spawn(async move {
        let Some(LockRequest::Extend { responder, .. }) = rx.recv().await else {
            panic!("an extend request must arrive");
...
async fn dropping_guard_performs_no_io_and_does_not_release() {
    let (mut rx, guard) = LockGuard::channel("rate-limit".to_owned(), 1);
    assert_eq!(guard.name(), "rate-limit");
