...
// @cpt-dod:cpt-cf-clst-dod-service-discovery-handle:p1
...
impl ServiceHandle {
    ...
    pub async fn set_state(&self, state: InstanceState) -> Result<(), ClusterError> {
        // @cpt-begin:cpt-cf-clst-flow-service-discovery-drain:p1:inst-dr-disable
        let (reply_tx, reply_rx) = oneshot::channel();
        if self
            .commands
        ...
        // @cpt-end:cpt-cf-clst-flow-service-discovery-drain:p1:inst-dr-disable
    }
    ...
    pub async fn deregister(self) -> Result<(), ClusterError> {
        // @cpt-begin:cpt-cf-clst-flow-service-discovery-drain:p1:inst-dr-deregister
        let (reply_tx, reply_rx) = oneshot::channel();
        if self
            .commands
        ...
        // @cpt-end:cpt-cf-clst-flow-service-discovery-drain:p1:inst-dr-deregister
    }
