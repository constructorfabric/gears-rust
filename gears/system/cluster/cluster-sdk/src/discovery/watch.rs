...
// @cpt-dod:cpt-cf-clst-dod-service-discovery-watch:p1
...
#[derive(Debug)]
pub(crate) enum ServiceWatchTrySendError {
...
impl ServiceWatchSender {
    ...
    /// Tries to deliver `event` without ever blocking the caller, returning the
    /// unsent event if the buffer is full or the consumer dropped the watch.
    ///
    ...
    ) -> Result<(), ServiceWatchTrySendError> {
