...
impl ServiceDiscoveryBackend for ScopedServiceDiscoveryBackend {
    ...
    async fn register(&self, mut reg: ServiceRegistration) -> Result<ServiceHandle, ClusterError> {
        ...
        // @cpt-begin:cpt-cf-clst-algo-scoping-polyfill-prefix-translate:p1:inst-pt-sd
        // @cpt-begin:cpt-cf-clst-algo-scoping-polyfill-prefix-translate:p1:inst-pt-sd-meta
        // @cpt-begin:cpt-cf-clst-flow-scoping-polyfill-scoped-names:p1:inst-sn-meta
        reg.name = scope::apply(&self.prefix, &reg.name);
        self.inner.register(reg).await
        // @cpt-end:cpt-cf-clst-flow-scoping-polyfill-scoped-names:p1:inst-sn-meta
        // @cpt-end:cpt-cf-clst-algo-scoping-polyfill-prefix-translate:p1:inst-pt-sd-meta
        // @cpt-end:cpt-cf-clst-algo-scoping-polyfill-prefix-translate:p1:inst-pt-sd
    }
