...
// @cpt-dod:cpt-cf-clst-dod-service-discovery-types:p1
...
impl DiscoveryFilter {
    ...
    pub fn matches(&self, instance: &ServiceInstance) -> bool {
        // @cpt-begin:cpt-cf-clst-algo-service-discovery-filter:p1:inst-fl-state
        if !self.state.matches(instance.state) {
            // @cpt-begin:cpt-cf-clst-algo-service-discovery-filter:p1:inst-fl-state-skip
            return false;
            // @cpt-end:cpt-cf-clst-algo-service-discovery-filter:p1:inst-fl-state-skip
        }
        // @cpt-end:cpt-cf-clst-algo-service-discovery-filter:p1:inst-fl-state
        // @cpt-begin:cpt-cf-clst-algo-service-discovery-filter:p1:inst-fl-meta
        self.metadata.iter().all(|(key, predicate)| {
            // @cpt-begin:cpt-cf-clst-algo-service-discovery-filter:p1:inst-fl-meta-check
            // @cpt-begin:cpt-cf-clst-algo-service-discovery-filter:p1:inst-fl-meta-skip
            predicate.matches(instance.metadata.get(key).map(String::as_str))
            // @cpt-end:cpt-cf-clst-algo-service-discovery-filter:p1:inst-fl-meta-skip
            // @cpt-end:cpt-cf-clst-algo-service-discovery-filter:p1:inst-fl-meta-check
        })
        // @cpt-end:cpt-cf-clst-algo-service-discovery-filter:p1:inst-fl-meta
    }
}
