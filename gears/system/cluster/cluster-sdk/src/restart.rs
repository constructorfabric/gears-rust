...
// @cpt-dod:cpt-cf-clst-dod-watch-auto-restart-policy:p1
...
impl RetryPolicy {
    ...
    fn backoff_for(&self, attempt: u32) -> Duration {
        // @cpt-begin:cpt-cf-clst-algo-watch-auto-restart-backoff:p1:inst-bo-grow
        ...
        // @cpt-end:cpt-cf-clst-algo-watch-auto-restart-backoff:p1:inst-bo-grow

        // @cpt-begin:cpt-cf-clst-algo-watch-auto-restart-backoff:p1:inst-bo-jitter
        ...
        // @cpt-end:cpt-cf-clst-algo-watch-auto-restart-backoff:p1:inst-bo-jitter
    }
}
...
// @cpt-dod:cpt-cf-clst-dod-watch-auto-restart-combinator:p1
...
impl<W: RestartableWatch> RestartingWatch<W> {
    ...
    pub(crate) fn new(inner: W, policy: RetryPolicy) -> Self {
        // @cpt-begin:cpt-cf-clst-flow-watch-auto-restart-wrap:p1:inst-ar-wrap
        ...
        // @cpt-end:cpt-cf-clst-flow-watch-auto-restart-wrap:p1:inst-ar-wrap
    }
    ...
    ///   `Reset` telling the consumer to re-read state);
    ...
    pub async fn recv(&mut self) -> Option<W::Event> {
        ...
        // @cpt-begin:cpt-cf-clst-flow-watch-auto-restart-wrap:p1:inst-ar-next
        let Some(event) = self.inner.next_event().await else {
            // Underlying stream ended without a terminal Closed; nothing to
            // reconnect to.
            self.terminated = true;
            return None;
        };
        // @cpt-end:cpt-cf-clst-flow-watch-auto-restart-wrap:p1:inst-ar-next
        match W::closed_error(&event) {
            // Not a terminal close: ordinary event, Lagged, or Reset.
            None => Some(event),
            Some(err) => {
                // @cpt-begin:cpt-cf-clst-flow-watch-auto-restart-wrap:p1:inst-ar-nonretryable
                if !err.is_retryable() || !self.inner.can_resubscribe() {
                    ...
                    // @cpt-begin:cpt-cf-clst-flow-watch-auto-restart-wrap:p1:inst-ar-propagate
                    self.terminated = true;
                    return Some(event);
                    // @cpt-end:cpt-cf-clst-flow-watch-auto-restart-wrap:p1:inst-ar-propagate
                }
                // @cpt-end:cpt-cf-clst-flow-watch-auto-restart-wrap:p1:inst-ar-nonretryable
                // Retryable and reconnectable: absorb the close and reconnect.
                // @cpt-begin:cpt-cf-clst-flow-watch-auto-restart-wrap:p1:inst-ar-retryable
                if self.reconnect().await {
                    // @cpt-begin:cpt-cf-clst-flow-watch-auto-restart-wrap:p1:inst-ar-reset
                    Some(W::reset_event())
                    // @cpt-end:cpt-cf-clst-flow-watch-auto-restart-wrap:p1:inst-ar-reset
                } else {
                    // Retry cap exhausted — propagate the most recent close.
                    // @cpt-begin:cpt-cf-clst-flow-watch-auto-restart-wrap:p1:inst-ar-cap
                    // @cpt-begin:cpt-cf-clst-flow-watch-auto-restart-wrap:p1:inst-ar-cap-propagate
                    self.terminated = true;
                    Some(event)
                    // @cpt-end:cpt-cf-clst-flow-watch-auto-restart-wrap:p1:inst-ar-cap-propagate
                    // @cpt-end:cpt-cf-clst-flow-watch-auto-restart-wrap:p1:inst-ar-cap
                }
                // @cpt-end:cpt-cf-clst-flow-watch-auto-restart-wrap:p1:inst-ar-retryable
            }
        }
    }
    ...
    async fn reconnect(&mut self) -> bool {
        loop {
            // @cpt-begin:cpt-cf-clst-algo-watch-auto-restart-backoff:p1:inst-bo-cap
            if self
                .policy
                .max_retries
                .is_some_and(|cap| self.attempts >= cap)
            {
                // @cpt-begin:cpt-cf-clst-algo-watch-auto-restart-backoff:p1:inst-bo-stop
                return false;
                // @cpt-end:cpt-cf-clst-algo-watch-auto-restart-backoff:p1:inst-bo-stop
            }
            // @cpt-end:cpt-cf-clst-algo-watch-auto-restart-backoff:p1:inst-bo-cap
            let delay = self.policy.backoff_for(self.attempts);
            self.attempts = self.attempts.saturating_add(1);
            tokio::time::sleep(delay).await;
            // @cpt-begin:cpt-cf-clst-algo-watch-auto-restart-backoff:p1:inst-bo-reset
            if let Ok(fresh) = self.inner.resubscribe().await {
                self.inner = fresh;
                self.attempts = 0;
                self.emit_reset();
                return true;
            }
            // @cpt-end:cpt-cf-clst-algo-watch-auto-restart-backoff:p1:inst-bo-reset
        }
    }
}
