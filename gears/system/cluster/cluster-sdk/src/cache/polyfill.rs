...
// @cpt-dod:cpt-cf-clst-dod-scoping-polyfill-polling:p1
...
/// per key (`N + 1` round-trips), and detection latency is bounded by `interval`
/// — there is no millisecond-level precision and rapid create-then-delete churn
/// within one interval can be missed. Prefer a backend with **native**
/// prefix-watch support (`features().prefix_watch == true`) at scale; reach for
/// this polyfill only when the bound backend lacks it.
///
/// # Lifecycle
///
...
async fn poll_loop(
    ...
    // @cpt-begin:cpt-cf-clst-algo-scoping-polyfill-poll-diff:p1:inst-pd-tick
    loop {
        ticker.tick().await;
        ...
        // @cpt-begin:cpt-cf-clst-flow-scoping-polyfill-prefix-watch:p1:inst-pw-drop
        if sender.is_closed() {
            // @cpt-begin:cpt-cf-clst-flow-scoping-polyfill-prefix-watch:p1:inst-pw-stop
            return;
            // @cpt-end:cpt-cf-clst-flow-scoping-polyfill-prefix-watch:p1:inst-pw-stop
        }
        // @cpt-end:cpt-cf-clst-flow-scoping-polyfill-prefix-watch:p1:inst-pw-drop
        // @cpt-begin:cpt-cf-clst-algo-scoping-polyfill-poll-diff:p1:inst-pd-list
        // @cpt-begin:cpt-cf-clst-flow-scoping-polyfill-prefix-watch:p1:inst-pw-poll
        let current = match snapshot(cache.as_ref(), &prefix).await {
        ...
        // @cpt-end:cpt-cf-clst-flow-scoping-polyfill-prefix-watch:p1:inst-pw-poll
        // @cpt-end:cpt-cf-clst-algo-scoping-polyfill-poll-diff:p1:inst-pd-list
        // @cpt-begin:cpt-cf-clst-algo-scoping-polyfill-poll-diff:p1:inst-pd-diff
        // @cpt-begin:cpt-cf-clst-algo-scoping-polyfill-poll-diff:p1:inst-pd-emit
        // @cpt-begin:cpt-cf-clst-flow-scoping-polyfill-prefix-watch:p1:inst-pw-emit
        ...
        // @cpt-end:cpt-cf-clst-flow-scoping-polyfill-prefix-watch:p1:inst-pw-emit
        // @cpt-end:cpt-cf-clst-algo-scoping-polyfill-poll-diff:p1:inst-pd-emit
        // @cpt-end:cpt-cf-clst-algo-scoping-polyfill-poll-diff:p1:inst-pd-diff
        previous = current;
    }
    // @cpt-end:cpt-cf-clst-algo-scoping-polyfill-poll-diff:p1:inst-pd-tick
}

/// Lists the keys under `prefix` and reads each key's current version, realizing
/// the documented `N + 1` round-trip cost. A key that vanishes between the scan
/// and its `get` is simply omitted from the snapshot.
async fn snapshot(
    cache: &dyn ClusterCacheBackend,
    prefix: &str,
) -> Result<HashMap<String, u64>, crate::error::ClusterError> {
    let keys = cache.scan_prefix(prefix).await?;
    let mut snapshot = HashMap::with_capacity(keys.len());
    for key in keys {
        if let Some(entry) = cache.get(&key).await? {
            snapshot.insert(key, entry.version);
        }
    }
    Ok(snapshot)
}
