...
use crate::cache::types::{CacheConsistency, CacheEntry, CacheEvent, CacheFeatures};
        ...
        tokio::spawn(async move {
            while let Some(event) = inner.recv().await {
                ...
                if tx.send(forwarded).await.is_err() {
                    // The consumer dropped the watch: stop forwarding.
                    return;
                }
            }
        });
...
impl ClusterCacheBackend for ScopedCacheBackend {
    ...
    async fn put(
        ...
        self.inner
            .put(&scope::apply(&self.prefix, key), value, ttl)
            .await
    }
    ...
    async fn put_if_absent(
        ...
        self.inner
            .put_if_absent(&scope::apply(&self.prefix, key), value, ttl)
            .await
    }
