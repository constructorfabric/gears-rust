...
- **`ClusterProfile`** — a typed profile marker (`const NAME`) that is declared
  once on a zero-sized type, plus `profile_scope` and `validate_cluster_name`
  helpers that map a profile to the stable `cluster:{profile}` lookup scope.
...
```rust
use cluster_sdk::{profile_scope, ClusterProfile};

// Declare a profile once, by type.
struct OrdersProfile;
impl ClusterProfile for OrdersProfile {
    const NAME: &'static str = "orders";
}

// Resolve the stable lookup scope for that profile.
let scope = profile_scope(OrdersProfile::NAME)?;
