...
use cluster_sdk::cache::{
    CacheEvent, CacheWatch, CacheWatchEvent, ClusterCacheBackend, ClusterCacheV1,
};
...
async fn cache_watch_preserves_per_key_ordering_end_to_end() {
    ...
    assert!(cache.put("k", b"v1", None).await.is_ok());
    assert!(cache.put("k", b"v2", None).await.is_ok());
