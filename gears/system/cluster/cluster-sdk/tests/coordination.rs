...
use cluster_sdk::cache::{CacheEvent, CacheWatchEvent, ClusterCacheBackend, ClusterCacheV1};
...
use cluster_sdk::{CasBasedDistributedLockBackend, CasBasedLeaderElectionBackend};
...
async fn cas_conflict_surfaces_on_stale_version() {
    ...
    let Ok(Some(created)) = cache.put_if_absent("cas", b"a", None).await else {
    ...
    assert!(matches!(
        cache
            .compare_and_swap("cas", created.version + 5, b"z", None)
            .await,
    ...
    let Ok(swapped) = cache
        .compare_and_swap("cas", created.version, b"b", None)
        .await
    else {
...
async fn composable_scoping_translates_prefixes_on_write_and_read() {
    ...
    assert!(scoped.put("key", b"v", None).await.is_ok());
