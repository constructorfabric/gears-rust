...
use crate::domain::ports::metrics::{
    ...
    SaOpMetricsPort, TenantLifecycleMetricsPort, UserOpMetricsPort,
};
...
impl Gear for KeycloakIdpPluginGear {
    async fn init(&self, ctx: &GearCtx) -> anyhow::Result<()> {
        ...
        // (DESIGN "Security and Data Protection"), symmetric with the AM DB DSN's `${PGPASSWORD}`.
        let cfg: KeycloakIdpPluginConfig = ctx
            .config_expanded()
            .map_err(|e| anyhow::anyhow!("keycloak-idp-plugin: failed to load config: {e}"))?;

        // DESIGN "Component Model": validate the SecretRef template at init by interpolating
        ...
        // An empty allowlist is legitimate ("no scope is requestable"), so
        // `validate` cannot reject it — but it is also what an operator who
        // simply forgot the key gets, and the symptom is every scoped create
        // answering 400 for a reason that reads as a caller bug. Say it once
        // at startup so the config, not the callers, is the first suspect.
        if cfg.service_account.scope_allowlist.is_empty() {
            tracing::info!(
                target: "keycloak_idp_plugin.init",
                realm = %cfg.service_account.realm,
                "service_account.scope_allowlist is empty: no client scope is requestable, \
                 so every create carrying a non-empty `scopes` list will be rejected as \
                 invalid input; set the key if callers need scopes"
            );
        }

        // ---- Plugin-owned system ctx (DESIGN "Security and Data Protection"). ----
        ...
        let sa_metrics: Arc<dyn SaOpMetricsPort> = Arc::clone(&metrics_adapter) as _;
        ...
        // Pre-warm bootstrap admin token per DESIGN "Component Model" init lifecycle.
        // DESIGN "Component Model": init fails fast if the token endpoint is unreachable.
        ...
        // ---- Facades. ----
        // Build the service-account facade first because tenant deprovision
        // requires its purge port at construction time. Omitting the cascade
        // is therefore a compile-time wiring error, never a silent no-op.
        let sa_facade = Arc::new(ServiceAccountFacade::new(
            cfg.service_account.clone(),
        ...
        let tenant_facade = Arc::new(TenantFacade::new(
            ...
            Arc::clone(&failure_metrics),
            Arc::clone(&sa_facade) as Arc<dyn TenantAccountPurgeHook>,
        ));
        ...
        // suffix that distinguishes this plugin from the in-process
        // `static-idp-plugin` (which publishes as
        // `cf.builtin.static_idp.plugin.v1`). Both sit in the
        // `cf.builtin` namespace; the vendor knob, not the namespace,
        ...
        // No separate machine-identity registration: the service-account
        // methods are part of `IdpPluginClient`, so the single scoped
        // registration below carries them along with tenant and user
        // lifecycle. AM resolves one client for all three halves.
        ...
        tracing::info!(
            ...
            "published PluginV1<IdpPluginSpecV1>, registered scoped IdpPluginClient (tenant, user, and machine-identity halves)"
        );
...
fn is_pre_warm_retryable(err: &crate::domain::error::PluginError) -> bool {
    ...
    match err {
        ...
        // OpenBao readiness lag is a real deploy-ordering transient — keycloak-idp
...
#[allow(
    ...
    reason = "single retry loop whose per-attempt tracing (attempt count, outcome, budget) is the operator-facing contract; splitting it would separate the log fields from the branch that emits them"
)]
    ...
    let (attempts_used, final_err) = last_err.unwrap_or_else(|| {
        ...
        (
            ...
            crate::domain::error::PluginError::Config {
                detail: "internal: pre_warm retry loop exhausted with no recorded error".to_owned(),
            },
        )
    });
    Err(anyhow::anyhow!(
