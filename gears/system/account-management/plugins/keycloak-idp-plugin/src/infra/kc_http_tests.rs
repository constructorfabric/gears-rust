...
impl ReqwestKcTransport {
    ...
    #[must_use]
    pub fn with_retry_policy(http: reqwest::Client, retry_policy: HttpRetryPolicy) -> Self {
...
///
/// Pins the DESIGN "Component Model" Transport responsibility: W3C trace
/// propagation on every outbound Keycloak Admin REST call, so the Keycloak
/// hop joins the platform trace tree.
#[tokio::test]
async fn raw_request_injects_traceparent_header() {
    ...
    let captured = std::sync::Arc::new(parking_lot::Mutex::new(None::<bool>));
    let server = wiremock::MockServer::start().await;
    let cap = captured.clone();
    wiremock::Mock::given(wiremock::matchers::method("GET"))
        .respond_with(move |req: &wiremock::Request| {
            let has_tp = req.headers.contains_key("traceparent");
            *cap.lock() = Some(has_tp);
...
// ─────────────────────────────────────────────────────────────────────────────
// Retry-policy tests (DESIGN "Component Model" / PRD "Gear-Specific NFRs")
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// Method-aware transport retry (run am-functional-22)
//
...
/// A non-idempotent `POST` that receives a retryable-looking HTTP status must
/// be surfaced after one attempt. The response does not prove that the server
/// skipped the mutation, so replaying it could turn an uncertain create into a
/// misleading conflict.
#[tokio::test]
async fn raw_request_does_not_retry_post_on_503() {
    let server = wiremock::MockServer::start().await;
    wiremock::Mock::given(wiremock::matchers::method("POST"))
        ...
        .await;
    wiremock::Mock::given(wiremock::matchers::method("POST"))
        .respond_with(wiremock::ResponseTemplate::new(409).set_body_string("already exists"))
        .mount(&server)
        .await;

    let transport = transport_with_retries(3);
    let url = format!("{}/admin/realms/test/clients", server.uri());
    let (status, _) = transport
        .raw_request("POST", &url, "tok", None, "/admin/realms/{realm}/clients")
        .await
        .expect("the first HTTP response must be returned without replay");
    ...
    assert_eq!(
        reqs.len(),
        1,
        "non-idempotent POST must not be replayed after a 5xx response"
    );
}
