...
///
/// DESIGN "Component Model" mandates that every `KcRest` construction site on the transport
...
/// tracing span into the request's header map. Per DESIGN "Component Model" this binds
...
#[must_use]
pub fn method_static(m: &Method) -> &'static str {
    match *m {
        Method::GET => "GET",
        Method::POST => "POST",
        Method::PUT => "PUT",
...
#[must_use]
pub fn transport_error(method: &Method, path_template: &str, e: &reqwest::Error) -> PluginError {
...
impl ReqwestKcTransport {
    ...
    /// anchor (DESIGN "Component Model" `tls_ca_bundle_ref`). When `None`, the system trust
    /// store is used.
    ///
    /// # Errors
    ...
    /// `retry_http_statuses` independently gates retrying 5xx/429 replies.
    /// Neither a transport failure nor a transient-looking HTTP reply proves
    /// that the server skipped a mutation, so both gates must be disabled for
    /// non-idempotent requests. Idempotent verbs and the safe-to-replay token
    /// `form_post` enable both gates.
            ...
            match result {
                Ok(resp) => {
                    ...
                    if retry_http_statuses
                        && crate::domain::error::is_retryable_status(status)
                        && attempt < policy.max_retries
                    {
                        ...
                        // Drain the body so the connection can be reused; its content is
...
impl KcTransport for ReqwestKcTransport {
    async fn raw_request(
        ...
        // Non-idempotent verbs (POST) must not be replayed after either a
        // transport failure or a retryable HTTP status: the first request may
        // already have mutated server state. Idempotent verbs enable both.
        let retry = idempotent_method(&m);
        self.execute_with_retry(build, &m, path_template, retry, retry)
            .await
    }
    ...
    async fn form_post(
        ...
        let build = move || {
            ...
            // DESIGN "Component Model": every KC Admin REST hop carries W3C
        ...
        // side effect, so it opts back into both retry classes — important
        // under load where the token endpoint is the bottleneck.
        self.execute_with_retry(build, &Method::POST, path_template, true, true)
            .await
    }
    ...
    async fn get_unauthenticated(
        ...
        let build = move || {
            ...
            // DESIGN "Component Model": health-probe GETs reach KC and MUST carry
            // traceparent so a flat outage surfaces with the platform
            // trace link intact for post-mortem.
            inject_trace_headers(built.headers_mut());
            Ok(built)
        };
        self.execute_with_retry(build, &Method::GET, path_template, true, true)
            .await
    }
}
