...
//!
//! Per DESIGN "Observability", `realm_name` is a partner-controlled label.
...
//! the `realm_name` label is dropped from all subsequent emissions for
//! realms not already recorded. The adapter also offers the value to the
//! current span as a `realm.name` field, but that is a no-op unless an
//! enclosing span declares the field — no span in this crate does, so
//! today an over-cap realm name is dropped rather than relocated. The
...
use crate::domain::metrics::{
    ...
    KEYCLOAK_IDP_PLUGIN_SA_OP_DURATION, KEYCLOAK_IDP_PLUGIN_USER_OP_DURATION,
};
...
pub struct KeycloakIdpPluginMetricsAdapter {
    ...
    // Cardinality watchdog (DESIGN "Observability"). `0` disables.
...
impl KeycloakIdpPluginMetricsAdapter {
    ...
    /// AM's posture; a `tracing::warn!` naming the realm that wanted in
    /// fires on EVERY over-cap observation, not once — over-cap realms
    /// are deliberately never recorded, so there is no state that would
    /// suppress the repeat. Operators should expect a steady signal
    /// while the fleet sits above the cap, and should alert on its
    /// presence rather than on a single transition.
    pub fn observe_realm_and_should_emit(&self, realm: &str) -> bool {
        ...
        if self.seen_realms.len() < cap {
            ...
            self.seen_realms.insert(realm.to_owned());
            return true;
        }
        // Cap reached and this realm is genuinely new. This warns on
        // every such observation rather than once: the realm is not
        // inserted (that is what keeps the budget honest), so nothing
        // records that we already complained about it. The warn names
        // the realm that wanted in, so operators see the proximate
        // cause on each occurrence.
        tracing::warn!(
            target: "keycloak_idp_plugin.metrics",
...
impl SaOpMetricsPort for KeycloakIdpPluginMetricsAdapter {
