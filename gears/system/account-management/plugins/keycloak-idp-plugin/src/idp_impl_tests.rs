...
#[test]
fn translate_provision_realm_create_not_retained_is_clean_failure() {
    let e = PluginError::RealmCreateNotRetained {
        realm: "partner-acme".into(),
        detail: "ambig:kc_realm_create: kc:POST /admin/realms -> 503: unavailable".into(),
    };
    let f = translate_provision_failure(e);
    assert!(matches!(f, IdpProvisionFailure::CleanFailure { .. }));
    assert!(f.detail().contains("partner-acme"));
    assert!(f.detail().contains("retry is safe"));
...
/// (400). The pre-KC caller-input rejections moved to
...
// ----- deprovision_tenant service-account cascade -----
...
mod deprovision_cascade {
    ...
    use super::*;
    use crate::config::{ServiceAccountConfig, TenantFacadeConfig, UserFacadeConfig};
    ...
    use crate::domain::test_support::{
        ...
        noop_metadata_metrics, noop_sa_metrics, noop_tenant_metrics, noop_user_metrics,
    };
    ...
    fn build_plugin(server: &MockServer) -> KeycloakIdpPlugin {
        ...
        let tenant_facade = Arc::new(TenantFacade::new(
            ...
            noop_failure_metrics(),
            Arc::clone(&sa_facade) as Arc<dyn crate::domain::ports::purge::TenantAccountPurgeHook>,
        ));
    ...
    #[tokio::test]
    async fn deprovision_tenant_missing_metadata_does_not_purge_service_accounts() {
    ...
    /// Tenant teardown deletes the tenant's service accounts before the
    /// group — no orphaned `client_credentials` clients survive.
    #[tokio::test]
    async fn deprovision_tenant_purges_service_accounts() {
        let server = MockServer::start().await;
        mount_tokens_and_probe(&server).await;
        let tenant = Uuid::new_v4();
        let group_uuid = Uuid::new_v4();
        let sa_uuid = "55555555-5555-5555-5555-555555555555";
        ...
        Mock::given(method("DELETE"))
            .and(path(format!("/admin/realms/platform/clients/{sa_uuid}")))
