...
"#;
    let cfg: KeycloakIdpPluginConfig = serde_saphyr::from_str(yaml).expect("valid yaml");
...
fn template_with_invalid_chars_is_rejected_via_secret_ref_validation() {
    ...
    let bad_template = "bad:realm:{realm_name}";
...
#[test]
fn service_account_defaults_apply_when_section_absent() {
    // `service_account:` key is intentionally absent — exercises #[serde(default)] wiring.
    let yaml = r"
security_context: { platform_tenant_id: '00000000-0000-0000-0000-000000000000' }
keycloak:
  base_url: https://k
...
";
    let cfg: KeycloakIdpPluginConfig = serde_saphyr::from_str(yaml).expect("valid yaml");
    assert_eq!(cfg.service_account.realm, "platform");
    assert!(cfg.service_account.scope_allowlist.is_empty());
    assert_eq!(cfg.service_account.per_tenant_quota, 10);
    assert!(cfg.service_account.validate().is_ok());
}

#[test]
fn service_account_realm_must_not_be_blank() {
    let cfg = ServiceAccountConfig {
        realm: "   ".into(),
        ..Default::default()
    };
    let err = cfg.validate().expect_err("must reject blank realm");
    let PluginError::Config { detail } = err else {
        unreachable!("blank realm must be a typed config error");
    };
    assert!(detail.contains("service_account.realm"));
}
