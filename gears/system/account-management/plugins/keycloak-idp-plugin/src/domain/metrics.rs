...
//! in one place.
//!
//! ## Cardinality watchdog (DESIGN "Observability")
//!
...
/// KC Admin token refresh. Counter, labelled by `outcome` × `tier` ×
/// `realm` (capped). Emitted from the token-acquisition slow path in
/// [`crate::domain::kc::factory`].
pub const KEYCLOAK_IDP_PLUGIN_KC_ADMIN_TOKEN_REFRESH: &str =
    "keycloak_idp_plugin_kc_admin_token_refresh_total";

/// Admin `client_secret` re-resolution. Counter, labelled by `outcome` ×
/// `tier` × `realm` (capped). Emitted alongside
/// [`KEYCLOAK_IDP_PLUGIN_KC_ADMIN_TOKEN_REFRESH`] on every credential
/// resolution, including the first acquisition for a `(realm, client_id)`.
...
/// Service-account op latency. Histogram, unit seconds.
pub const KEYCLOAK_IDP_PLUGIN_SA_OP_DURATION: &str = "keycloak_idp_plugin_sa_op_duration_seconds";
