...
#[test]
fn realm_create_not_retained_detail_and_metric_label_are_stable() {
    ...
    assert_eq!(
        format!("{e}"),
        "realm 'partner-acme' was not retained after failed create; retry is safe: \
         ambig:kc_realm_create: kc:POST /admin/realms -> 503: unavailable"
    );
    assert_eq!(failure_variant_label(&e), "realm_create_not_retained");
}
...
fn ambiguous_stage_display_strings_are_stable() {
    ...
    // (DESIGN "Interactions & Sequences" / "API Contracts") and MUST NOT silently change — Phase
...
#[test]
fn sa_error_variants_have_stable_labels() {
    let invalid = PluginError::SaInvalidInput {
        detail: "d".into(),
...
#[test]
fn sa_ambiguous_stages_display() {
    assert_eq!(
...
#[test]
fn sa_invalid_input_detail_format() {
    let e = PluginError::SaInvalidInput {
        detail: "name too long".into(),
        field: Some("name".into()),
    };
    assert_eq!(format!("{e}"), "sa invalid input: name too long");
}

#[test]
fn sa_not_found_detail_format() {
    let e = PluginError::SaNotFound {
        detail: "client abc not found".into(),
    };
    assert_eq!(format!("{e}"), "sa not found: client abc not found");
}
