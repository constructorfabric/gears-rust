//! `TenantFacade` — provision/deprovision tenants against Keycloak per DESIGN "Interactions & Sequences".
//!
...
//!
//! See DESIGN "Domain Model" (the Adopted row's narrow emptiness scope and
//! the `admin_secret_ref = None` rule for default-shared-realm tenants),
//! DESIGN "Component Model" (tenant lifecycle coordinator), DESIGN
//! "Interactions & Sequences" (group-emptiness check — an absent parent is
//! success-equivalent), and PRD "Explicit Realm Binding" (input contract —
//! `mode = adopted` requires an empty parent tenant group).
//!
...
/// client's service account (PRD "Administrator Credentials" least-privilege table for the
...
/// Decoded `provisioning_metadata` input (PRD "Explicit Realm Binding" wire shape).
#[domain_model]
#[derive(Debug)]
struct ParsedInput {
    realm_binding: RealmBinding,
    realm_name: String,
    /// Optional Keycloak user UUID for admin-bind bootstrap (the admin-bind scope note §A).
...
/// Wire shape of `provisioning_metadata` (PRD "Explicit Realm Binding" — `mode`/`realm_name`).
///
...
struct WireMetadata {
    /// Realm-binding mode. Optional: absent ⇒ `shared`. The
    ...
    ///
    /// `""` (empty string) also deserialises as `None`. Reason: a deployment
    /// that wires this field through `${VAR}` expansion keeps the bind step
    /// default-on; where the env var is unset → POSIX default-empty expansion leaves an empty string,
...
/// Compiled form of the PRD "Explicit Realm Binding" realm-name regex. Initialised once via
...
// `ambiguous_from_kc_error` and `PROVISIONING_TENANT_ID_ATTR` live in
// `crate::domain::kc_provisioning` — shared with the service-account facade,
// which must not depend on this module (see that module's header).
...
/// [`PluginError::DeprovisionRetryable`] per DESIGN "API Contracts" — transport /
...
/// Classify failures from the service-account purge step when it runs as
...
fn deprovision_translate_sa_purge(e: PluginError) -> PluginError {
    match e {
        PluginError::KcRest {
            method,
            path_template,
            ...
            detail: format!(
                "service-account purge: kc:{method} {path_template} -> {status}: {body_first_2kb}"
            ),
        },
        PluginError::KcRest {
            method,
        ...
        other @ PluginError::AmbiguousCreated { .. } => PluginError::DeprovisionRetryable {
            detail: format!("service-account purge ambiguous: {other}"),
        },
        // No explicit `SaNotFound` arm: `purge_tenant_accounts` swallows
        // per-client 404s internally, so it never surfaces here. The
        // catch-all maps any hypothetical future `SaNotFound` to
        ...
        other => PluginError::DeprovisionTerminal {
            detail: format!("service-account purge: {other}"),
        },
    }
}
...
pub struct TenantFacade {
    ...
    /// Used by Created mode (DESIGN "Component Model" / "Security and Data Protection") to persist the per-realm
    ...
    // Segregated metric ports (DESIGN "Observability"). One `Arc` per subdomain so a
    ...
    /// Mandatory cascade hook for deleting every machine credential before
    /// tenant teardown. Constructor-required wiring makes omission a compile-
    /// time error rather than a silent runtime skip.
    ///
    /// Held as `dyn TenantAccountPurgeHook` rather than the concrete
    /// `ServiceAccountFacade`: that keeps this module free of a dependency on
    /// `service_account_facade` and lets tenant-lifecycle tests inject a spy.
    sa_purge_hook: Arc<dyn TenantAccountPurgeHook>,
}
...
impl TenantFacade {
    ...
    /// `cfg.provision_timeout_ms` (DESIGN "Interactions & Sequences"). A timeout returns
        ...
        match tokio::time::timeout(timeout, self.provision_tenant_saga(ctx, req)).await {
            ...
            Err(_elapsed) => {
                ...
                let err = PluginError::AmbiguousCreated {
                    ...
                    detail: format!(
                        "provision_tenant timed out after {}ms (provision_timeout_ms)",
    ...
    /// wrapper can apply `provision_timeout_ms` (DESIGN "Interactions & Sequences").
    // NOT cancel-safe: the timeout wrapper may drop this future after a KC or
    // Credential Store mutation has landed but before its response is observed;
    // the wrapper must therefore classify cancellation as AmbiguousCreated.
    async fn provision_tenant_saga(
        ...
        // Pre-saga `health_probe` (DESIGN "Component Model") — a flat-out KC outage
    ...
    /// parent tenant group becomes empty (DESIGN "Interactions & Sequences", "API Contracts", "Audit and Metrics Boundary").
    ///
    /// **Missing-metadata branch** (DESIGN "Interactions & Sequences" / "Audit and Metrics Boundary" / "Observability"): when
    ...
    /// (DESIGN "Observability" trigger metric; wired to `OTel`).
    ///
    /// # Errors
    ///
    ...
    pub async fn deprovision_tenant_inner(
        ...
        // otherwise hang the AM caller indefinitely; per DESIGN "Interactions & Sequences" the
        ...
        match tokio::time::timeout(timeout, self.deprovision_tenant_saga(ctx, req)).await {
            ...
            Err(_elapsed) => {
                ...
                let err = PluginError::DeprovisionRetryable {
                    detail: format!(
                        "deprovision_tenant timed out after {}ms (provision_timeout_ms)",
                        self.cfg.provision_timeout_ms
                    ),
                };
                self.failure_metrics
    ...
    /// outer wrapper can apply the per-call timeout (DESIGN "Interactions & Sequences").
    // NOT cancel-safe: the timeout wrapper may drop this future after a delete
    // has landed but before confirmation or later teardown steps complete;
    // retry safety depends on every delete path remaining idempotent.
    async fn deprovision_tenant_saga(
        ...
        // ---- Missing-metadata branch (DESIGN "Interactions & Sequences" / "Audit and Metrics Boundary"). ----
        let Some(blob) = req.tenant_context.metadata.as_ref() else {
            if ctx.subject_type() == Some("am.system") {
                // Increment the DESIGN "Observability" trigger metric; the audit event
        ...
        // Pre-saga `health_probe` (DESIGN "Component Model") — runs AFTER decode so
        ...
        match self
            .sa_purge_hook
            .purge_tenant_accounts(req.tenant_context.tenant_id)
            .await
        {
            Ok(deleted_count) => {
                if deleted_count > 0 {
                    audit::emit_service_accounts_purged(
                        ctx,
                        req.tenant_context.tenant_id,
                        deleted_count,
                    );
            ...
            Err(e) => {
                let err = deprovision_translate_sa_purge(e);
                self.failure_metrics
                    .failure(PluginOp::DeprovisionTenant, FailureVariant::from(&err));
                return Err(err);
            }
    ...
    async fn deprovision_after_decode(
        ...
        // (DESIGN "Domain Model" — Shared uses the env-backed inline secret
        ...
        // (DESIGN "Credentials, Token Cache, and Rotation"). `deprovision_translate_kc`
        ...
        let group_url = format!(
            ...
            encode_component(&metadata.realm_name),
        ...
        // translates verbatim (DESIGN "API Contracts"); other KcRest
        ...
        if has_remaining_tenants {
            // Realm still has live tenants — keep it (DESIGN "Interactions & Sequences").
            return Ok(false);
        }
    ...
    /// (operator runbooks key on the body text — DESIGN "Audit and Metrics Boundary").
    ...
    fn parse_input(req: &IdpProvisionTenantRequest) -> Result<ParsedInput, PluginError> {
        ...
        // (PRD "Explicit Realm Binding" — applies to inherited / generated / explicit alike).
    ...
    async fn provision_shared(
        ...
        // converges without operator-driven plugin restart (DESIGN "Credentials, Token Cache, and Rotation").
        self.kc_factory
        ...
        // Shared mode (DESIGN "Domain Model" / "Component Model" — Shared is the single
        ...
        // `admin_secret_ref = None` (DESIGN "Domain Model" — the env-backed admin
    ...
    /// Adopted-mode provisioning (DESIGN "Domain Model" Adopted row + PRD "Explicit Realm Binding" validation +
    /// "Interactions & Sequences"). The caller supplies an arbitrary realm
    ...
    /// either absent, or present with zero subgroups (per "Interactions & Sequences": "absent
    ...
    async fn provision_adopted(
        ...
        // probes parent tenant group emptiness (DESIGN "Interactions & Sequences" / "Security and Data Protection"
        ...
        // to expand — see the admin-bind scope note §A).
        Ok(TenantIdpMetadataV1 {
            version: "v1".into(),
            realm_name: parsed.realm_name.clone(),
    ...
    /// Created-mode provisioning (DESIGN "Interactions & Sequences" for the
    /// Created-row KC admin ops and the credential-custody saga). The plugin
    /// owns the realm lifecycle — it
    ...
    ///
    /// **Ambiguous failure tagging** (DESIGN "API Contracts"): every step
    /// after the validation probe is tagged with the corresponding
    /// [`AmbiguousStage`] so the operator reconciliation runbook (PRD "Risks" risk
    ...
    async fn provision_created(
        ...
        // DESIGN "Realm Binding and Tenant Provisioning", hardened per run am-functional-22: a
        ...
        // per-realm `realm-management` roles (PRD "Administrator Credentials" least-privilege
        ...
        // the reconciliation contract (PRD "Risks", orphaned-resource-after-lost-response row).
        let secret_ref_str = self
            .realm_admin_cfg
            .secret_ref_template
            .replace("{realm_name}", &parsed.realm_name);
    ...
    /// Idempotent realm-create step (DESIGN "Realm Binding and Tenant Provisioning", hardened per run
    ...
    /// * probe `Absent` → `POST /admin/realms`. On success → `Ok`. A staged
    ///   create ambiguity (409 unique-name from a duplicate POST, 5xx,
    ///   transport / timeout) is re-probed: if the realm now exists and is ours the create
    ///   landed despite the error → idempotent `Ok`; if foreign →
    ///   `CreatedRealmExists`; if still absent → `RealmCreateNotRetained`
    ///   (the 404 proves retry is safe); if the re-probe itself cannot answer →
    ///   preserve the original `AmbiguousCreated { stage: KcRealmCreate }` so
    ///   the operator reconciliation runbook still fires.
    ///
    ...
    async fn ensure_realm(
        ...
        // `create_realm` also performs local allowlist validation before it
        // sends the POST. Such a pre-mutation error needs no reconciliation
        // and must not be relabelled merely because the realm remains absent.
        if !matches!(
            &create_err,
            ...
            .await
        {
            ...
            // A definitive 404 proves the failed create retained no realm.
            // Surface a clean retryable category rather than sending the
            // operator through ambiguity reconciliation for known-absent state.
            Ok(RealmProbe::Absent) => Err(PluginError::RealmCreateNotRetained {
                realm: realm_name.to_owned(),
                detail: create_err.to_string(),
            }),
            // The probe itself could not answer, so retained state still
            // cannot be ruled out. Preserve the original staged ambiguity and
            // its POST status/body for the reconciliation runbook.
    ...
    /// `CleanFailure` per DESIGN "API Contracts" «`mode = shared` but realm does not
    /// exist»).
    ///
    /// **403 special case (DESIGN "API Contracts"):** this helper is ALWAYS
    ...
    async fn bind_admin_user_to_tenant(
        ...
        //       `user_facade::provision_user`, PRD "Out of Scope"
        ...
        attrs_obj.insert("user_type".to_owned(), serde_json::json!(["user"]));
    ...
    async fn ensure_tenant_group(
        ...
        if status == 409 {
            let lookup_url = format!(
                ...
                encode_component(realm_name),
                parent_id,
                encode_component(&tenant_id.to_string()),
            );
    ...
    /// "parent absent" is treated as success-equivalent (DESIGN "Interactions & Sequences").
    ///
    /// **403 special case (DESIGN "API Contracts"):** the Adopted emptiness
    ...
    /// landed in 26).
    ///
    /// **403 special case (DESIGN "API Contracts"):** only the Adopted-mode
