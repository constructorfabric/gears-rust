...
- **Realizes**: the service-principal product's name-policy requirement, which that contract leaves to the registered adapter.
...
- **Realizes**: the service-principal product's scope-allowlist and per-tenant-quota requirements, both of which that contract leaves to the registered adapter.
...
- **Service-Principal SDK**: `service-principal-sdk` (delivered by the service-principal module)
- **Service-Principal product (owning contract)**: the service-principal module's PRD and DESIGN
