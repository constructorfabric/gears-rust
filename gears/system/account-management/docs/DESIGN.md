...
The AM-owned contract is configured independently from the optional bootstrap saga and is reconciled during `init`. Schema drift, ownership conflicts, and an existing root whose durable type UUID differs are always startup-fatal; `bootstrap.strict` applies only to the later tenant-creation saga. During migration, `bootstrap.root_tenant_type` and `bootstrap.root_tenant_type_idp_provisioning` remain deprecated aliases and must agree with `root_tenant_type` when both forms are present.

After reconciliation, AM routes every read of the configured root type and its schema chain to authoritative persistent Types Registry storage. Unrelated deployment-owned entities temporarily continue through the process-local client until the general persistent-read cutover. Consequently, the platform-root schema is not duplicated under `types-registry.config.entities` in quickstart or E2E configuration; a persistent read failure fails closed instead of falling back to stale YAML data.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `root_tenant_type.gts_id` | string (chained `GtsSchemaId`) | Required when bootstrap is enabled | Canonical concrete schema identifier with exactly one concrete segment directly derived from `gts.cf.core.am.tenant_type.v1~`. AM creates it in authoritative Types Registry storage if absent and semantically validates it if present. Deployment-specific — e.g., `gts.cf.core.am.tenant_type.v1~cf.core.am.provider.v1~`. |
