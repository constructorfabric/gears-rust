...
1. [ ] - `p1` - Resolve the independently reconciled `root_tenant_type.gts_id` through AM's authoritative-root Types Registry view using DESIGN §3.1 effective-trait resolution; this bootstrap-owned root preflight does not call downstream barrier features because bootstrap is earlier in the feature DAG - `inst-algo-saga-type-check`
