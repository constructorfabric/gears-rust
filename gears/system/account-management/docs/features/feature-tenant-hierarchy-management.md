...
4. [ ] - `p2` - Run the 8 pure-Rust classifiers (`orphan`, `cycle`, `depth`, `self_row`, `strict_ancestor`, `extra_edge`, `root`, `barrier`) over the loaded `Snapshot`. Each returns a list of violations carrying the offending-row fields documented in DESIGN §3.2 - `inst-algo-integ-rust-classifier`
