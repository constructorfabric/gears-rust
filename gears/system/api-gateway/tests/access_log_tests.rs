...
/// the `OTel` context at request time and re-attaches it in `emit`. This asserts a
/// valid span context is current at emit time on that drop path — without the
/// re-attach it would be empty and the splice would produce nothing.
#[tokio::test]
async fn splices_trace_id_on_client_disconnect_drop_path() {
    ...
    // Records, per access_log event, the trace id of the OTel context current at
    // emit time — exactly what the log-correlation formatter reads to splice.
    #[derive(Clone, Default)]
    ...
    let tracer = provider.tracer("access-log-drop-test");
    ...
    // Issue the request inside a span so the middleware captures a valid OTel
    // context, then drop the response WITHOUT consuming its body — a client
    // disconnect. That fires `CountingBody::drop` -> `emit` on this thread,
    // where the request span is no longer active.
    let span = tracing::info_span!("drop_path_test");
    let response = test_app().oneshot(req).instrument(span).await.unwrap();
    drop(response);
    ...
    assert_eq!(
        ids[0].len(),
        32,
        "a valid 32-hex trace_id must be current when the access log is emitted \
         from the drop path: {:?}",
        ids[0]
    );
    assert!(
        ids[0].chars().all(|c| c.is_ascii_hexdigit()),
