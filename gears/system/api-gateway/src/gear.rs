...
impl ApiGateway {
    ...
    fn build_route_policy_from_specs(&self) -> Result<auth::GatewayRoutePolicy> {
        ...
        for spec in &self.openapi_registry.operation_specs {
            ...
            // anonymous (auth-skip). Visibility (`is_public`) is a *separate*
        ...
        let route_policy =
            auth::build_route_policy(&config, authenticated_routes, public_routes, proxy_registry)?;
    ...
    pub(crate) async fn serve(
        ...
        if cfg.gateway_proxy.enabled {
            self.start_proxy_sync(&cfg, cancel.clone()).await;
        }
    ...
    /// Connect to the `DirectoryService` and start the embedded-edge reverse-proxy
    /// directory-sync task. Connect failures are logged and non-fatal: the gateway
    /// keeps serving its native routes and simply proxies nothing until a later run.
    async fn start_proxy_sync(&self, cfg: &ApiGatewayConfig, cancel: CancellationToken) {
        if let Err(err) = self.try_start_proxy_sync(cfg, cancel).await {
            tracing::warn!(error = %err, "reverse proxy inactive: directory-sync did not start");
        }
    }

    /// Connect to the `DirectoryService` and spawn the directory-sync task.
    ///
    /// # Errors
    /// Returns an error if `directory_endpoint` is unset or the connection fails.
    async fn try_start_proxy_sync(
        &self,
        ...
        let client = if let Some(internal_auth) = &cfg.gateway_proxy.internal_auth {
            let interceptor =
        ...
        let directory: Arc<dyn DirectoryClient> = Arc::new(client);
        let interval = Duration::from_secs(cfg.gateway_proxy.sync_interval_secs);
        crate::proxy::spawn_directory_sync(
            Arc::clone(&self.proxy_registry),
            directory,
            interval,
            cancel,
        );
        tracing::info!(%endpoint, "reverse-proxy directory-sync started");
        Ok(())
    }
    ...
    fn mount_proxy_fallback(&self, router: Router) -> Result<Router> {
        let forwarder = toolkit_gateway::Forwarder::new(
            Arc::clone(&self.proxy_registry),
            toolkit_http::HttpClient::new()?,
        );
