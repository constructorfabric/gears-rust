...
|--------|-------------|--------------|-----------------|----------------------|
| `cpt-cf-token-issuer-nfr-no-key-custody` | No private key material in process | `SigningClientV1` port; `infra::plugin_select` | The gear only ever holds a `SigningKeyRef` (a validated name) and reads `PublicKeyVersion` PEMs. No signing primitive is called locally on the production path: JWKS publication parses public keys with `pkcs8`/`spki` (structural DER only, per ADR 0004), so no pure-Rust curve implementation reaches the shipped graph. | Mechanically checkable — `cargo tree -i p256 --workspace -e normal` returns nothing; `p256` appears only under `[dev-dependencies]`, where the test signer stands in for the plugin |
...
| Domain | Claim assembly, scope canonicalization and down-scoping, JWS assembly, JWKS construction, capability verification, both caches, loop guard, metrics | Pure Rust, `pkcs8`/`spki` (structural DER only), `jsonwebtoken`, `base64`, `sha2` |
