...
| `DELETE` | `/policy-engine/v1/bundles/{id}/versions/{version}` | Delete a draft version — the only deletion `cpt-cf-policy-engine-fr-lifecycle-states` permits. Requires the entity-tag precondition like every other mutating operation here; a version that is no longer draft is a conflict, and one the caller cannot see is not found | stable |
...
sequenceDiagram
    ...
    participant S as Snapshot registry
    participant D as Decision service
    O ->> M: list the bundle's assignments and their targets
    O ->> M: list assignments governing each affected tenant
...
| outcome | smallint | Evaluation result: permit, prohibit, superseded-by-deadline where the caller's deadline passed before a conclusion was reached, and deferral once that requirement ships |
