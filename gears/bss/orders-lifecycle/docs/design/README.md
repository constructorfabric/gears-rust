...
- [`04-versioning.md`](./04-versioning.md) — amendment from `submitted` / `pending_approval` / `approved`; new-version append with `supersedesVersion`; gate re-run and re-pin; historical version retrieval; the non-versioned audited administrative edit path; the absence of any amendment row from `in_fulfillment` onward (engine `not-admissible`; `04 §2.2` *Amendment stops at `in_fulfillment`*) (PRD §6.2).
