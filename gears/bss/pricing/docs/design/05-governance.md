...
**2026-09-10 read amendment (D-365):** approval list/detail add current `submitter`
and `approver` names through AM/IdP after approval-read authorization. AM also
requires its own user-list permission; Pricing grants no extra identity access.
Names are not stored or pinned. Mutation/audit identity remains UUID-only.
See [UI read contracts](./ui-read-contracts.md#approval-participant-names) for
nullable-name states, no-store responses, provider and principal-mapping limits.

**D-367:** `GET /bss-pricing/v1/approvals/counts` uses the same `approval × read`
gate with no target resource, retaining all resource-ID constraints in the SQL
aggregate. It returns `total/submitted/approved/rejected/voided` over the whole
visible set, not one page; withdraw counts as voided. No filters, pagination or
AM lookup. See [examples](./ui-read-contracts.md#approval-counts).
...
| `gts.cf.bss.pricing.config.v1~` | Tax-display policy + the region/brand/partner/orgTier taxonomies (the tenant config plane; partner/orgTier added by D-120) + the declared rounding and GL-code vocabularies (D-334, D-356) | `write`, `read` |
...
2. [ ] - `p1` - **FROM** submitted **TO** rejected **WHEN** the reviewer rejects with a mandatory reason → the Plan **stays** `draft` — it never left it, and the review state was the unit's throughout (**D-359**) — and the submitter is notified. For **non-plan subjects**, reject returns the change set to its slice-defined pre-submit state (cutover unit discarded, membership change not applied, batch not committed) — the owning slice defines the surface; this slice defines the record - `inst-as-reject`
...
|--------|------|---------|-------------|
| `GET` | `/bss-pricing/v1/approvals` | List pending/decided approvals (reviewer scope). The plan reads surface a pending unit's **id** so a client need not page and join here (**D-359**); the unit's own content — submitter, verdict, reason, pinned document — stays behind `approval × read` | — |
| `POST` | `/bss-pricing/v1/approvals/{id}/approve` | Approve (independent principal only). A `taxonomy_value` approval atomically applies its stored patch and taxonomy audit in the decision transaction; no second PATCH or config-write role is needed. Pin/guard/apply failure rolls back the verdict (**D-362**). Other subject workflows are unchanged | per decision |
