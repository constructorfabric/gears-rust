...
provenance, phased trial→interim→evergreen — such that an invalid or ambiguous shape **cannot
...
3. [ ] - `p1` - **Membership (D-356, 2026-09-09)**: when the tenant has declared a GL-code vocabulary (`GET/PUT /bss-pricing/v1/config/gl-codes`, §6 `pricing_gl_code_taxonomy`), a **present** `glCode` MUST be an `active` member of it or publish fails `GL_CODE_UNKNOWN` — **one finding per revision**, since the code is a single descriptor field and not a per-row value; an **empty** vocabulary constrains nothing (opt-in — declaring the first value turns the check on, so no existing plan fails on landing), and an absent or blank `glCode` stays step 1's `DESCRIPTOR_INCOMPLETE` rather than being reported twice. The rule is handed a set the publish resolves from `pricing_gl_code_taxonomy` and never learns who wrote it: the MVP provider is the tenant's `PUT`, the anticipated one an ERP gear populating or reconciling the same table (D-356 *Owed*). D-48's v1 field list is unchanged — this adds an optional membership check to one of its elements - `inst-ds-glcode`
...
| `kind` | `enum` | `trial \| interim \| evergreen` — the behavioural classifier and nothing else (**D-358**, 2026-09-09: `interim` was `intro`; it names the slot "any time-boxed non-trial phase", and its business meaning is the label below). The `CHECK` is strict — `intro` is refused at the store and `400` at the write — and frozen snapshots from before the rename keep `"kind": "intro"`, which nothing parses back |
| `display_name` | `text` | optional operator label (**D-357**): free-form, no uniqueness, no behaviour, not an identity and never an axis value; versions with the revision (D-83); framed by the approval content pin (`v16`); projected as presentation (`displayName`, `null` when absent), **not** rostered under `EVALUATION_POLICY_GENERATION`. A blank string is read as absent at the REST boundary |
...
no overlay — and written through its own route, `GET/PUT /bss-pricing/v1/config/gl-codes`
...
for Subscriptions runtime. A phase MAY carry an optional operator **`displayName`** (**D-357**): free-form, no behaviour, versioned with the revision, framed by the approval content pin and projected as presentation (`null` when absent); a blank label MUST read back as absent. The kind roster is **`trial` | `interim` | `evergreen`** (**D-358**): `intro` MUST be refused at the write (`400`, `phases.kind`) and at the store, while frozen snapshots taken before the rename keep `"kind": "intro"` and MUST NOT be rewritten.
...
- [ ] A phased plan trial→interim→evergreen publishes its phase map + `displayTrialDays`; a cyclic `convertsToPhaseId` fails
