...
**Date:** 2026-01-21
**Updated:** 2026-05-14 (renumbered ADR-2–7 → ADR-3–8 after ADR-0002 JSON-RPC/MCP was written; then ADR-3–8 → ADR-5–10 after ADR-0003 Workflow DSL and ADR-0004 Temporal Workflow Engine were written; then ADR-5–10 → ADR-6–11 after ADR-0005 Thin Host Gear, Fat Runtime Plugins was written); 2026-07-30 (section 3 added — gaps found during the consumer-SDK review); 2026-08-19 (G-07 – G-09 added — invocation-id assignment and the plugin → host event port, found during PR review of the consumer SDK)
...
| ID | Gap | Impact | Where |
|----|-----|--------|-------|
| G-01 | **No authorization error type.** The invocation contract enumerates five error types (`not_found`, `not_active`, `validation`, `quota_exceeded`, `sync_suspension`) and none of them means "not authorised", although authorisation is asserted throughout the design. Four further reachable conditions are also unenumerated: a control action that does not apply (whether because of the run's state or because the callable's kind does not support it), an invocation mode the callable does not offer (`supported_invocations` can exclude either mode, and `sync_suspension` covers only a run that already started), no plugin registered for a callable's adapter type, and plugin unavailability. | The consumer SDK declares `AccessDenied`, `UnsupportedMode`, `UnsupportedControl`, `NoPluginAvailable` and `ServiceUnavailable` variants with no host error type to map onto, so in-process and REST behaviour cannot yet be proven to agree for those five. | `DESIGN_GTS_SCHEMAS.md` error catalog; fold into ADR-11 and/or ADR-6 |
...
| G-07 | **Invocation-id assignment blocks idempotency, and nothing guarantees id uniqueness across plugins.** The invocation flow assigns the `invocation_id` at step 7 ("Plugin creates its `InvocationRecord` in `queued`, assigns an `invocation_id`") but applies `Idempotency-Key` dedup at step 2, five steps earlier. The host therefore has no identity to reserve the key against before dispatching. Separately, every plugin mints ids into one shared `invocation_index`, and no rule prevents two plugins producing the same one. | Dedup fails in exactly the case it exists for: two concurrent requests carrying the same key both pass step 2 before either reaches step 7, both dispatch, and the work runs twice — a caller retrying an unacknowledged charge is charged twice. A cross-plugin id collision would conflate two unrelated runs in the index. | **Recommended resolution — the host mints the `invocation_id`.** Assign it at step 2 and pass it to the plugin in the request; the plugin keeps its own backend handle (a Temporal workflow id, say) and correlates the two. Dedup then reserves `key → id` atomically before dispatch, and one minter makes ids unique by construction. This also closes G-08's read-after-invoke race (the host can create the index row at dispatch in `queued`) and most of G-09 (the host supplies `id`, `function_id`, `adapter`, `tenant` and `owner`, leaving the port only `status`, `timestamps` and `error`). It requires an id field on the invocation request, a change to step 7, and retiring the invariant "the host never writes to the index directly" — already inaccurate, since tenant and owner are host-derived. The accurate form is "the host never invents execution state". | `DESIGN.md` §3.6 steps 2 / 5 / 7; §Invocation index |
| G-08 | **Nothing orders the first status notification before `invoke` returns.** The index is "populated only from plugin-emitted events", and on the async path the plugin returns the `invocation_id` immediately (§3.6 step 8) while the row is created by the notification it emits at step 7. Emission is not commitment, and no rule requires the row to exist before the call returns. | A caller that starts a run in the background and immediately reads it can be told the run does not exist — indistinguishable from a wrong id. The consumer SDK's `usecase-start-async` postcondition ("the caller can retrieve it by identifier") is not guaranteed by the defined protocol. | Closed by G-07's recommended resolution: if the host mints the id it can create the row at dispatch in `queued`, and the notification updates rather than creates it. | `DESIGN.md` §3.6 steps 7–8; §Invocation index |
| G-09 | **The plugin → host event port is unordered, non-idempotent, and cannot create the row it is required to create.** `publish_invocation_status` carries no sequence number, timestamp or event id, and returns a `Result` — so a plugin whose call times out will resend. Nothing deduplicates a replay or rejects a stale one. Separately, the index invariant requires `id`, `function_id`, `adapter`, `status`, `timestamps` and `error summary` to originate in plugin-emitted events, but the port's signature carries only the id, status and error. No reconciliation or resync path exists anywhere in the design. | A replayed or reordered notification can move a run backwards from `Succeeded` to `Running`, and consumers read that regression as real state. A single lost notification leaves the index permanently wrong — a run shows `Running` hours after it finished — with no mechanism to repair it. And as written the row cannot be created at all, since three required columns have no source. | Partly closed by G-07: with host-minted ids the host supplies `id`, `function_id`, `adapter`, `tenant` and `owner`, leaving the port `status`, `timestamps` and `error` — so timestamps still need adding. Ordering, idempotency and repair remain open and need deciding together: at minimum a monotonic sequence per invocation so stale updates can be dropped, and a way to resync a run from the plugin when the index is suspected stale. | `DESIGN_RUST_TYPES.md` `InvocationIndexPort`; `DESIGN.md` §Invocation index |
