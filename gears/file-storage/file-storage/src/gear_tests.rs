...
fn gear_provides_p1_and_p2_migrations() {
    ...
    //      plus index hardening: idempotency_keys(file_id) covering the
    //      cascade from files, multipart_uploads(state, expires_at,
    //      lease_until) covering the sweep's full `list_expired` OR
    //      predicate, and files(created_at, file_id) WHERE content_id IS NULL
    //      covering the versionless-orphan-file cleanup sweep
