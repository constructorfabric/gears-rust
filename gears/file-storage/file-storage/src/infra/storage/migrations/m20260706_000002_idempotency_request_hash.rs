//! Bind idempotency keys to a canonicalized hash of the request body.
//!
//! Without a fingerprint of the request that created it,
//! `FileService::create_file`'s replay path returns the stored ticket
//! unconditionally once a live record is found for the composite key
...
//! Conflict`, alongside (not instead of) the `subject_id` check added by
//! `m20260706_000001_idempotency_subject_id`.
//!
