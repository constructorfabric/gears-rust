...
If the referenced file is deleted while an apply is in flight, the subscriber cannot self-react and its await-record follows the ordinary un-acknowledged path — it stays `awaiting` and the `apply_notification` is re-delivered on the usual interval, because the tracker never moves a record to a terminal state on a timer (§4.2 *Apply Outcome Tracker*). This is the same behaviour as for any consumer that cannot apply. Separately, the `file.deleted` event flags the value `needs_review` and keeps it out of later applies (DESIGN.md §4.4 *Events Consumed*).
