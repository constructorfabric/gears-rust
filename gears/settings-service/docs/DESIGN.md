...
Identity and shape are separate types, both the gear's. A setting `key` derives from `gts.cf.core.settings.setting_type.v1~`; its derived half is authored by the deploying party (module or admin), whose vendor is not necessarily `cf`. The value's shape is a curated **value type** from the catalogue under `gts.cf.core.settings.type_*~`, named by `value_type_id`. The catalogue sits in the gear's namespace because the gear is what defines and registers it, from `settings-service-sdk`: no toolkit library ships these, and an identifier naming one would point at an owner that does not exist.
...
| `value_type_id` | string | Yes | GTS id of the **value type** the value is validated against — a curated type from the catalog `gts.cf.core.settings.type_*~`, **registered** in GTS. Independent of `key` (§4.7). Carries the trait set (incl. `secret`). |
...
| Secret boundary unchanged — **revive only** | `409 SecretnessChanged` | A re-declaration at a retired key MUST NOT name a type that flips `has_secret_trait`: secret and non-secret values are stored differently. Checked before the value type, since crossing the boundary is always a retype too (§4.3 *Re-declare to revive*) |
| Value type unchanged — **revive only** | `409 ValueTypeChanged` | A re-declaration at a retired key MUST name the stored `value_type_id`: the setting's own GTS type is registered with it and the Types Registry does not replace a registered type; a retype rides the new-major path (§4.3 *Re-declare to revive*) |
...
| `get_effective_bulk` | `keys[] \| category`, `scope` | `Result<EffectiveValueResponse>[]` | Batched read sharing one ancestry walk. **Per-key outcomes:** each element is `Ok` or `Err(Unavailable \| Retired \| NotFound)` for that key — never all-or-nothing. Bounded at `BULK_LIMIT` (500) keys: a longer list, or a category that expands past it, is answered per key with `bulk_too_large`, never partially. A category whose enumeration fails — bad id, store unreachable, a stored key that does not parse — is answered with an empty batch and logged at `warn`, never shortened by the rows it could not name. |
