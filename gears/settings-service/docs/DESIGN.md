...
| Owner unchanged — **revive and evolve** | `409 ContributedDeclarationImmutable` | A `module_contributed` declaration changes only through its owning module: an administrative re-declaration refuses to revive a retired one, as it refuses to evolve an active one. The `settings` package an admin key is composed with is not reserved from modules, so a retired contribution can sit at exactly the composed key; the reconciler refuses the mirror case, a row it does not own (§4.2 *Contribution Reconciler*). Checked before the other revive refusals, since it decides who may change the row at all |
