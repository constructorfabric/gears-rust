...
Both rows exist, matched by the version-stripped path `cf.toolkit.settings.cat.sett1`, but only the successor is **active**: the `…sett1.v1` declaration is **retired** by the migration and every `bool` override under it is **retained** with it (read-only), which is what frees the leaf name for `…sett1.v2` (§4.7). The `…sett1.v2` declaration is created; each old `bool` override is **copied** and re-validated against `string.v1~` — those that coerce cleanly become `string` overrides on the new key, those that do not are flagged `needs_review`. `…v2`'s predecessor is found by query (same stripped path, highest major below 2) — nothing is stored. (Per GTS grammar the derived half carries no `gts.` prefix — that prefix appears once at the head of the chain.)
