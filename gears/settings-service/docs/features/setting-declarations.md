...
12. [x] - `p1` - Validate `default_value` against `value_type_id` through the Type Validator trait — a secret's placeholder included, so `""` passes for a string-shaped secret type and `null`, `[]` or `{}` do not, exactly as on the contribution path - `inst-decl-create-12`
