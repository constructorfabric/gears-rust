...
| **File-Valued Settings**                       | `p2`         | A setting value may be a file, held **by reference** to the platform's `file-storage` gear; bytes are never stored in this service. Always pinned to an immutable version. Validated on write (exists, declared content-type/size constraints, caller authorized); Apply re-resolves and refuses a broken reference; a reference reported deleted is flagged `needs-review`. Scope/cascade, staging → Apply, visibility flags, category access and audit unchanged; content never indexed by search. |
...
| **Category Access Restriction**                | `p2`         | Access separable per category: reachable by default, and a platform administrator may mark a category restricted so that reaching its settings also requires being granted it. Grants themselves are assigned outside this service. |
...
A file-valued setting **MUST** behave like any other setting everywhere else — scope and cascade, staged change and Apply, `tenant-visible` / `tenant-overridable`, category access ([§5.7](#57-security-secrets--audit)), and audit. A tenant **MUST** be able to override a platform-level file exactly as it overrides any other value, so per-tenant branding is an ordinary override and not a separate mechanism.

Writing the reference **MUST** be validated: the referenced file **MUST** exist and the caller **MUST** be authorized to use it at write time. Authorization over the file itself **MUST** remain with the file storage — this service **MUST NOT** keep a second copy of file permissions. A declaration **MUST** be able to constrain what its reference may point at — permitted content types and a maximum size — and staging **MUST** fail closed when the referenced file violates a stated constraint. Constraints on the file's **content** are out of scope: the service never reads the bytes, so it can assert what the file storage's own metadata reports and nothing finer. A read **MUST** return the reference itself; fetching the content is then the caller's own business with the file storage, and the content **MUST NOT** travel through this service on any path. The service **MUST NOT** be required to issue, proxy, or cache a download URL. File content **MUST NOT** be indexed by search.

A reference **MUST** name an **immutable version** of the file, never a mutable pointer to "whatever that file currently holds". Otherwise the content behind an applied setting can be replaced with no staged change, no Apply, no audit record and no activation signal — configuration would change for every consumer while the setting stood still, and the activation guarantee ([§5.5](#55-staged-change--apply)) would stop meaning anything. Showing consumers different content **MUST** therefore be an ordinary staged change that repoints the setting.
...
An Apply **MUST** re-resolve every reference in the set and **MUST** refuse to apply one that no longer resolves, so a broken reference never becomes effective. A reference whose target is reported deleted **MUST** be handled like any other value that no longer validates — flagged `needs-review`, excluded from Apply, and surfaced to an administrator — never served as a valid value and never turned into a hard read failure.

The service **MUST NOT** be required to detect that a referenced *version* disappeared under an already-applied value. The file storage publishes no version-deleted signal, and only the file's current version is protected from deletion, so the version a settled setting points at is the one most freely removed; there the failure surfaces at the consumer's fetch. Closing this needs either a version-deleted signal or a hold on referenced versions **from the file storage**, which alone can count the references such a hold would have to respect.

> **Note**: The platform already owns the gear this requires — `file-storage`, which separates a control plane (metadata and signed-URL issuance) from a data plane that alone moves bytes, so content never transits the Settings Service. Its inter-gear client (`FileStorageClientV1`) currently exposes only `module_name`, so the typed call this FR assumes must be added there first. The dependency is on that gear's roadmap; no new storage is introduced in this service.
...
- [ ] `p2` - **ID**: `cpt-cf-settings-service-fr-category-access`

A right to read or change setting values covers every setting the caller can otherwise see, so "this role manages the billing settings, that one the infrastructure settings" cannot be expressed. The system **MUST** allow access to be separated by **category**, which is the unit a setting already belongs to exactly once.

A category **MUST** be reachable by default: with nothing configured, a caller holding the ordinary settings rights reaches its settings, so a newly installed gear contributing settings into a new category needs no grant before anyone can administer it. A platform administrator **MUST** be able to mark a category **restricted**, after which reaching its settings **MUST** additionally require that the caller be granted that category; other categories **MUST** be unaffected. Marking and unmarking **MUST** be auditable like any other administrative change ([§5.7](#57-security-secrets--audit)).

Restriction **MUST** narrow only: it never grants what visibility, licence, or scope rules withhold, and an unrestricted category confers nothing by itself. The smallest unit that can be restricted is the category — a single setting inside one **MUST NOT** be separately restricted. Assigning the grants themselves is **out of scope** for this service: it holds no roles and provides no way to bind rights to them; it declares what can be granted and enforces the answer it is given ([§5.7](#57-security-secrets--audit) authorization gating).

- **Rationale**: Adopters need role separation over configuration — billing staff should not be reading infrastructure settings — and the only per-setting attributes available today, `tenant-visible` and licence gating, govern all administrators of a tenant alike. Defaulting a category to reachable is what keeps that from becoming a tax on every install: a gear's settings work the moment it lands, and an operator spends a grant only where separation is actually wanted.
- **Actors**: `cpt-cf-settings-service-actor-platform-admin`, `cpt-cf-settings-service-actor-tenant-admin`
