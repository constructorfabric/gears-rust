...
| `cpt-cf-credstore-fr-credential-record` | One item shape (ADR-0004): the `credentials` collection item and the point read carry metadata only by default; the secret value is a selectable `value` field of that same item, disclosed only under `$select` and `read_secret`, never a separate sub-resource (§4.1, §4.3) |
...
**Record without a value.** A `PATCH {"value": null}` against an existing, own `active` record, or a `PUT` whose `value` is an explicit `null` — on create, or replacing an existing `active` record — are the only ways to reach this state (§4.3, §6.1), reached only on purpose: `PUT`'s `value` is tri-state, and an absent `value` key is rejected as `VALUE_REQUIRED` before anything is written, so there is no accidental path to `declared` through a forgotten field. It is a **stored lifecycle state** — `status = 4`, `declared` — and not an inference from a missing fingerprint; §6.1 carries the reasoning and the predicate table.

The column-level invariant is `declared ⇔ value_id IS NULL` — a `declared` row's pointer is null, an `active` row's is not. `PATCH {"value": null}`, and a replacing `PUT` with an explicit `"value": null`, clear the pointer and, in the same transaction, enqueue the row's old `value_id` into `credstore_value_gc` with reason `removed`, for the periodic maintenance job to delete from the backend (§6.1, §6.4, gc table in §4.7); a `PUT` that creates the row `declared` directly touches neither the pointer nor the gc table, since there was never a backend entry to enqueue.
...
- It **is never produced by a crash**: creation is atomic in both forms — a value-carrying `PUT` lands a fully `active` row or none, through the write protocol of §6.2; a `PUT` with an explicit `null` value makes no backend call at all and is a single row insert, landing a fully `declared` row or none — so there is no in-flight state between "no record" and a resting record that could look like a crashed write. A crashed value-carrying create leaves either no row at all (crash before the row CAS) or a fully `active` row (crash after), plus a `pending` gc entry the periodic maintenance job reconciles either way; a `null`-value create has no backend call and mints no gc entry to crash between.
...
| `get_secret` | `(ctx, key) → Result<Option<Secret>, CredStoreError>` | Hierarchical read of the resolved **value** — the in-process equivalent of `GET /credentials/{ref}?$select=reference,type,expires_at,value`, without taking a field selector itself. A winning record with no value (`declared`, or suppressed) is the canonical miss. Requires `read_secret`. |
...
> Decision: [ADR-0004](./ADR/0004-cpt-cf-credstore-adr-secret-value-exposure.md) (`cpt-cf-credstore-adr-secret-value-exposure`, status: proposed). The entity address returns one item shape, the credential record, by default; `value` appears in that same representation only when `$select` names it, under `read_secret` — never a separate sub-resource. `{ref}` is the caller-chosen `SecretRef` — never the row UUID, which is a generation id kept out of every response body so the `ETag` remains the one CAS validator source.

| Address | PDP action | Success | Key headers | Preconditions |
|---|---|---|---|---|
| `GET /credstore/v1/credentials` | `list` for any record field selected, `read_secret` per distinct type when `$select` contains `value`, both when both | `200` | `Cache-Control: no-store` — the body varies by tenant and by subject; value mode is additionally audited per item | OData `$filter`/`$orderby` on an indexed allowlist (§4.7), `$select` on the `Credential` field allowlist plus `value` (unknown name → 400), opaque cursor, `limit`; no total count. `$select=…,value` switches to **value mode** (below): `limit`/`cursor` rejected (400), `$orderby` rejected (400), capped at `cap + 1` fetch with `400 TOO_MANY_MATCHES` on overflow, flat item list with no `next_cursor` |
...
**Create is one request.** `PUT /credstore/v1/credentials/{ref}` with `If-None-Match: *` creates the record; `value` is tri-state on that same body (table above): absent is rejected (400 `VALUE_REQUIRED`) before anything is written, a string creates the record and writes the value atomically through the value write protocol (§6.2) — record the write's intent, `plugin.put` the bytes under a freshly minted `value_id`, then one transaction inserts the `active` row already pointing at that `value_id`; a crash before the row insert leaves at most a `pending` gc entry over an orphan the periodic maintenance job collects, never a row — and an explicit `null` creates the record `declared` directly, with no backend call at all: a single row insert, nothing to crash between. `declared` (`status = 4`) is reached in exactly two ways, both deliberate: a `PATCH {"value": null}` against an existing record, or a `PUT` with an explicit `null` value at creation or at replace of an `active` record (§6.1) — never by omission, since an absent `value` key is rejected before either path runs.
...
This sequence is the value-mode path — `get_secret`, or `GET /credentials/{ref}?$select=…,value` — evaluating `read_secret` and finishing with the plugin round-trip. A plain `get` (or `GET /credentials/{ref}` with a `$select` that omits `value`) runs the identical resolution query and returns once the winning row is found: it evaluates `read` in place of `read_secret` and never reaches the plugin — the two calls share one query, diverging only in the action checked and whether the last two steps run at all.
...
**Declared.** A record reaches this state in exactly two ways, both deliberate: a `PATCH {"value": null}` against an existing, own `active` record, or a `PUT` whose `value` is an explicit `null` — at creation, or replacing an existing `active` record (§4.1, §4.3.1) — never by omission, since `PUT`'s `value` is tri-state and an absent key is rejected (`VALUE_REQUIRED`) before either path runs. It is a distinct, deliberately long-lived resting state. The property that identifies it is which table names the row's version: a `declared` row's own `value_id` column is null, while an in-flight write's target id lives only in `credstore_value_gc` (reason `pending`) and no row references it at all — so a `declared` row can never be mistaken for a crashed write.

**How a record reaches it.** Two paths touch an existing row, one transaction each, nothing best-effort before the commit; a third never touches a row at all. `PATCH {"value": null}`, and a replacing `PUT` with an explicit `"value": null`, are each `UPDATE row SET value_id = NULL, status = 4, value_fp = NULL, fp_key_id = NULL, version = version + 1, <metadata keys>` plus `INSERT gc(old_id, removed)` in the same transaction — row and gc bookkeeping commit or roll back together; no backend call happens before that commit. Only afterward does the gear best-effort delete the old backend entry and, on success, drop its gc row — the periodic maintenance job drains it either way (§6.4). A delete failure leaves the old version enqueued and unreachable, never overwritten (immutable entries are never overwritten in place; the next write mints its own `value_id`). A creating `PUT` with an explicit `"value": null` is simpler still: a plain `INSERT` of a `declared` row — no intent record, no `plugin.put`, no gc entry, since there was never a backend write to account for:
...
| A value-carrying create crashed between the intent insert and the row CAS | **no row at all** — only a `pending` gc entry, possibly an orphaned backend write | the maintenance job reclaims the orphan (`plugin.delete` + `DELETE gc`) once the pending entry is older than `gc.pending_max_age_secs`, or just drops the gc row if nothing was ever written; there is no row state resembling a crashed create — a `null`-value create has no backend call to crash mid-way through, and a value-carrying create's crash never leaves a row at all to confuse with `declared` |
...
**Listing and value-mode reason codes (ADR-0004, ADR-0005).** `GET /credentials`, in both metadata and value (`$select=value`) mode (§4.3.1), reuses the platform's standard OData/cursor-pagination reason codes (`guidelines/DNA/REST/PAGINATION.md`) rather than defining its own:
...
| `VALUE_REQUIRED` | `InvalidArgument` | 400 | `PUT` with no `value` key in the body at all — `value` is tri-state, and the absent key is the only shape rejected; an explicit `null` is accepted and creates or leaves the record without a value (§4.3.1, §6.1) |
