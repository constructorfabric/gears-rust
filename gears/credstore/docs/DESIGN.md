...
- It **does not resolve** for a value read — a `GET .../secret` on it is the ordinary not-found. This needs no new predicate: resolution already selects `status = 2`, so a `declared` row is excluded by the filter that is already there.
...
It is also distinct from the existing `value_fp IS NULL` out-of-band-seeding case (§4.10): that row already has a value in the backend and is only missing its fingerprint, served on trust until backfilled; a `declared` row has no backend value at all. Neither state resolves for a value read, and neither hides a `shared` value inherited from an ancestor under the same reference (§4.1).
