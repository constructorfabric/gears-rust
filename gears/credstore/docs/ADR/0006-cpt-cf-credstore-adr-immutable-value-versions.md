...
There is no resident process watching this table (D7). A **periodic maintenance job** — entered in-process through the SDK trait `CredStoreMaintenanceV1::run_gc` (registered in `ClientHub`, no REST endpoint) by the host, a `gc` subcommand of the application binary run by a Kubernetes CronJob or a scheduler gear, on an operator-chosen cadence, daily by default, weekly acceptable — runs two independent passes against it in bounded batches, until nothing is left:
...
- Contract: no timer task exists in the gear process — hygiene runs only when `CredStoreMaintenanceV1::run_gc` is invoked by the host, on a schedule or by an operator, never on an in-process interval; the gear exposes no REST route for it.
...
| Reaper | no — withdrawn | The in-process reaper loop — its resident sweep, its `reaper.*` config (`tick_secs`, `provisioning_timeout_secs`, `deprovisioning_timeout_secs`) and its metrics (`provisioning_rollback`, `provisioning_reaped`, `deprovisioning_reaped`) — is removed outright, not renamed. A periodic maintenance job replaces it: the SDK trait `CredStoreMaintenanceV1::run_gc`, called by the host on an operator-chosen schedule, taking `gc.pending_max_age_secs` (default 3600) and `gc.batch_size` (default 256), and emitting `gc_deleted`, `gc_pending_reclaimed`, `expired_deleted` plus a run outcome/duration. |
...
- The in-process reaper is withdrawn: its resident sweep, its `reaper.*` config (`tick_secs`, `provisioning_timeout_secs`, `deprovisioning_timeout_secs`) and its metrics (`provisioning_rollback`, `provisioning_reaped`, `deprovisioning_reaped`) are removed outright, not renamed. A periodic maintenance job replaces it — the SDK trait `CredStoreMaintenanceV1::run_gc`, configured by `gc.pending_max_age_secs` and `gc.batch_size` and invoked by the host on an operator-chosen schedule (a `gc` subcommand of the application binary under a Kubernetes CronJob, or a scheduler gear), out of this ADR's scope; dashboards and alerts built on the old reaper names are retired at rollout, not migrated.
...
A deployment that already holds credentials is not covered by the greenfield assumption this ADR was drafted under, and the gap is not cosmetic. The backend key shape changes from `tenant/reference/class` to `tenant/value_id`, so no value written before this decision is reachable by the new contract: the plugin has no method left that can name the old address. Such a deployment has to move its values across before it can serve on this ADR's model.

That move is not the gear's work, and deliberately so. `m0002` cannot perform it — a migration is handed a database connection and nothing else, so it can never read the backend (`DatabaseCapability::migrations` takes no context, and the `db` phase runs before any plugin registers itself in `init`) — but the constraint only binds code living inside the gear. A snapshot of `credstore_secrets` taken before `m0002` runs carries every old address and every fingerprint the migration is about to null, and taking it needs nothing but database access and the backend's own API, which is where the old key format is known anyway. So `m0002` stays an ordinary, irreversible migration that leaves each row `declared`, and a script external to the gear moves the fence key, carries each value to a freshly minted `value_id`, and promotes the rows.

Three properties of this ADR have to survive that trip, because each of them fails quietly rather than loudly when it does not. The fence key moves first, or the next boot mints a new one and every fingerprint stops verifying — the values arrive and read as 404. Each value is checked against its recorded fingerprint before it is carried, or the fence ADR-0003 built is spent: a poisoned entry gets copied under a fresh `value_id` and becomes indistinguishable from a legitimate version. And a row whose value could not be carried is left `fallback = none`, or resolution walks past it and serves an ancestor's secret in place of an honest 404.

The runbook and the prompt that generates the scripts are in [`docs/migration/value-migration.md`](../migration/value-migration.md), which carries the conditions for its own deletion.
