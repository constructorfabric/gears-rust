...
7. The concrete secret type is authorized per distinct type the first step found (`list`), and a denied type's references never reach the second step. Because the type is invariant across a chain, the types found in step one are the winners' types, so this is a clamp rather than a post-query drop.
