...
value and its metadata came from one writer. The fence key is auto-generated and stored in the backend under a reserved, API-unreachable entry (split knowledge: fingerprints in the DB, key with the values). The `ETag` becomes the generation-bound pair `"<row-id>.<version>"`, closing the ABA. This is the application-level fencing cluster ADR-002 prescribes for an "external storage layer with fencing support" — here the gear supplies that support on the backend's behalf. See §4.10 of DESIGN.

### Consequences

* A crosswise LWW interleave, a half-completed overwrite, or a lost/replaced fence key can only produce a fingerprint mismatch → 404, healed by any subsequent successful PUT. No state serves a value under mismatched metadata.
* LWW is no longer strict last-writer-wins under a same-value/different-sharing race: the loser's intent can be lost. Not a disclosure — a served value's sharing label was set by the writer of *that* value (the atomic co-write), so no value is exposed under a sharing nobody assigned it.
