...
**Constraints**:
- UNIQUE on `(tenant_id, source, external_id)`
...
- CHECK `source != 'hub' OR (enabled = FALSE AND auto_attach = FALSE)` — hub-discovered servers MUST land with `enabled=false` and `auto_attach=false`
...
TerminalOutcome::ToolUse { tool_use_id, name, input } => {
    if name == "search_knowledge" && knowledge_search.is_some() {
        ...
        // MCP tool dispatch
        if let Err(e) = validate_arguments(&input, &route.schema_hash) {
...
3. Apply server visibility policy (`allowed_tenants`, `allowed_roles`, `auto_attach`)
...
| `mcp.tool_cache_ttl_secs` | `integer` | `300` | **ConfigMap** | In-memory tool list cache TTL |
