...
The system MUST expose REST endpoints for listing available servers (`GET /mini-chat/v1/mcp-servers`), retrieving server details (`GET /mini-chat/v1/mcp-servers/{id}`), and listing cached tools per server (`GET /mini-chat/v1/mcp-servers/{id}/tools`). An admin/operator or controlled-role endpoint (`POST /mini-chat/v1/mcp-servers/{id}/tools:refresh`) MUST allow explicit tool metadata refresh. All list endpoints MUST support cursor-based pagination following the existing mini-chat pagination pattern.
...
The system MUST expose: `POST /mini-chat/v1/admin/roles/{role}/mcp-servers` (assign server to role), `DELETE /mini-chat/v1/admin/roles/{role}/mcp-servers/{sid}` (revoke), and `GET /mini-chat/v1/admin/roles/{role}/mcp-servers` (list role's servers, paginated). These endpoints are admin-only. An explanatory endpoint `GET /mini-chat/v1/chats/{id}/mcp-tools/effective` MUST return the effective MCP servers/tools and omission diagnostics for a chat (based on the caller's roles).
...
|--------|----------|-------------|
| GET | `/mini-chat/v1/mcp-servers` | List available MCP servers for the tenant (paginated) |
| GET | `/mini-chat/v1/mcp-servers/{id}` | Get MCP server details |
| GET | `/mini-chat/v1/mcp-servers/{id}/tools` | List tools exposed by a server (cached/persisted metadata) |
| POST | `/mini-chat/v1/admin/roles/{role}/mcp-servers` | Assign MCP server to a role (admin-only) |
| DELETE | `/mini-chat/v1/admin/roles/{role}/mcp-servers/{sid}` | Revoke MCP server from a role (admin-only) |
| GET | `/mini-chat/v1/admin/roles/{role}/mcp-servers` | List MCP servers assigned to a role (admin-only, paginated) |
| GET | `/mini-chat/v1/chats/{id}/mcp-tools/effective` | Explain effective MCP servers/tools and omissions for a chat (based on caller's roles) |
| POST | `/mini-chat/v1/mcp-servers/{id}/tools:refresh` | Refresh tool metadata (admin/operator or controlled role) |
...
**Main Flow**:
1. Admin lists available MCP servers via `GET /mini-chat/v1/mcp-servers`
2. Admin assigns a server to a role via `POST /mini-chat/v1/admin/roles/{role}/mcp-servers` with `server_id`
...
**Main Flow**:
1. Admin lists role's servers via `GET /mini-chat/v1/admin/roles/{role}/mcp-servers`
2. Admin revokes a server from the role via `DELETE /mini-chat/v1/admin/roles/{role}/mcp-servers/{sid}`
...
- [ ] Administrators can assign MCP servers to user roles via `POST /mini-chat/v1/admin/roles/{role}/mcp-servers` and revoke via `DELETE /mini-chat/v1/admin/roles/{role}/mcp-servers/{sid}`; only enabled servers visible to the tenant can be assigned
...
- [ ] `Mcp-Protocol-Version` and `Mcp-Session-Id` headers configured in OAGW upstream header passthrough allowlist
...
- OAGW supports header passthrough configuration for MCP-specific headers (`Mcp-Protocol-Version`, `Mcp-Session-Id`)
...
- MCP tool cache TTL: 300 seconds (deployment config: `mcp.tool_cache_ttl_secs: 300`)
