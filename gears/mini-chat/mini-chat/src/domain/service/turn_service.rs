...
> TurnService<TR, MR, CR, MAR>
{
    ...
    async fn mutate_for_stream(
    ...
    ) -> Result<(MutationResult, FlushHandle), MutationError> {
        ...
        let (user_content, snapshot_boundary, chat_model, web_search_enabled, pending) = self
            .db
            .transaction(|tx| {
                Box::pin(async move {
                    ...
                    Ok((user_content, boundary, chat_model, web_search_enabled, pending))
                })
            })
            .await
            .map_err(unwrap_mutation_err)?;
