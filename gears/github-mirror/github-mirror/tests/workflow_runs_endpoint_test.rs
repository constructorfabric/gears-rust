...
use github_mirror::domain::repo::{RepositoryRecord, WorkflowRunRecord};
...
async fn workflow_runs_are_listed_newest_first() {
    ...
    let response = get(router, "/github-mirror/v1/repos/acme/widget/actions/runs").await;
...
async fn workflow_runs_of_unknown_repository_return_404() {
    ...
    let router = router_for(service, ctx);
    let response = get(router, "/github-mirror/v1/repos/acme/nope/actions/runs").await;
