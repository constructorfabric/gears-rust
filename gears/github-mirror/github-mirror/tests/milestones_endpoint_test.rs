...
use github_mirror::domain::repo::{MilestoneRecord, RepositoryRecord};
...
async fn milestones_are_listed_by_number() {
    ...
    let response = get(router, "/github-mirror/v1/repos/acme/widget/milestones").await;
...
async fn milestones_of_unknown_repository_return_404() {
    ...
    let router = router_for(service, ctx);
    let response = get(router, "/github-mirror/v1/repos/acme/nope/milestones").await;
