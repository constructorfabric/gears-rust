    ...
    async def test_default_type_is_generic_and_echoed_in_metadata(
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            resp = await client.put(
                ...
                headers=tenant_a_headers,
                json={"value": "sk-123", "sharing": "tenant", "type": API_KEY},
            )
            assert resp.status_code == 204, resp.text
    ...
    async def test_type_is_immutable(
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            await client.put(
                f"{secrets_url}/{ref}",
                headers=tenant_a_headers,
                json={"value": "sk-1", "sharing": "tenant", "type": API_KEY},
            )
            resp = await client.put(
                f"{secrets_url}/{ref}",
                headers=tenant_a_headers,
    ...
    async def test_personal_token_is_private_only(
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            resp = await client.put(
                f"{secrets_url}/{ref}",
                headers=tenant_a_headers,
                json={"value": "tok", "sharing": "private", "type": PERSONAL_TOKEN},
            )
            assert resp.status_code == 204, resp.text
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            for bad in ('{"client_id": "cid-only"}', "not-json-at-all"):
                resp = await client.put(
                    ...
                    headers=tenant_a_headers,
                    json={"value": bad, "sharing": "tenant", "type": OAUTH2_CLIENT},
                )
                assert resp.status_code == 400, resp.text
