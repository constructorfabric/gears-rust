...
mod tests {
    ...
    #[test]
    fn new_constructs_service() {
        let _svc = Service::new(empty_hub(), "hyperspot".into());
    }
    ...
    #[tokio::test]
    async fn get_plugin_returns_client_when_registered() {
        let instance_id = test_instance_id();
        let hub =
            hub_with_registry_and_plugin(&instance_id, "hyperspot", MockPlugin::returns(None));

        let svc = Service::new(hub, "hyperspot".into());
        // Should succeed — client is registered
        let _client = svc.get_plugin().await.unwrap();
    }
