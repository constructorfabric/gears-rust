...
This DECOMPOSITION covers the Tenant Resolver Plugin (`tr-plugin`) sub-system, a read-only, in-process query facade that runs inside the parent `account-management` crate and serves barrier-aware hierarchy reads over the parent module's canonical `(tenants, tenant_closure)` storage. It is a separate SDLC artifact from the parent [account-management DECOMPOSITION](../DECOMPOSITION.md) because the `cf-tr-plugin` namespace is a distinct Constructor Studio sub-system (see `.cf-studio/config/artifacts.toml` children.autodetect block): the tr-plugin owns its own PRD, DESIGN, ADR, and schema under `modules/system/account-management/docs/tr-plugin/`, and per the registry contract must define its own feature IDs in its own DECOMPOSITION file rather than in the parent's.
