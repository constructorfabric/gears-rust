...
The Event Broker is a composite, tenant-scoped event streaming module for Cyber Ware. It provides an append-only, GTS-typed event log with monotonic per-`(topic, partition)` sequences, enabling decoupled inter-module communication and reliable replay.
...
The **producer client library** (`cyberware-event-broker-sdk`) is built on top of `modkit-db`'s transactional outbox. Producers enqueue events within their business transaction via the outbox; the outbox pipeline (sequencer → processor) then delivers them to the ingest service. This gives producers **transactional guarantees** — events are only published if the business transaction commits — and **exactly-once semantics** via the idempotent producer protocol. The same outbox machinery also powers the ingest service itself: ingested events flow through a per-topic outbox partition before being persisted to the storage backend and broadcast to delivery.
...
| Long-poll consumption | `cpt-cf-evbk-fr-long-poll` | Event-driven notification with configurable timeout |
...
| Multi-tenant isolation | Cyber Ware platform | All data tenant-scoped via SecureConn |
| Flexible deployment topology | NFR: deployment modes | Standalone (all-in-one) and cluster (ingest + delivery + dispatcher) |
| ModKit integration | Cyber Ware platform | Single-executable deployment, trait-based DI, secure ORM |
...
- `cpt-cf-evbk-design-sequence-assignment` — Three-sequence model: producer chain (`previous`, `sequence`) for ingest-side dedup (chained / monotonic / stateless modes); outbox sequence (modkit-db) for in-pipeline order preservation; offset (backend) for consumer-visible ordering
...
- future ADR (outbox-ingest) — Outbox-based producer library and ingest pipeline built on `modkit-db` outbox
...
- `cpt-cf-evbk-adr-partition-selection` — Partition selection algorithm: MurmurHash3-32 over `partition_key` with producer override and broker-side range validation. See [`ADR/0001-partition-selection.md`](ADR/0001-partition-selection.md).
...
| `ClusterCapabilities` (modkit) | Cluster coordination: pub/sub, leader election, distributed locks, service discovery. Pluggable providers (K8s, Redis, NATS, DB-only, standalone/in-process) |
...
Single-executable deployment via ModKit (Cyber Ware platform requirement). Standalone mode is the default.

**ID**: `cpt-cf-evbk-constraint-multi-sql`

Multi-SQL backend portability: PostgreSQL, MySQL, SQLite. `modkit-db` requirement; avoid backend-specific features. Sequence generation must work correctly across all backends.
...
- **Event** (`gts.cf.core.events.event.v1~`): An immutable record in a `(topic, partition)` log. The `partition` is **broker-derived** at ingest (MurmurHash3-32 over the optional `partition_key` body field, falling back to `tenant_id` — so a tenant's events are totally ordered by default; see [`ADR/0001-partition-selection.md`](ADR/0001-partition-selection.md)); producers do not set `partition` on publish input. The `sequence` (formerly `offset`) is **backend-assigned** during `persist` (Kafka offset / DB row sequence / file segment offset / etc.), monotonic within `(topic, partition)`, and is the consumer-visible ordering key. `sequence_time` records when the backend assigned it. The producer chain `meta.producer_id` / `meta.previous` / `meta.sequence` lives inside the publish-input `meta` block (chained / monotonic modes; absent in stateless), is used for ingest-side dedup, and is stripped from the consumer read projection. See §3.2 "Producer Modes" and §3.6 "Two Sequences".
...
- **ConsumerGroup** (`gts.cf.core.events.consumer_group.v1~`): A GTS-typed consumer group identifier. **Persistent broker resource** stored in `evbk_consumer_group` (one row per registered group). Two creation paths, one table — but each path is exclusive to one of the two GTS-instance shapes:
  ...
  `POST /v1/consumer_groups` always produces an anonymous identifier — there is no way for the caller to request a named shape via this endpoint. Named groups MUST be registered via `types_registry`. Each shape has one provisioning path; no overlap.
...
- **Cursor**: Group-scoped offset tracking, persisted in the broker DB. Keyed by `(consumer_group, topic, partition)`. Stores `acked` and `last_examined` (offset adviser, see R57). Survives subscription churn, delivery instance failover, and group emptiness. **`topic` is in the key for partition disambiguation only** (a partition number is meaningless without saying "of which topic") — `topic` is NOT part of group identity.
...
- `tenant_id` (UUID): Tenant the event belongs to. **Producer-supplied**; ingest validates the producer's principal is authorized to publish to this tenant via the platform's authz resolver.
- `source` (String, ASCII): Origin of the event (e.g., service name).
- `subject` (String, ASCII): Subject entity identifier. Required. Used as the partition-hash input when `partition_key` is absent.
...
- `partition_key` (String, optional, ASCII, ≤ 1024 bytes): Optional producer-supplied routing key. When present, partition = `murmur3_32(ascii_bytes(partition_key)) % topic.partitions`; when absent, the same hash is applied to `tenant_id` (per-tenant ordering by default). There is no explicit producer-set partition. See [`ADR/0001-partition-selection.md`](ADR/0001-partition-selection.md).
...
- `meta` (Object, optional): Publish-time transport-metadata block carrying producer-protocol fields. Versioned (`meta.version`), per-event. Carries `producer_id`, `previous`, `sequence` per the registered producer mode. Omit entirely for stateless publish. See [`ADR/0002-event-schema.md`](ADR/0002-event-schema.md) and [`ADR/0003-idempotent-producer-protocol.md`](ADR/0003-idempotent-producer-protocol.md).

`readOnly` fields (server-stamped; rejected with `400 BadRequest` if supplied on publish):

- `partition` (i32): Broker-derived partition for the event. Computed at ingest via the partition contract above; surfaced for consumer convenience.
- `sequence` (i64): Server-assigned monotonic ordering key per `(topic, partition)`. **Consumer-visible ordering key** — same value space as `cursor.max_sequence` / `received` / `sent` / `acked`. Distinct from `meta.sequence` (publish-only producer-side chain field).
- `sequence_time` (Timestamp): When the server assigned `sequence`.

The `required` array is the union of publish-required and read-required fields; producers using strict validators that don't honor `readOnly` MUST filter `partition`/`sequence`/`sequence_time` before submission. The broker enforces per-direction semantics at the wire regardless of the producer's validator behavior.
...
An ephemeral session a consumer instance opens against a consumer group; carries its `interests[]`, `assigned` partitions, and session liveness state. Lives in cache only, not in the broker's database. See [`schemas/subscription.v1.schema.json`](schemas/subscription.v1.schema.json) and [`ADR/0004-subscription-filter-typing.md`](ADR/0004-subscription-filter-typing.md).
...
- `interests` (Array of `Interest`, ≥1, ≤64): Topic-anchored typed-filter selections. Each `Interest` has `topic`, `tenant_id`, `types` (≥1 GTS pattern strings) required, plus paired-optional `expression_type` + `expression`. Per ADR-0004. Different members of the same group MAY declare different `interests[]` sets (rolling-deploy preserved by construction).
- `assigned` (Array of `{topic, partition}` pairs): Subset of the group's `(topic, partition)` pairs assigned to this subscription by the rebalance algorithm. Computed at JOIN and updated on every topology change. Topic-centric; matches the existing ack / seek mechanics. The topic set is the union of `interest.topic` values across all interests (broker doesn't derive topics — they're explicit on the wire).
- `session_timeout` (String, ISO 8601 Duration): TTL refreshed on every poll/ack (default: `PT30S`).
- `last_seen_at` (String, ISO 8601): Last poll/ack activity timestamp.
...
TTL: `max(member.expires_at)` across active members. An empty group is reaped immediately. Cursor data lives in the cache regardless of group lifecycle (see "Cursor Schema" below for persistence properties).

#### Cursor Schema (Ephemeral, In-Cache, Group-Scoped)

Cursors live in the **ClusterCapabilities-backed cache** (NOT a SQL table). Keyed by `(consumer_group, topic, partition)`. Stores `acked` (highest offset the group has confirmed processing for that `(topic, partition)`) and `last_examined` (highest offset the broker has scanned for this group on that `(topic, partition)`, used by the offset-adviser feature — see R57). Persistence properties match the cache provider: in standalone (in-process cache) cursors die with the process; in cluster with a persistent provider (Redis-with-disk, etcd) cursors survive shard failover. Consumers that need stronger durability use the future `/v1/groups/{group}/offsets` KV endpoint (§4.8) or track offsets in their own DB and seek on JOIN via `POST /v1/subscriptions/{id}/positions`.
...
Per-subscription progress (the `sent` position concept — "delivered but not yet acked") lives in cache as part of subscription state — purely advisory. Topic-level positions (`received`, `max_offset`) are derivable from `backend.segments` / `backend.query` (the storage backend's own `MAX(offset)`) and don't need cache entries.
...
| `gts.cf.core.events.event.v1~` | Event (single schema, `readOnly` / `writeOnly` markers per direction) | [`schemas/event.v1.schema.json`](schemas/event.v1.schema.json), [`ADR/0002-event-schema.md`](ADR/0002-event-schema.md) |
...
| `gts.cf.core.events.subscription.v1~` | Subscription resource (ephemeral, in-cache) | [`schemas/subscription.v1.schema.json`](schemas/subscription.v1.schema.json), [`ADR/0004-subscription-filter-typing.md`](ADR/0004-subscription-filter-typing.md) |
| `gts.cf.core.events.backend.v1~` | Storage backend plugin contract | DESIGN.md §3.2 (Storage Backend Plugin System) |
| `gts.cf.core.events.filter.v1~` | Filter engine plugin contract | [`ADR/0004-subscription-filter-typing.md`](ADR/0004-subscription-filter-typing.md) |
...
1. **Authentication / authorization** at the API edge. `modkit-auth` populates `SecurityContext`; `authz-resolver` enforces `event_type:produce` and `tenant_id`-related scopes via the platform tenant resolver. Rejected publishes return `403`.
2. **Schema-level validation** against `event.v1.schema.json` (per [`ADR/0002-event-schema.md`](ADR/0002-event-schema.md)): structural conformance, ASCII encoding of event-field strings, per-field length caps, `trace_parent` format. Violations return `400 InvalidEventFieldEncoding`, `400 EventFieldTooLong`, `400 InvalidTraceParent`, etc.
...
7. **Partition derivation.** Per [`ADR/0001-partition-selection.md`](ADR/0001-partition-selection.md): `partition = murmur3_32(ascii_bytes(partition_key ?? tenant_id)) % topic.partitions`. The broker stamps `partition` server-side.
8. **Producer-mode shape check.** If `meta` is present, the broker validates `meta.version`, `meta.producer_id`, and the chained / monotonic / stateless mode-shape per [`ADR/0003-idempotent-producer-protocol.md`](ADR/0003-idempotent-producer-protocol.md).
9. **Ingest enqueue and outbox handoff.** The event passes to the modkit-db outbox; backend persist assigns `sequence` and `sequence_time` later. See §3.6 "Event Publish Flow."
...
│       ├── producer.rs            # Outbox-based producer client (wraps modkit-db outbox)
...
└── event-broker/                  # Single module crate with internal service isolation
    └── src/
        ...
        │   │   ├── delivery.rs    # GET /v1/events:stream, ack, positions
...
**Crate Naming**: Directory names hyphenated (`event-broker-sdk`), package names `cyberware-` prefixed (`cyberware-event-broker-sdk`), library names underscored (`event_broker_sdk`).
...
Producer-facing. Owns the write path. Built on the `modkit-db` transactional outbox.
...
- Derive `partition` from the publish body: hash `partition_key` if present, else `tenant_id`. Reject any top-level `partition` field on publish input with `400 BadRequest`. Reject `meta.partition_hint` mismatches with `400 PartitionHashMismatch`. See [`ADR/0001-partition-selection.md`](ADR/0001-partition-selection.md)
...
- Update group cursor (`acked` field, in cache) on `POST /ack`
- Refresh subscription TTL on each poll/ack
...
- `acknowledge(ctx, subscription_id, partition_offsets: HashMap<i32, i64>) → Result<()>` — updates the group cursor in cache
- `seek(ctx, subscription_id, partition_positions: HashMap<i32, i64>) → Result<()>` — overrides the group cursor's `acked` (forward or backward)
...
Cache 1 — Subscription Resolution:
  ...
  TTL:    session_timeout (refreshed on every poll/ack)
...
```
GET /v1/events:stream?subscription_id={S}      (or /ack, /positions, /subscriptions/{S}, DELETE)
  ↓
...
`POST /v1/subscriptions` (JOIN) is the only endpoint that doesn't start with subscription_id resolution — the consumer provides `consumer_group` and `interests[]` directly in the body (per [ADR-0004](ADR/0004-subscription-filter-typing.md)). The JOIN response creates both cache entries (subscription resolution and, if first member, the group endpoint mapping), plus the compiled-filter handles for each interest with paired-optional `expression_type` + `expression`.
...
pub trait StorageBackend: Send + Sync {
    /// Persist events. The backend ASSIGNS the consumer-visible `offset` as part
    /// of its native write (Kafka offset, DB row sequence, file segment offset,
    /// etc.) but does NOT return offsets inline. Consumers learn offsets via
    /// `query` after persist commits.
    ///
    ...
    /// this order when assigning offsets natively.
    ///
    ...
    /// Read events where sequence > offset, ordered by sequence, up to limit.
    /// `offset` and returned `event.sequence` values are the backend's own
    /// sequence space (Kafka offsets, DB rows, etc.).
    async fn query(
        &self, ctx: &SecurityContext, topic: &Topic,
...
pub enum StorageError {
    ...
    #[error(status = 409, problem_type = "sequence-conflict")]
    SequenceConflict { topic: String, sequence: i64 },

    #[error(status = 412, problem_type = "sequence-violation")]
    SequenceViolation {
        topic: String,
        partition: i32,
...
| `POST /v1/subscriptions/{id}/ack` | Delivery | Acknowledge consumed events (commit per-partition offsets) |
| `POST /v1/subscriptions/{id}/positions` | Delivery | Seek (override broker's tracking, replay or skip) |
| `GET /v1/events:stream` | Delivery | Long-poll for new events (subscription-based) |
| `POST /v1/subscriptions` | Delivery | Create subscription |
| `GET /v1/subscriptions` | Delivery | List subscriptions |
| `DELETE /v1/subscriptions/{id}` | Delivery | Delete subscription |
| `GET /v1/topics` | Shared | List topics |
| `GET /v1/topics/segments` | Shared | Get topic segment info |
...
2. DeliveryService reads from the **per-partition in-memory cache** (an append-only linked list of recent batches; one cache instance per `(topic, partition)` owned by this delivery shard). The stream holds an iterator into the list at the consumer's `cursor.acked` position.
...
The broker supports three producer modes — **chained**, **monotonic**, **stateless** — for ingest-side idempotent publishing. **Mode is declared once at producer registration** (`POST /v1/producers { "mode": "chained" | "monotonic" }`) and enforced per request. Stateless publish does not register; the producer omits the `meta` block entirely and the broker performs no dedup. Producer-protocol fields (`producer_id`, `previous`, `sequence`) live inside the publish-time `meta` block on the event (marked `writeOnly` per [ADR-0002](ADR/0002-event-schema.md)) and are stripped on the consumer-visible read response.
...
1. `POST /v1/subscriptions` creates a subscription with `interests[]` (topic-anchored typed-filter selections per [ADR-0004](ADR/0004-subscription-filter-typing.md)) and `session_timeout`. JOIN validation is all-or-nothing — see ADR-0004 § JOIN Validation Order.
...
5. Acknowledgment: `POST /v1/subscriptions/{id}/ack` advances each partition's `cursor.acked` position.
...
Each subscription has a Cursor tracking four positions within the topic:

```text
                    acked     sent     received    max_sequence
                      │        │          │            │
                      ▼        ▼          ▼            ▼
Events: ─────[1]──[2]──[3]──[4]──[5]──[6]──[7]──[8]──[9]──→
              ▲                         ▲                ▲
              │                         │                │
         consumer confirmed      delivered to       ingested
         processing complete     consumer           into topic
```
...
| `sent` | Highest sequence delivered to this consumer | DeliveryService (on poll response) |
| `acked` | Highest sequence consumer has confirmed processing | DeliveryService (on offset commit) |

The gap between `acked` and `sent` represents events delivered but not yet confirmed. The gap between `sent` and `received` represents events available for the next poll.

#### Outbox-Based Producer and Ingest Pipeline

**ID**: `cpt-cf-evbk-component-outbox-ingest`

The `modkit-db` transactional outbox is the foundation for both the **producer client library** (SDK-side) and the **ingest service** (server-side). This reuses proven infrastructure — the outbox's four-stage pipeline (enqueue → sequencer → processor → vacuum), per-partition ordering, and transactional guarantees — rather than building a parallel mechanism.
...
HTTP POST /v1/events
    ...
    → Sync mode (opt-in): wait for outbox processor → backend.persist completion;
      respond 201 Created { event_id, sequence, partition, ... }

Asynchronously, in the outbox processor:
    → backend.persist(events without broker sequence)
        → Backend ASSIGNS broker sequence as part of native write
          (Kafka offset / DB sequence / file segment offset / etc.)
        → Returns events with sequence populated
...
**The vacuum stage** (last of the outbox's four stages) is implemented entirely by `modkit-db`'s outbox library — not by the broker. Vacuum's cluster ownership, failure recovery, leader election, and any tuning live in `modkit-db`. The broker contributes nothing beyond using the outbox.
...
|---|---|---|
| Producer outbox (modkit-db, in producer module's DB schema) | Transactional write integrity — if the producer's business transaction commits, the event WILL reach the broker, even if the broker is unreachable right now | At-least-once delivery from producer to broker, atomic with business state |
| Ingest outbox (modkit-db, in event-broker module's DB schema) | In-pipeline order preservation + decoupling backend latency from HTTP latency | At-least-once persistence to backend, per-`(topic, partition)` order |

**No table-name collision in standalone**: every Cyber Ware module owns its own DB schema (platform invariant), so `producer_outbox` in the producer module's schema and `ingest_outbox` in the event-broker's schema are distinct tables in the same database. The producer SDK never names the ingest's outbox table — it talks to its own module's outbox and lets `IngestService` (in-process trait call in standalone, HTTP in cluster) handle the rest.
...
In cluster mode, the dispatcher routes producer requests (ingest path) and consumer requests (delivery path) using two independent strategies. Neither relies on exclusive `(topic, partition)` ownership for correctness — the modkit-db outbox handles ingest serialization at row granularity (see R03), and delivery uses cache-based instance lookup (R04 / R58).
...
| `<prefix>.*` | Match any topic GTS string starting with `<prefix>.` | `gts.cf.vendor.audit.*` matches `gts.cf.vendor.audit.users.v1` |
...
Earlier drafts of this design framed ingest as "topic-sharded with a single owner per `(topic, partition)`" — that framing is wrong (see R03). The outbox's claim-based locking serializes per-`(topic, partition)` at row granularity inside the modkit-db outbox, regardless of how many ingest instances exist or how the dispatcher routes. The dispatcher's routing is a **load-distribution choice**, not a correctness invariant:
...
- Group cursor: cache entry survives if the cache provider is persistent (Redis-with-disk, etcd) and not bound to B's lease. New owner reads `evbk.cursor:{group}:{topic}:{partition}` and resumes from `acked`. If the cache also lost the cursor, recovery falls through to `earliest_available_offset` per `backend.query` (consumers re-process per at-least-once).
...
│   POST /v1/events:batch          │   POST /v1/subscriptions          │
│                                  │   POST .../{id}/ack               │
│                                  │   POST .../{id}/positions         │
│                                  │   DELETE .../{id}                 │
│                                  │                                   │
...
│   GET /v1/topics, GET /v1/topics/segments                            │
└──────────────────────────────────────────────────────────────────────┘
```
...
| `partition` | i32 | Yes | Target partition within the topic. Producer-selected (key-hash, explicit, or other — see partition-selection ADR placeholder). Must be in `[0, topic.partitions)`. |
...
- `412 SequenceViolation` — chained mode: `meta.previous` doesn't match the broker's `last_sequence` for this `(producer_id, topic, partition)`. Response carries broker's known `last_sequence`. Recover via `GET /v1/producers/{producer_id}/cursors`.
...
- `412 SequenceViolation` — chained mode: a chain link in the batch doesn't match the broker's `last_sequence`. Atomic batch rejection.
...
**POST /v1/producers** — Register a producer. Returns a server-issued `producer_id` bound to the calling principal.
...
The poll body is empty. The broker uses each `(consumer_group, topic, partition)`'s `cursor.acked` to determine which events to return. Consumers advance cursors via `POST /v1/subscriptions/{id}/ack`, override via `POST /v1/subscriptions/{id}/positions`.

Behavior:
1. Events with `sequence > cursor.acked` available on any assigned `(topic, partition)` → return `200 OK` immediately
...
Each entry in `batches` carries its own `topic` + `partition` and the events from that pair. `assignments` lists every `(topic, partition)` currently owned by this subscription with its `offset` (= `cursor.acked` — where the next consumer joining the group would resume) and `last_examined` (the broker's highest scanned sequence for this group/partition regardless of filter match — see R57). When this member's filter rejects all events scanned, `last_examined` advances while `offset` stays put. When events do match, both advance together. (Filter is per-member: this advancement reflects the current owner's filter.)

Errors:
- `404 SubscriptionNotFound` — Subscription expired or deleted

**POST /v1/subscriptions/{id}/ack** — Acknowledge consumed events (commit per-`(topic, partition)` offsets).

The name reflects the semantics: the consumer is **acknowledging** that it has successfully processed events up to a given sequence per `(topic, partition)`. Future stateless offset KV — keyed by group, not subscription — is reserved at `/v1/groups/{group}/offsets` and is intentionally separate from this acknowledgment endpoint.

Request body: two-level map keyed by topic, then partition, with the highest acked sequence:

> _Illustrative example removed; authoritative shape: [openapi.yaml](openapi.yaml)._

The two-level shape mirrors the `batches` response: events arrive grouped by `(topic, partition)`; acks go back the same way.

Broker semantics:
- `cursor.acked` advances per `(consumer_group, topic, partition)` using `MAX(current, requested)` — late commits are accepted as long as the `(topic, partition)` is still owned by this subscription
- Commits to `(topic, partition)` pairs not assigned to this subscription return `409 PartitionNotAssigned` with the current assignment in the body, so the consumer can self-heal

**SDK behavior on `409 PartitionNotAssigned` during ack** (normative): a rebalance has reassigned the partition to a different member of the group. The SDK MUST drop the in-flight batch — the new owner will receive those events on its next poll. The application's commit of business-side state for the dropped events is not lost; the broker only guarantees **at-least-once** delivery, so the new owner re-receiving them is expected and the application MUST be idempotent on event processing (per §2.1 Design Principles). Self-heal: SDK reads `topology_version` and `assignments` from the response body and resumes polling against the updated assignment.

`POST /ack` can also be called **before any poll** to override broker tracking — used at startup when the consumer has its own offset state (e.g., its own DB) that's ahead of the broker's `cursor.acked`. If the consumer wants to **rewind** (read older events), use `POST /positions` instead — `/ack` only advances forward via the `MAX` rule.

Response: `204 No Content`

Errors:
- `409 PartitionNotAssigned` — Commit references a `(topic, partition)` not currently assigned to this subscription (state mismatch, not permission). Body includes current `topology_version` and `assignments` for self-heal:

> _Illustrative example removed; authoritative shape: [openapi.yaml](openapi.yaml)._

- `404 SubscriptionNotFound` — Subscription expired or deleted

**Pre-stream SEEK and start-position resolution** (added by the `consumer-seek-semantics` change).
...
- `Exact(i64)` — the **last offset the consumer has already processed** (the stored cursor value, returned verbatim; symmetric with the `save_on_eb(..., offset)` ack argument). The broker emits from `offset + 1`.
...
Each built-in `OffsetManager` (`BrokerOffsetManager`, `LocalDbOffsetManager`, `InMemoryOffsetManager`) requires a `Fallback` (`Earliest` | `Latest`) argument in its constructor — the policy applied when the backing store has no cursor for a `(group, topic, partition)`. Optional per-partition seed overrides for replay/backfill via `.with_overrides([...])`.
...
3. SEEK: `POST /v1/subscriptions/{id}/positions` with the resolved per-partition values (integer or sentinel string).
4. Stream: `GET /v1/events:stream?subscription_id=...`.

**Sentinel resolution semantics on the broker** (positions endpoint):

- Request body's `partition_positions` map accepts `int64 | "earliest" | "latest"` per partition. Integers are last-processed offsets; the broker validates against `[retention_floor - 1, high_water_mark]` and rejects out-of-range with `400 InvalidInitialPosition`.
...
**`:stream` defensive backstop**: `GET /v1/events:stream` rejects with `409 PositionsNotSet { unseeded: [...], recovery_hint }` when any assigned partition has no committed cursor in the subscription's group. A well-behaved SDK seeds positions before opening the stream and never observes this on the happy path; the SDK recovers from it by re-resolving via `position()` and re-SEEKing (sharing the `SubscriptionRecoveryExhausted` budget).

**Topology-frame re-SEEK**: when a `WireFrame::Topology { topology_version, assigned }` arrives mid-stream, the dispatcher diffs `prev_assignments` vs `new_assignments`. For partitions in `new \ prev`, the same `position() → SEEK` flow runs before the broker may deliver events for them. Continuing partitions keep their cursor; lost partitions follow the existing drop-in-flight behavior.

**Forward-only enforcement during streaming**: while a subscription's `:stream` is open, SEEK requests are forward-only (`MAX(stored, requested)` per partition; backward moves rejected with `409 SeekBackwardNotAllowed`). Pre-stream SEEK (before the first `:stream` after JOIN, or after a partition's cursor was admin-cleared) accepts any value in the valid range — this is how `Earliest`/`Latest` reseeds and `Custom` replay overrides work.
...
The `partitions` field exposes the topic's partition count — necessary for producers to compute `partition` from a key hash, and for consumers to know the full partition set when subscribing with explicit assignment.
...
Consumer groups are persistent broker resources (`evbk_consumer_group` table). Each group's GTS shape determines its provisioning path: **anonymous** (UUID-backed) groups via `POST /v1/consumer_groups`; **named** (vendor-namespaced) groups via `types_registry` registration. JOIN authorization differs by shape: anonymous → owner-tenant equality; named → explicit `:consume` permission via PEP.

**POST /v1/consumer_groups** — Register an **anonymous** consumer group. The broker mints the group identifier server-side; the request body MUST NOT carry an `id`.

Request body:
> _Illustrative example removed; authoritative shape: [openapi.yaml](openapi.yaml)._

Response (`201 Created`, `Location: /v1/consumer_groups/<encoded-gts-id>`):
> _Illustrative example removed; authoritative shape: [openapi.yaml](openapi.yaml)._
...
(The `400 InvalidConsumerGroupId`, `400 NamedGroupRequiresRegistry`, and `409 ConsumerGroupAlreadyExists` error classes that previously applied to the create path are no longer reachable here — there is no caller-supplied `id` to validate or collide on. `400 NamedGroupRequiresRegistry` and `409 ConsumerGroupAlreadyExists` remain in the broker's error catalog for JOIN-side and named-via-`types_registry` cases respectively.)

**GET /v1/consumer_groups/{id}** — Read a registered consumer group. Returns the full record.

Errors: `403 Forbidden` (no permission), `404 ConsumerGroupNotFound`.

**GET /v1/consumer_groups** — List consumer groups visible to the caller (filtered by `AccessScope` from the PEP — typically the caller's tenant's anonymous groups plus any named groups the caller has `:consume` permission on).

**DELETE /v1/consumer_groups/{id}** — Remove a consumer group from the registry. Allowed only when there are no active members (cache state empty for this group).

Errors: `403 Forbidden`, `404 ConsumerGroupNotFound`, `409 ConsumerGroupHasActiveMembers`.
...
```
POST   /v1/subscriptions                          # JOIN — create subscription, get assignments+offsets
       Body: { consumer_group: <GTS-id>,           # REQUIRED, GTS identifier (named or anonymous UUID)
               topics: [<GTS-id>, ...],            # REQUIRED, this member's topics
               types?, subject_types?, cel?,       # this member's filters (per-member, not canonical)
               session_timeout?, auto_commit? }
       Returns 201: { id, topology_version, expires_at,
                      assignments: [ { topic, partition, offset, last_examined } ] }

GET    /v1/events:stream?subscription_id={uuid}
                                                  # POLL — long-poll for events
       Returns 200: { topology_version,
                      assignments: [ { topic, partition, offset, last_examined } ],
                      batches: [ { topic, partition, events: [...] } ],
                      retry_after_seconds? }
       Returns 410 Gone: { detail: "Subscription terminated; re-JOIN to recover" }
                                                  # owning delivery shard released ownership
       Returns 404:    { detail: "Subscription not found or expired" }
                                                  # subscription_id unknown to current owner

POST   /v1/subscriptions/{id}/ack                # ACK — acknowledge consumed events (advance cursors)
       Body: { "<topic>": { "<partition>": offset, ... }, ... }  # 2-level: topic → partition → offset
       Returns 204
       (forward-only via MAX rule; for rewind use /positions)

POST   /v1/subscriptions/{id}/positions          # SEEK — override broker's tracking (forward or backward)
       Body: { "<topic>": { "<partition>": offset, ... }, ... }  # same 2-level shape
       Returns 204
       Variants: /positions/beginning, /positions/end (target all assigned (topic, partition) pairs)

DELETE /v1/subscriptions/{id}                     # LEAVE — explicit teardown
       Returns 204

GET    /v1/subscriptions                          # LIST — active subscriptions for the caller
       Returns 200: { subscriptions: [...] }

GET    /v1/subscriptions/{id}                     # READ subscription state
       Returns 200: { id, consumer_group, topics: [...], topology_version,
                      assignments: [ { topic, partition, offset, last_examined } ], ... }
```
...
**Assignments shape**: Each entry: `{ "topic": <string>, "partition": <i32>, "offset": <i64>, "last_examined": <i64> }`. `offset` is `cursor.acked` for `(consumer_group, topic, partition)` — events with `sequence > offset` will be returned on the next poll. `last_examined` is the highest sequence the broker has scanned for this group/`(topic, partition)` regardless of filter match (see R57 for the offset-adviser rationale).

**Auto-commit option**: Auto-commit can be enabled per subscription via `auto_commit: true` in the `POST /v1/subscriptions` body — the broker advances `cursor.acked` to `cursor.sent` immediately after sending the poll response (i.e., **broker-side**, before the consumer has processed the events). This is **at-most-once**: a consumer that crashes between receiving the response and finishing processing loses those in-flight events — they will not be redelivered, since the cursor has already advanced past them. Default is `auto_commit: false` (explicit commit required), which is the at-least-once path: events stay in-flight until the consumer calls `POST /ack`, and any consumer crash before ack causes redelivery on the next subscription. **Recommendation**: leave `auto_commit: false` unless the application explicitly tolerates event loss (e.g., metrics aggregation where occasional drops are acceptable).

#### Error Response Format

**ID**: `cpt-cf-evbk-interface-error-codes`

All errors follow RFC 9457 Problem Details (`application/problem+json`) with GTS type identifiers.

| Error Type | HTTP | GTS Instance ID | Retriable | Description |
|---|---|---|---|---|
| InvalidType | 400 | `gts.cf.core.errors.err.v1~cf.core.type.invalid.v1` | No | Invalid GTS type format |
| InvalidOffset | 400 | `gts.cf.core.errors.err.v1~cf.core.offset.invalid.v1` | No | Invalid offset value |
| InvalidSequence | 400 | `gts.cf.core.errors.err.v1~cf.core.sequence.invalid.v1` | No | Invalid sequence value |
| InvalidPartition | 400 | `gts.cf.core.errors.err.v1~cf.core.partition.invalid.v1` | No | Partition out of range `[0, topic.partitions)` |
| MixedPartitionBatch | 400 | `gts.cf.core.errors.err.v1~cf.core.batch.mixed_partition.v1` | No | Batch contains events from different `(topic, partition)` combinations |
| BatchTooLarge | 400 | `gts.cf.core.errors.err.v1~cf.core.batch.too_large.v1` | No | Batch exceeds size limit (event count or total bytes) |
| PartitionNotAssigned | 409 | `gts.cf.core.errors.err.v1~cf.core.subscription.partition_not_assigned.v1` | No | Operation references a partition not assigned to this subscription. Body includes `topology_version` and current `assignments` for self-heal. |
| TopicNotFound | 404 | `gts.cf.core.errors.err.v1~cf.core.topic.not_found.v1` | No | Topic not found |
| EventTypeNotFound | 404 | `gts.cf.core.errors.err.v1~cf.core.event_type.not_found.v1` | No | Event type not found |
| SubscriptionNotFound | 404 | `gts.cf.core.errors.err.v1~cf.core.subscription.not_found.v1` | No | Subscription not found or expired |
| SequenceConflict | 409 | `gts.cf.core.errors.err.v1~cf.core.sequence.conflict.v1` | Yes | Broker sequence contention (retry with re-read) |
| SequenceViolation | 412 | `gts.cf.core.errors.err.v1~cf.core.sequence.violation.v1` | No | Chained mode: `meta.previous` doesn't match the broker's `last_sequence` for this `(producer_id, topic, partition)`. Response carries broker's known `last_sequence`. Recover via `GET /v1/producers/{producer_id}/cursors`. See [`docs/features/0001-idempotent-producers.md`](features/0001-idempotent-producers.md) §4.2 and §9. |
| InvalidConsumerGroupId | 400 | `gts.cf.core.errors.err.v1~cf.core.consumer_group.invalid_id.v1` | No | `consumer_group` is missing, malformed, or not a valid GTS identifier conforming to `gts.cf.core.events.consumer_group.v1~` (raised on JOIN / lookup / delete paths; the create endpoint mints its own id and cannot raise this) |
| ConsumerGroupNotFound | 404 | `gts.cf.core.errors.err.v1~cf.core.consumer_group.not_found.v1` | No | `consumer_group` GTS identifier is not registered in `evbk_consumer_group` (anonymous: register via `POST /v1/consumer_groups`; named: register via `types_registry`) |
| ConsumerGroupNotOwned | 403 | `gts.cf.core.errors.err.v1~cf.core.consumer_group.not_owned.v1` | No | (Anonymous groups only) caller's `tenant_id` does not match the group's `owner_tenant_id`. Cross-tenant sharing of anonymous groups is post-MVP. |
| ConsumerGroupAlreadyExists | 409 | `gts.cf.core.errors.err.v1~cf.core.consumer_group.already_exists.v1` | No | GTS identifier already registered — raised by `types_registry` upsert when a named instance collides, not by the anonymous create path (broker-minted UUIDs do not collide) |
| NamedGroupRequiresRegistry | 400 | `gts.cf.core.errors.err.v1~cf.core.consumer_group.named_requires_registry.v1` | No | A caller referenced a named-instance identifier on a path that requires registry-provisioning (JOIN against an id that's not in `types_registry`); named groups must first be registered via `types_registry` |
| ConsumerGroupHasActiveMembers | 409 | `gts.cf.core.errors.err.v1~cf.core.consumer_group.has_active_members.v1` | No | `DELETE /v1/consumer_groups/{id}` rejected because the group still has active subscriptions in cache |
| SyncNotSupported | 400 | `gts.cf.core.errors.err.v1~cf.core.persist.sync_not_supported.v1` | No | Sync persist mode requested but the bound backend doesn't support it (e.g., S3 with eventual consistency) |
| BackendUnavailable | 503 | `gts.cf.core.errors.err.v1~cf.core.persist.backend_unavailable.v1` | Yes | (Sync persist mode) backend is unavailable; retry later. Async mode buffers in the outbox; this error is sync-mode-only. |
| NoIngestForTopic | 503 | `gts.cf.core.errors.err.v1~cf.core.routing.no_ingest_for_topic.v1` | Yes | (Sharded mode only) no healthy ingest instance has a `serves_topic_patterns` entry that matches the event's topic GTS, and no instance carries the `*` default fallback. Dispatcher retries until a matching instance is healthy. |
| ValidationError | 422 | `gts.cf.core.errors.err.v1~cf.core.payload.validation_error.v1` | No | Event data doesn't match type schema |

**Standard Fields** (RFC 9457):
- `type`: GTS identifier for the error type
...
- `retry_after_seconds`: Retry guidance (for retriable errors)

#### Authentication & Authorization

**Authentication** (Inbound, Client → Event Broker): Bearer token via `modkit-auth`.
...
| `gts.cf.core.events.consumer_group.v1~` | — | consumers (named groups; explicit grant required) | group creators (POST `/v1/consumer_groups`) and `types_registry` upserts |
...
**Poll / ack / seek / cancel** — no re-check. Authorization is done at JOIN; the cached AccessScope governs all subsequent reads. `subscription_id` is the bearer of authority for follow-up operations. There is no separate `manage` permission — subscription create / list / ack / seek / cancel are gated by the same `consume` permission.
...
| `modkit-db` | Database persistence (SeaORM, multi-backend) and the transactional outbox library used by both producer SDK and ingest pipeline. |
| `modkit-auth` | Bearer token authentication; populates `SecurityContext`. |
...
sequenceDiagram
    ...
    participant OB as Ingest Outbox (modkit-db)
    ...
    SB->>SB: Backend assigns broker sequence natively<br/>(Kafka offset / DB sequence / file offset)
    SB-->>OP: events with broker sequence populated
...
#### Offset Acknowledgment Flow

```text
Consumer → POST /v1/subscriptions/{id}/ack { "T1": { "4": 12345 }, "T2": { "0": 6789 } }
  → DeliveryService.acknowledge(ctx, subscription_id, { ("T1", 4): 12345, ("T2", 0): 6789 })
    ...
    → for each ((topic, partition), offset):
        cache.update("evbk.cursor:{G}:{topic}:{partition}",
                     |cur| { acked: max(cur.acked, offset), updated_at: now() })
        (and verify (topic, partition) is in this subscription's assignment, else 409)
    → Backend may later delete events at its discretion per its own retention policy (broker has no Cleaner)
  → 204 No Content
  → 409 PartitionNotAssigned (with current assignments in body) if any partition is not in this subscription's assignment
```
...
Cache value: { acked: i64, last_examined: i64, updated_at: timestamp }
...
**Why cache-only**: cursor durability is delegated to the ClusterCapabilities provider. In standalone, the cursor lives in the in-process cache and dies with the process — recovery is "resume from `earliest_available_offset`". In cluster mode with a persistent cache provider (Redis-with-disk, etcd), cursors survive shard failover via the cache. The broker has no SQL table for cursors and no native durability story beyond what the cache provides — by design. Consumers that want stronger durability use the future `/v1/groups/{group}/offsets` KV endpoint (see §4.8) or track offsets in their own DB and seek on JOIN via `POST /v1/subscriptions/{id}/positions`.

**`acked`**: The highest offset the group has confirmed processing. Advanced by `POST /ack` (forward-only via MAX rule). The next consumer to join the group resumes from `acked + 1` if the cache entry is still alive.

**`last_examined`** (offset adviser, see R57): The highest offset the broker has scanned for this group on this partition, regardless of whether events matched the subscription's filters. Lets consumers skip ahead when filters reject most of the topic content. Always satisfies `last_examined ≥ acked`.

The four conceptual positions (`received`, `max_offset`, `sent`, `acked`) are split by where they belong:
- `acked` (group's confirmed processing) → in cache (this entry)
- `last_examined` (broker's scan position regardless of filter) → in cache (this entry)
- `sent` (per-subscription "delivered but not yet acked") → in cache, per-subscription advisory
...
- **`POST /v1/consumer_groups`** — anonymous-only; `id` is **broker-minted** (`gts.cf.core.events.consumer_group.v1~<uuid>`), the request body MUST NOT carry `id`. `tenant_id` and `owner_principal` come from `SecurityContext` at create time; non-overridable. JOIN authz is owner-tenant equality.
...
The wire surface has **two consumer-visible sequence concepts** and one **internal** one (the ingest outbox sequencer), each owned by exactly one component. After [ADR-0002 Event Schema](ADR/0002-event-schema.md), the producer chain lives in `meta` (publish-only via `writeOnly`, stripped on read) and the server-assigned ordering key is named `sequence` (renamed from `offset`) — matching the cursor model's `max_sequence` / `received` / `sent` / `acked` terminology.
...
| **Backend `sequence`** (`event.sequence`, `readOnly`) | **Storage backend** | per `(topic, partition)` | Consumer-visible ordering key. What `cursor.acked` tracks; seek positions reference. Renamed from `offset` per [ADR-0002 § Terminology Cleanup](ADR/0002-event-schema.md#terminology-cleanup-offset--sequence) | Consumers; the cursor model uses the same value space |
| **Outbox sequence** (internal) | modkit-db ingest-side outbox | per outbox partition | **Order preservation** in the ingest pipeline (events for a `(topic, partition)` are delivered to backend in enqueue order) | Internal to broker plumbing; never exposed on the wire |

**Critically**: the outbox does NOT assign the consumer-visible `sequence`. It assigns its own internal queue ordering number, which is invisible. The consumer-visible `sequence` is whatever the backend produces natively (Kafka offset, DB row sequence, file segment offset, etc.).
...
**Outbox sequence** — assigned by `modkit-db`'s outbox sequencer when an event is enqueued in the ingest-side outbox. Guarantees events for the same outbox partition are delivered to `backend.persist` in enqueue order. Stays internal to the outbox; not exposed in the API; not stored by the backend.

**Backend `sequence`** — assigned by the backend during `persist`:
- **DB backend**: native DB sequence (e.g., Postgres `BIGINT GENERATED ALWAYS AS IDENTITY` per `(topic, partition)` partition, or per-row sequence within segment); backend's storage layout dictates the implementation.
- **Kafka backend**: Kafka's own offset. Native, atomic. The wire-level `sequence` IS the Kafka offset.
- **File / S3 backend**: backend's own counter (segment-relative offset + segment base, or a backend-managed durable counter).
- **In-memory (test)**: `AtomicI64`.
...
- **Consumers see only `sequence`.** Consumer's seek parameter is in the backend's sequence space; backend's `query` translates seek positions natively (Kafka: `seek(offset)`; DB: `WHERE "sequence" > $sequence`).
...
- Per-(topic, partition) sequence monotonicity is enforced by the **backend's** native sequence mechanism (Kafka offsets, DB sequences, etc.). The broker doesn't need a global UNIQUE index across segments.
...
- Cursor positions satisfy: `0 ≤ acked ≤ backend.MAX(sequence) for that (topic, partition)`. `sent` is per-subscription advisory state in cache (not on the cursor); `received`/`max_sequence` are derivable via `backend.segments()` or `backend.query()`.
...
modules:
  event_broker:
    ...
    # ClusterCapabilities provider is configured at the platform level (modkit),
...
| `evbk_cursor_lag` | Gauge | `tenant_id`, `consumer_group`, `topic`, `partition` | Gap between topic's `MAX(offset)` and `cursor.acked` for the group |
...
| `evbk_filter_eval_timeout_total` | Counter | `consumer_group` | Per-event filter evaluation exceeded `EVAL_TIMEOUT_MICROS`. Event silently dropped from this consumer's batch; poll does NOT fail. Per ADR-0004. |
...
- Transient failures (network, backend slow, retryable error): outbox retries with backoff per modkit-db's retry policy.
...
- **Tenant depth for topic visibility**: Should `tenant_depth` control topic visibility? Options: `ALL` (full hierarchy), `DIRECT` (immediate children only), `CURRENT` (own tenant only). Current design uses `CURRENT` only.
- **Partition selection algorithm**: How does the producer decide the target partition? Captured as ADR placeholder `cpt-cf-evbk-adr-partition-selection`. Candidates: key-hash (default, Kafka-style), explicit, round-robin, custom plugin. Decision deferred.
...
- **Outbox extension for partition-level scaling**: should `modkit-db`'s outbox grow native partitioning so a topic with N partitions has N outbox sub-queues processed independently (rather than one outbox queue per topic with the sequencer juggling partition order)? Two shapes to consider: (a) multiple outbox table instances (schema-per-partition or table-per-partition), (b) a partition-aware sequencer + processor pool inside a single outbox table. Current design uses one outbox per topic; works for moderate throughput. Hot-topic deployments may need this. Decision deferred to post-MVP, gated by the outbox library's roadmap.
- **Long-poll protocol revisit**: the per-partition in-memory cache + condvar-driven iterator model (see §3.2 Long-Poll Mechanism) needs a focused pass once feedback is closed. Open sub-questions: cache eviction policy (size-based, time-based, both?); backfill path when a consumer's `cursor.acked` is older than the cache's earliest retained batch (fall through to `backend.query`?); iterator-vs-cursor reconciliation across partition reassignment; condvar / notification ordering and fairness across many waiters; interaction with `cluster.subscribe` provider semantics (lossy vs lossless pub/sub).
- **Auto-commit semantics**: Should the broker auto-commit consumer offsets on poll (advancing `acked` to `sent`), or always require explicit `POST /v1/subscriptions/{id}/ack`? Auto-commit is convenient but risks losing events on consumer crash mid-processing.
...
- **Anonymous offset KV**: `GET /v1/groups/{group}/offsets` — namespace reserved for stateless offset storage keyed by group
...
- **Server-side persistent offset KV**: a stateless offset-storage endpoint at `GET / PUT /v1/groups/{group}/offsets`, keyed by `consumer_group`, decoupled from subscription lifecycle. Consumers that need offset durability stronger than what the ClusterCapabilities cache provides (e.g., standalone mode where the cache is in-process) can opt in by writing offsets here. Reserved namespace; not in MVP.
...
| `cpt-cf-evbk-fr-long-poll` | §3.2 Long-Poll Mechanism; §3.5 Long-Poll Consumption Flow |
...
| `cpt-cf-evbk-fr-anonymous-group-create` | §3.3 Consumer Group Lifecycle (`POST /v1/consumer_groups`); §3.6 `evbk_consumer_group` |
...
- **Apache Kafka `BuiltInPartitioner`** — algorithm reference for the partition-selection ADR: <https://kafka.apache.org/documentation/#producerconfigs_partitioner.class>
...
- [ADR/0001-partition-selection.md](ADR/0001-partition-selection.md) — partition-selection ADR.
