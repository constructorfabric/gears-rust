...
```
  ...
  ┌──────────────────────────────────────────────┐
  │  Polling                                      │
  │  Server holds: cursor.{acked, sent} per       │
...
|---|---|---|---|
| **C1** | Single, DB-backed, solo | Joins group with itself as sole member, gets all partitions, processes & commits in own DB transaction (broker's `cursor.acked` is a checkpoint, consumer's own DB is source of truth) | On restart: same Producer-Id, server recreates assignment if `session_timeout` expired or resumes if still active |
| **C2** | Single, broker-backed, solo | Joins group, polls, broker tracks all cursor state | On crash: `session_timeout` expires; new instance joins → gets all partitions → reads from `cursor.acked` → re-receives un-acked events |
...
| **C7** | Bulk historical replay | Join with a dedicated `consumer_group` (e.g., `replay-{job-id}`), then `POST /positions { "P": 0 }` to seek to start, then poll | Standard subscription path; the dedicated group ensures replay traffic doesn't compete with primary consumers. Anonymous offset-based read is **out of scope for MVP** — see DESIGN.md §4.6/§4.8. |
...
- Cursor `(consumer_group, topic, partition)` advances based on the current owner's processing
...
- Events that F1 rejected but F2 would accept may have been already scanned-past by F1 (advanced via `last_examined`); F2 will not see them
...
T5: B wakes, polls (B's offsets reflect A's last acked position — cursor is in cache, sticky)
    B's response: { topology_version: 3,
                    assignments: [{topic:"T", partition:0, offset:<A's acked>, last_examined:<A's last_examined>}],
                    batches: [{topic:"T", partition:0, events:[<events with offset > A's acked>]}] }
    Events polled-but-not-acked by A are re-delivered to B (at-least-once)
```
...
T1: B: POST /v1/subscriptions {consumer_group: "gts.cf.core.events.consumer_group.v1~vendor.audit.v1", client_agent: "audit-svc/1.0", interests: [{topic: "T", tenant_id: ..., types: ["gts.cf.core.events.event.v1~*"]}]}
    ...
    Response 201: { id: B, topology_version: 2,
                    assignments: [ {topic:"T", partition:4, offset:<A's acked on 4>, ...},
                                   {topic:"T", partition:5, offset:<A's acked on 5>, ...},
...
T2: A's stream wakes on topology v=2
    ...
    A returns 200: {
      topology_version: 2,
      assignments: [ {topic:"T", partition:0, offset:<A's acked>, ...}, ...,
                     {topic:"T", partition:3, offset:<A's acked>, ...} ],
...
T3: B polls
    ...
    Owning instance reads cursor[(G,T,4..7)].acked — sticky offsets preserved
    B gets events from (T, 4..7) starting at those offsets
    Includes anything A polled-but-didn't-ack (at-least-once redelivery)

T4: B processes events, acknowledges via:
    POST /v1/subscriptions/B/ack { "T": { "4": 130, "5": 78 } }
```
...
rebalance(group G):  # G is the GTS-typed consumer_group identifier
  ...
  try:
    ...
    # Existing acked values are preserved automatically (group-keyed, sticky in cache)
    for (topic_T, partition) in new_assignments.keys():
      cache.put_if_absent("evbk.cursor:{G}:{topic_T}:{partition}",
                          { acked: 0, last_examined: 0, updated_at: now() })
...
**At-least-once contract**: When partitions migrate from sub A to sub B, `cursor.acked` is preserved (sticky); `cursor.sent` is reset to `cursor.acked` so B re-receives any events A polled but didn't ack. Consumers MUST be idempotent in their processing.
