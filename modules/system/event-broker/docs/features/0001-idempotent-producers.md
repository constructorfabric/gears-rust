...
<!-- Revised: 2026-05-12 — aligned to ADR-0002 Event Schema, ADR-0003 Idempotent Producer Protocol, and revised ADR-0001 Partition Selection -->

# Feature: Idempotent Producers

- [ ] `p1` - **ID**: `cpt-cf-evbk-feature-idempotent-producers`
...
- [5. Producer / Outbox Contract](#5-producer--outbox-contract)
- [6. States](#6-states)
...
Idempotent-producer support lets producers retry safely after transport failures, partial commits, or network timeouts without admitting duplicates into the log. Combined with the producer-side transactional outbox (`modkit-db`), this gives effective exactly-once semantics for in-platform event flows.

### 1.3 Actors

- **Producer SDK** (`cyberware-event-broker-sdk`): on first publish, calls `POST /v1/producers` to obtain a `producer_id` bound to its principal; populates `meta.producer_id` / `meta.previous` / `meta.sequence` per the registered mode; computes partition locally via Murmur3-32 for outbox routing; retries on transient failures.
...
- [ADR-0001 Partition Selection (revised)](../ADR/0001-partition-selection.md)
- [ADR-0002 Event Schema](../ADR/0002-event-schema.md)
- [ADR-0003 Idempotent Producer Protocol](../ADR/0003-idempotent-producer-protocol.md)
...
# body: { "producer_id": "<uuid>", "mode": "chained", "client_agent": "<echoed>" }
...
## 5. Producer / Outbox Contract

The "exactly-once via idempotent producer" property is derived from a chain of guarantees:

1. **Producer business transaction → modkit-db outbox**: the producer's business code writes both its domain state and the outbox row in one transaction. If the business txn rolls back, the outbox row is never persisted.
2. **modkit-db outbox → SDK publish call**: the outbox pipeline reads pending rows in order and calls the SDK `publish` function. Network failures cause the SDK to retry the same outbox row.
...
| 2. Outbox → ingest network failure | SDK times out; modkit-db outbox keeps the row pending; retry on next sweep | Broker dedups via chain check on the next attempt (200 OK if first attempt had succeeded) |
...
| 400 | `PartitionHashMismatch` | SDK-stamped internal `meta.partition_hint` disagrees with broker's re-hash |
...
- **ADRs**:
  - [`0001-partition-selection`](../ADR/0001-partition-selection.md)
  - [`0002-event-schema`](../ADR/0002-event-schema.md)
  - [`0003-idempotent-producer-protocol`](../ADR/0003-idempotent-producer-protocol.md)
