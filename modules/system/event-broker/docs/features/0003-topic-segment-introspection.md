...
def scrape_lag_metrics(topic: str, partition: int) -> dict:
    ...
    env  = resp.json
    acked = cursor.get((group, topic, partition))      # from broker-side cursor cache
    return {
        "lag":              env["end_sequence"] - acked,
...
def maybe_skip_to_earliest(topic, partition, cursor_acked):
    env = http.get(f"/v1/topics/segments?topic={topic}&partition={partition}").json
    if cursor_acked < env["start_sequence"]:
        # consumer is behind retention; SEEK forward to the earliest still-retained
        http.post(f"/v1/subscriptions/{sub_id}/positions", json={
...
- **AC-5**: An operator dashboard built on this endpoint shows lag = `end_sequence - cursor.acked` for each consumer group on a partition.
