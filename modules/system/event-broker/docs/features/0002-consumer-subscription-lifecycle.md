...
- [2. Actor Flows (CDSL)](#2-actor-flows-cdsl)
  ...
  - [2.2 Seek (offset override)](#22-seek-offset-override)
...
resp = http.post("/v1/consumer_groups", json={
...
# Step 1 — JOIN with topic-anchored typed-filter interests (per ADR-0004)
...
sub_id              = resp.json["id"]
assigned            = resp.json["assigned"]            # list of (topic, partition); topic-centric for ack/seek
...
resp = http.post(f"/v1/subscriptions/{sub_id}/positions", json={
...
for frame in http.get(f"/v1/events:stream?subscription_id={sub_id}", stream=True):
    handle(frame)
    ack(frame.events)                                  # also POST /v1/subscriptions/{id}/positions
...
# Broker serves from cursor + 1 (where `cursor` is the last-processed offset
# stored in evbk_group_offsets). Sentinels resolve at admission:
...
### 2.2 SEEK (set cursor; pre-stream or forward-ack)

The `POST /v1/subscriptions/{id}/positions` endpoint serves two roles for the same subscription:
...
2. **Forward ack during streaming** — call it to advance the cursor past acked events. While `:stream` is open against the subscription, the broker enforces `MAX(stored, requested)` per partition; backward moves are rejected with `409 SeekBackwardNotAllowed`.
...
# Consumer side — forward ack during streaming (integers only; forward-only enforced)
resp = http.post(f"/v1/subscriptions/{sub_id}/positions", json={
        ...
        for frame in http.get(f"/v1/events:stream?subscription_id={sub_id}", stream=True):
            ...
            if frame.kind == "topology":
                ...
                if new_slots:
                    ...
                    http.post(f"/v1/subscriptions/{sub_id}/positions",
...
#   cluster + persistent → cursor preserved; resume from cursor.acked + 1
...
def remove_member(G, member_id, reason):               # reason in {"leave", "session_timeout"}
    ...
    # elsewhere — the new owner re-reads from cursor.acked + 1 (single source of truth).
...
#   v1 member owns (T, p) with cursor.acked = X
#   rebalance reassigns (T, p) to a v2 member
#   v2 member resumes from cursor.acked + 1 = X + 1
...
#   cluster + persistent → cursor.acked survives via Redis-with-disk / etcd
```

## 3. Processes / Business Logic (CDSL)

The lock around `cluster.distributed_lock("evbk.group.<G>")` is the serialization point for every GroupState mutation (JOIN, LEAVE, rebalance, ownership transfer). Reads (poll, ack, seek) are NOT lock-protected — they touch `cursor.*` cache keys directly and rely on the cache's own consistency guarantees.
...
- `cursor.acked` is updated atomically via cache CAS (compare-and-swap with `>=` semantics — forward-only by default; the SEEK endpoint bypasses with explicit override).
...
- **AC-1 (cold JOIN)**: POST /v1/consumer_groups with `{ "client_agent": "<valid User-Agent string>" }` returns 201 + Location + broker-minted id + echoed `client_agent`; subsequent POST /v1/subscriptions referencing that id succeeds. POST without `client_agent` is rejected with the canonical RFC 9457 400 problem type.
- **AC-2 (seek)**: SEEK on assigned partitions updates `cursor.acked`; next poll returns events from the new offset; SEEK on unassigned partition returns 409.
...
- **cursor CAS forward-only**: ack of offset older than current `cursor.acked` is a no-op (forward-only).
...
- **Version resolution**: parameterize over (registered types, pattern) tuples and assert resolved set matches per-name-latest + minor-version-omitted rule from ADR-0004.
- **Filter context (CEL engine)**: assert `event.id` / `event.type` / ... visible; `event.meta` reference → `FilterError::CompileFailed`.

## 8. E2E Test Plan

- **S1 — full lifecycle**: POST /v1/consumer_groups → JOIN → poll → ack → seek → poll → LEAVE; verify every state transition.
...
- **S3 — rolling deploy**: start with 5 v1 members; introduce 5 v2 members one at a time; remove v1 members; assert no event is lost, no event is double-acked.
...
- **S7 — cluster cursor durability**: in cluster mode with persistent cache provider, restart owning shard; new shard takes over; cursor preserved; consumer resumes from `cursor.acked + 1`.
