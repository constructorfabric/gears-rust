...
| `partition` | body, producer-set | body, `readOnly` | Broker derives from `partition_key` or `subject` per [ADR-0001](0001-partition-selection.md); rejected on publish; surfaced on read |
...
The outbox sequence inside `modkit-db` is internal plumbing and not part of the broker's wire surface.
...
This is a platform-wide convention, not a broker-specific choice. It keeps the partition-selection hash (Murmur3-32) deterministic across language SDKs without normalization concerns, and it keeps event-field parsing in any language cheap.
...
- **PRD**: [PRD.md](../PRD.md)
  ...
  - `cpt-cf-evbk-fr-producer-modes` — producer modes consume `meta.{producer_id, previous, sequence}` per [ADR-0003](0003-idempotent-producer-protocol.md)
...
- **Related ADRs**:
  - [`0001-partition-selection`](0001-partition-selection.md) — partition derivation contract
  - [`0003-idempotent-producer-protocol`](0003-idempotent-producer-protocol.md) — producer modes / `meta`-block-shape enforcement / registration TTL / reset endpoint
- **Schemas**:
