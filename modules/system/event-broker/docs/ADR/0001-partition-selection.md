...
revision-history:
  ...
  - 2026-05-12 — revised (drop explicit `partition` override; broker re-hashes and is authoritative; `partition_key` is the only producer-facing override)
...
# Partition Selection — MurmurHash3-32 Over `tenant` By Default, Optional `partition_key` Override
...
- [Pros and Cons of the Options](#pros-and-cons-of-the-options)
  - [MurmurHash3-32 Over `tenant` Default + Optional `partition_key` Override (chosen)](#murmurhash3-32-over-tenant-default--optional-partition_key-override-chosen)
...
- The broker assigns the consumer-visible `event.sequence` per `(topic, partition)`. Producers never set `sequence`; they only carry chain state for ingest-side dedup in `meta.previous` and `meta.sequence` (see [ADR-0002 Event Schema](0002-event-schema.md)).
- `Event` carries `subject` (a per-event entity identifier) and `subject_type`. A dedicated `partition_key` field exists as an optional override of the default key.
...
* Cross-language SDK parity: the algorithm must be specifiable in one paragraph so future Go / Python / TypeScript producers route the same key to the same partition as the Rust producer
...
* MurmurHash3-32 over `subject` by default, optional `partition_key` body override; broker re-hashes and is authoritative (chosen — this revision)
...
Adopt **MurmurHash3-32 over `tenant` by default, with optional `partition_key` body field as the only producer-facing override**. Partition selection runs in both the producer SDK (for outbox routing) and the broker (for authoritative assignment); both implementations pin to the same canonical formula. There is no explicit `partition` field on publish input.
...
partition_input = partition_key  if partition_key is present
                  tenant_id      otherwise
partition       = (murmur3_32(ascii_bytes(partition_input)) & 0x7FFFFFFF) % topic.partitions
```

- Hash function: **MurmurHash3 (32-bit, x86 variant)** with a fixed seed of `0x00000000`. Matches Kafka's `DefaultPartitioner` / `BuiltInPartitioner` choice and is the de-facto standard for partition selection in log-structured brokers.
- The mask `& 0x7FFFFFFF` strips the sign bit so the modulo operates on a non-negative `u31` value, matching Kafka's behaviour and avoiding the negative-modulo edge case in languages with signed `%`.
- The bytes hashed MUST be the **ASCII byte representation** of the input. Per the platform convention recorded in [ADR-0002 Event Schema § Event Field Encoding](0002-event-schema.md#event-field-encoding-ascii-only), all event string fields (`partition_key`, `subject`, etc.) are ASCII; UTF-8 is permitted only inside `data`. This eliminates UTF-8-vs-ASCII determinism concerns in the hash path.
- Producers MUST NOT use other hash functions for the default path. Cross-language producer parity depends on this being fixed.
...
- `partition_key` is the explicit override controlling *co-location* when neither the tenant default nor subject is the desired grouping.
...
- **Producer SDK** computes the partition locally before calling `outbox.enqueue()`, so the `modkit-db` outbox can route the row to the correct per-`(topic, partition)` outbox shard and preserve order.
- **Broker** re-computes the partition on ingest from `partition_key` / `subject` in the received event. The broker's value is authoritative; if persisted at all, the SDK-computed value is treated as a hint only.
...
- Eliminates the producer-override surface (no explicit `partition` field that producers can stamp).
...
The SDK still fetches `topic.partitions` once at startup (or on first publish to a topic) via `GET /v1/topics/{id}` and caches it for the lifetime of the producer process. The count is immutable per `Topic.partitions` design, so the cache never goes stale.

### Broker-Side Validation

On `POST /v1/events` and `POST /v1/events:batch`, the ingest service MUST:

- Compute `partition` from `partition_key` (if present) or `tenant_id` using the canonical formula.
...
All inputs to the hash are ASCII per [ADR-0002 § Event Field Encoding](0002-event-schema.md#event-field-encoding-ascii-only). The broker rejects publishes with non-ASCII bytes in `partition_key` or `tenant_id` with `400 InvalidEventFieldEncoding` before partition computation is attempted. `partition_key` is additionally length-capped at 1024 bytes (`400 EventFieldTooLong` on overflow).
...
- Good, because producer SDKs in other languages can be added later with one paragraph of contract: "Murmur3-32, seed 0, mask sign bit, mod partition count, ASCII bytes." No protocol negotiation needed.
...
- Bad / accepted limitation, because **adversarial keys can hot-spot a partition**. Murmur3 is not cryptographic. The broker's threat model treats producers as authenticated trusted modules.
- Bad / accepted cost, because **the broker now spends one Murmur3-32 hash per ingest** that the SDK already computed. Murmur3-32 over ≤ 1024 ASCII bytes is sub-microsecond; negligible against the DB write that follows. Accepted in exchange for removing the producer-override surface.
- Bad / accepted limitation, because **producers that use `partition_key` inconsistently** (sometimes set, sometimes left to the `subject` fallback) silently split ordering of a logical key across two partitions. Mitigation: producer-author guidance — set `partition_key` consistently per logical entity, not per code-path. Documented in `docs/features/0001-idempotent-producers.md`.

### Confirmation

The decision is verified by:

- **SDK unit tests** pinning the Murmur3 implementation: known input → known partition for representative `partition_key` / `tenant_id` strings (ASCII printable, ASCII control bytes, empty `partition_key` falling back to `tenant_id`), with `topic.partitions` values 1, 2, 16, 64. The tests SHALL fail any future SDK change that drifts from the canonical formula.
- **Broker-side test** of the same fixture vector: the broker's re-hash matches the SDK's per-vector value bit-for-bit.
- **Cross-language fixture vector** maintained in the SDK contract documentation: a list of `(input, partitions, expected_partition)` tuples (using `input = partition_key or tenant_id`) that every language SDK MUST reproduce bit-for-bit. New SDKs in other languages run this vector as their first compliance test. The vector from the initial draft of this ADR applies unchanged — the hashing rule is identical; only the *which input is hashed* logic changed (now: `partition_key` if set, else `tenant_id`).
...
### MurmurHash3-32 Over `tenant` Default + Optional `partition_key` Override (chosen)
...
* Good, because it matches Kafka's algorithm exactly, making the mental model transferable and the cross-language contract one-line
...
**Description**: Expose a `Partitioner` trait in `cyberware-event-broker-sdk`; the default impl is Murmur3-mod-N; users can register their own.
...
- **Pluggable Partitioner trait**: if a real second use case appears (e.g., a producer wanting weighted partition selection for hot-tenant isolation), the SDK could expose a `Partitioner` trait — but the broker would still re-hash with Murmur3 and reject mismatches, so any pluggable scheme would need to ship in lockstep with a broker-side equivalent. Decision deferred until a concrete request lands.
- **Hash function evolution**: Murmur3 has known weaknesses against adversarial inputs. The threat model treats producers as trusted, but if the broker ever opens to untrusted producers (e.g., a public ingest endpoint), an HMAC-keyed hash or xxHash3 with a server-issued seed becomes worth revisiting. Out of scope for MVP.
...
- Apache Kafka — `org.apache.kafka.clients.producer.internals.BuiltInPartitioner` (Murmur3-32 over UTF-8 key bytes; identical algorithm chosen here over ASCII): <https://kafka.apache.org/documentation/#producerconfigs_partitioner.class>
- Apache Kafka — `DefaultPartitioner` historical contract: <https://kafka.apache.org/documentation/#design_partitioning>
...
- **PRD**: [PRD.md](../PRD.md)
  ...
  - `cpt-cf-evbk-fr-publish-batch` — batch publish requires same `(topic, partition)` for all events (broker derives partition; producer guarantees a batch shares one `partition_key` or one `subject`-hash bucket)
...
- **DESIGN**: [DESIGN.md](../DESIGN.md)
  ...
  - §3.2 Producer Modes — references [ADR-0003](0003-idempotent-producer-protocol.md)
  - §3.6 Two Sequences — producer chain in `meta` / server-assigned `sequence` (per [ADR-0002](0002-event-schema.md))
