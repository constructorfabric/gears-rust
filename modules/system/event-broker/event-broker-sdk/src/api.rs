...
use crate::models::{
    ConsumerGroup, ConsumerGroupKind, CreateConsumerGroupRequest, EventType, PartitionLeader,
...
/// Partition assignment returned from a JOIN, extended with cursor positions
/// so the consumer knows where to start reading.
#[derive(Debug, Clone)]
pub struct AssignedPartition {
    pub topic: String,
    pub partition: u32,
    /// `cursor.acked` for `(group, topic, partition)` — events with `sequence > offset` are returned next.
    pub offset: i64,
...
pub struct JoinRequest {
    ...
    /// Per-member interests (topic-anchored typed-filter selections per ADR-0004).
    pub interests: Vec<SubscriptionInterest>,
    /// Session TTL, refreshed on every poll/ack. Default PT30S.
    pub session_timeout: Option<Duration>,
    /// If `true`, broker advances `cursor.acked` to `cursor.sent` after each send (at-most-once).
...
pub trait EventBroker: Send + Sync {
    ...
    async fn register_producer(
        ...
        mode_str: &str,
        client_agent: &str,
    ) -> Result<ProducerId, EventBrokerError>; // → POST /v1/producers
    ...
    async fn list_consumer_groups(
        &self,
        ctx: &SecurityContext,
    ) -> Result<Vec<ConsumerGroup>, EventBrokerError>; // → GET /v1/consumer_groups
    ...
    /// Acknowledge offsets. `offsets` is a 2-level `topic → partition → offset` map
    /// (forward-only, MAX rule).
    async fn ack(
        &self,
        ctx: &SecurityContext,
        id: SubscriptionId,
        offsets: &HashMap<String, HashMap<u32, i64>>,
    ) -> Result<(), EventBrokerError>; // → POST /v1/subscriptions/{id}/ack
    ...
    ) -> Result<(), EventBrokerError>; // → POST /v1/subscriptions/{id}/positions
    ...
    async fn get_topic(&self, ctx: &SecurityContext, id: &str) -> Result<Topic, EventBrokerError>; // → GET /v1/topics/{id}
    ...
    ) -> Result<Vec<TopicSegment>, EventBrokerError>; // → GET /v1/topics/{id}/segments
    ...
    async fn get_event_type(
        ...
        ctx: &SecurityContext,
        id: &str,
    ) -> Result<EventType, EventBrokerError>; // → GET /v1/event_types/{id}
}
