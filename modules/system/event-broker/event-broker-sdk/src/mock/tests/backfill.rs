...
async fn replay_group_does_not_perturb_primary() {
    ...
    // Primary acks up to offset 4.
    let mut m: std::collections::HashMap<String, std::collections::HashMap<u32, i64>> =
        Default::default();
    m.entry(TOPIC.to_owned()).or_default().insert(0, 4);
    broker
        .ack(&c, primary_sub.subscription_id, &m)
        .await
        .unwrap();
