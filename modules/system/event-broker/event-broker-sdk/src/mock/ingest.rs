...
/// Producer mode inferred from the `meta` fields on the event.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(super) enum ProducerMode {
    /// `producer_id + previous + sequence` — full chain dedup.
    Chained,
    /// `producer_id + sequence` only — monotonic dedup.
    Monotonic,
...
impl ProducerMode {
    pub(super) fn detect(event: &WireEnvelope) -> Self {
        match &event.meta {
            Some(m) if m.producer_id.is_some() && m.previous.is_some() && m.sequence.is_some() => {
                Self::Chained
            }
            Some(m) if m.producer_id.is_some() && m.sequence.is_some() => Self::Monotonic,
            _ => Self::Stateless,
        }
    }
}
...
/// derivation (ADR-0001: `partition_key` else `tenant_id`) → mode detection →
...
pub(super) fn ingest_one(
    core: &mut Core,
    event: &WireEnvelope,
) -> Result<(IngestOutcome, WireEnvelope), EventBrokerError> {
    // ── 0. GTS format validation ──────────────────────────────────────────────
    if let Err(e) = gts_id::validate_gts_id(&event.topic, false) {
    ...
    if let Err(e) = gts_id::validate_gts_id(&event.type_id, false) {
        return Err(EventBrokerError::InvalidEventField {
            field: "type",
    ...
    // ── 4. Partition derivation (ADR-0001: partition_key else tenant_id) ───────
    ...
    let mode = ProducerMode::detect(event);
...
pub(super) fn ingest_batch(
    core: &mut Core,
    events: &[WireEnvelope],
) -> Result<Vec<(IngestOutcome, WireEnvelope)>, EventBrokerError> {
    if events.is_empty() {
        return Ok(vec![]);
    }
    ...
    events.iter().map(|e| ingest_one(core, e)).collect()
}
