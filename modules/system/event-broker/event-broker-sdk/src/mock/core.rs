...
pub struct CursorEntry {
    /// Highest offset the group has confirmed processing.
    pub acked: i64,
...
impl GroupState {
    ...
    /// Ensure a `CursorEntry` exists for `(topic, partition)`, creating one at `acked=0` if absent.
...
pub(super) struct SubState {
    ...
    /// If true, broker advances `cursor.acked` to `sent` after each event emit (at-most-once).
    ...
    /// Reset to `cursor.acked` when a partition migrates → at-least-once redelivery.
