...
use crate::api::{AssignedPartition, EventBroker, JoinRequest, SubscriptionAssignment};
...
use crate::models::{
    ConsumerGroup, ConsumerGroupKind, CreateConsumerGroupRequest, EventType, PartitionRange,
...
impl EventBroker for MockBroker {
    ...
    async fn list_consumer_groups(
        &self,
        _ctx: &SecurityContext,
    ) -> Result<Vec<ConsumerGroup>, EventBrokerError> {
        let core = self.core.lock().await;
        Ok(core
            .groups_registry
            .iter()
    ...
    async fn join(
        ...
        let assigned: Vec<AssignedPartition> = sub
            ...
            .iter()
            .map(|(topic, partition)| {
                let cursor = group.cursor.get(&(topic.clone(), *partition));
                AssignedPartition {
                    topic: topic.clone(),
                    partition: *partition,
                    offset: cursor.map(|c| c.acked).unwrap_or(0),
                    last_examined: cursor.map(|c| c.last_examined).unwrap_or(0),
                }
            })
            .collect();
    ...
    async fn ack(
        ...
        if let Some(sub) = core.subscriptions.get_mut(&id) {
            sub.last_seen_at = Instant::now();
            sub.expires_at = sub.last_seen_at + sub.session_timeout;
        }

        // Advance cursor.acked using MAX rule (H5/H6: real (group, topic, partition) key).
        if let Some(group) = core.groups.get_mut(&group_id) {
            for (topic, partitions) in offsets {
                for (partition, offset) in partitions {
                    let entry = group.cursor.entry((topic.clone(), *partition)).or_default();
                    entry.acked = entry.acked.max(*offset);
                }
            }
        }
        Ok(())
    ...
    async fn seek(
        ...
        let mut core = self.core.lock().await;
        // Resolve sentinels and write into sub.seek.
        let mut resolved: HashMap<(String, u32), i64> = HashMap::new();
        for pos in positions {
            let offset = match &pos.value {
                ResolvedPosition::Exact(n) => *n,
                ResolvedPosition::Earliest => 0,
                ResolvedPosition::Latest => {
                    // Latest = next_offset - 1 (i.e., skip all current events).
                    core.topics
                        .get(&pos.topic)
            ...
            resolved.insert((pos.topic.clone(), pos.partition), offset);
        }
        if let Some(sub) = core.subscriptions.get_mut(&id) {
            sub.seek.extend(resolved);
        }
        self.notify.notify_waiters();
        Ok(())
    }
    ...
    async fn get_topic(&self, _ctx: &SecurityContext, id: &str) -> Result<Topic, EventBrokerError> {
        let core = self.core.lock().await;
        let t = core
            .topics
            .get(id)
            .ok_or_else(|| EventBrokerError::TopicNotFound {
                topic: id.to_owned(),
                detail: format!("topic '{id}' not registered in mock"),
                instance: String::new(),
            })?;
        Ok(Topic {
            id: id.to_owned(),
