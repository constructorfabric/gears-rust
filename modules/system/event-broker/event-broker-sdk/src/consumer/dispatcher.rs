...
use crate::consumer::backend::{
    ConsumerBackend, PartitionSlot, SeekPosition, SubscriptionAssignment, SubscriptionInterest,
    WireEvent, WireFrame,
};
    ...
    OM: OffsetManager + 'static,
{
    pub async fn run(
        ...
        let ack_cursors = cursors.clone();
        let ack_backend = self.backend.clone();
        let ack_om = self.offset_manager.clone();
        let ack_ctx = ctx.clone();
        let ack_sub = assignment.subscription_id;
            ...
            loop {
                tokio::select! {
                    ...
                    _ = interval.tick() => {
                        ...
                        let guard = ack_cursors.read().await;
                        for (slot, cursor) in guard.iter() {
                            ...
                            if cursor.latest_offset > cursor.committed {
                                ...
                                let _ = ack_backend.ack(
                                    &ack_ctx,
                                    ack_sub,
                                    &[(slot.partition, cursor.latest_offset)],
                                ).await;
                            }
                        }
                    }
    ...
    async fn route_reject(
        ...
        // Best-effort broker ack for the cursor advance.
        let _ = self
            .backend
            .ack(ctx, sub_id, &[(raw.partition, raw.offset)])
            .await;
    ...
    async fn join_subscription(
        ...
        let interests: Vec<SubscriptionInterest> = self
            .topics
            .iter()
            .zip(&self.event_type_patterns)
            .map(|(t, p)| SubscriptionInterest {
                topic: t.clone(),
                event_type_pattern: p.clone(),
                filter_engine: self.filter.as_ref().map(|(e, _)| e.clone()),
                filter_expression: self.filter.as_ref().map(|(_, expr)| expr.clone()),
            })
            .collect();
