...
#### OR-of-ANDs AccessScope Preservation

- [ ] `p1` - **ID**: `cpt-cf-usage-collector-constraint-or-of-ands-preservation`

`AccessScope` returned by the platform PDP encodes a disjunction of constraint groups (OR-of-ANDs): each `Constraint` is one AND-group, where all fields within a single `Constraint` are combined with AND (conjunction), and `AccessScope` itself is the disjunction (OR) of these AND-groups. The gateway's `compile_to_access_scope()` step MUST preserve this structure exactly. Flattening multiple constraints to independent AND conditions — treating cross-group fields as conjunctive — is a security violation: it can grant access to records that match no single PDP constraint group on its own. Storage plugin implementations of `query_aggregated` and `query_raw` MUST apply the OR-of-ANDs structure as mandatory row filters; collapsing the disjunction or dropping any AND-group is equivalent to a permissive fallback and violates `cpt-cf-usage-collector-principle-fail-closed`.
...
| `AggregationFn` | Aggregation function selector for `AggregationQuery`: one of `Sum`, `Count`, `Min`, `Max`, `Avg`. Maps 1:1 to the SQL-level aggregator pushed down to the active storage plugin. | [`usage-collector-sdk/src/models.rs`](../../usage-collector/usage-collector-sdk/src/models.rs) |
| `BucketSize` | Time-bucket granularity used when `GroupByDimension::TimeBucket` is requested (e.g. minute / hour / day / week / month). Carried inside `GroupByDimension::TimeBucket(BucketSize)`; `bucket_size` is mandatory whenever `group_by` includes `TimeBucket` and rejected with 400 otherwise. | [`usage-collector-sdk/src/models.rs`](../../usage-collector/usage-collector-sdk/src/models.rs) |
| `GroupByDimension` | GROUP BY dimension enum for `AggregationQuery`: `TimeBucket(BucketSize)`, `UsageType`, `Subject`, `Resource`, `Source`. The query may carry any combination; each active dimension causes the corresponding `Option` field on `AggregationResult` to be populated. | [`usage-collector-sdk/src/models.rs`](../../usage-collector/usage-collector-sdk/src/models.rs) |
| `AggregationQuery` | Query parameters compiled by the gateway: `scope: AccessScope` (OR-of-ANDs from PDP, see `cpt-cf-usage-collector-constraint-or-of-ands-preservation`), `time_range: (DateTime<Utc>, DateTime<Utc>)` (mandatory, strictly ascending), `function: AggregationFn`, `group_by: Vec<GroupByDimension>`, `bucket_size: Option<BucketSize>`, optional filters (`usage_type`, `resource_id`, `resource_type`, `subject_id`, `subject_type`, `source`). Tenant identity is carried inside `scope`; callers MUST NOT pass `tenant_id` as a query parameter. | [`usage-collector-sdk/src/models.rs`](../../usage-collector/usage-collector-sdk/src/models.rs) |
| `AggregationResult` | Result row returned by `query_aggregated`: `function: AggregationFn`, `value: f64`, plus dimension fields (`bucket_start`, `usage_type`, `subject_id`, `subject_type`, `resource_id`, `resource_type`, `source`) populated only when the corresponding `GroupByDimension` was requested. Absent dimensions are omitted from JSON (not serialized as `null`). | [`usage-collector-sdk/src/models.rs`](../../usage-collector/usage-collector-sdk/src/models.rs) |
| `RawQuery` | Raw record query parameters compiled by the gateway: `scope: AccessScope`, `time_range: (DateTime<Utc>, DateTime<Utc>)`, optional filters (`usage_type`, `resource_id`, `resource_type`, `subject_type`, `subject_id`), `cursor: Option<CursorV1>` (`modkit-odata`; exclusive lower-bound keyset on `(timestamp, id)`), `page_size: usize` (bounded by `[1, MAX_PAGE_SIZE]`, defaults to `DEFAULT_PAGE_SIZE`). Returns `Page<UsageRecord>` (`modkit-odata`) where absent `page_info.next_cursor` signals the final page. | [`usage-collector-sdk/src/models.rs`](../../usage-collector/usage-collector-sdk/src/models.rs) |
