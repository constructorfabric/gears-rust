...
async fn client_secret_not_found_returns_secret_not_found() {
    ...
    let err = plugin.authenticate(&mut ctx).await.unwrap_err();
    assert!(matches!(err, PluginError::SecretNotFound(ref s) if s.contains("test-client-secret")));
}
...
async fn token_fetch_failure_returns_internal_error() {
    ...
    let err = plugin.authenticate(&mut ctx).await.unwrap_err();
    assert!(matches!(err, PluginError::Internal(ref msg) if msg.contains("token fetch failed")));
}
