...
- **CI gates**: merges are gated by deterministic checks (lint/build/test) plus deterministic CPT ID validation (uniqueness + reference integrity), with additional heuristic review/tooling as needed.
...
**Deviations / possible future improvements:** add CI enforcement that every implemented requirement has corresponding test coverage with CPT ID references, and require explicit verification method documentation for any requirement not covered by automated tests.
...
**Fulfillment:** A single modular mono-repo with PR-based review and CPT IDs supports strong traceability and change control, aligning with ISO’s configuration and verification expectations.

**Deviations / possible future improvements:** the gap is that these practices are not explicitly documented as part of the requirements system, so ISO alignment depends on team discipline rather than required artifacts. Add a brief section that declares PR-based review, mono-repo modular boundaries, and Constructor Studio-based cross-validation as required process controls.
...
1. **Constructor Studio validation**: ensure all referenced IDs exist, no duplicates, and cross-doc links are consistent.
...
|-------------------|-----------------|
| **Stakeholders Needs** | Stakeholder requirements + governance in README; CPT ID traceability |
...
| **Traceability** | CPT ID system across all artifacts; bidirectional links |
...
| **Open-Source Mono-Repo Practices** | PR-based review; CPT IDs; documented in README |
| **CI-Based Cross-Validation** | Deterministic Constructor Studio validation; implicit via assumptions |
