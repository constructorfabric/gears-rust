...
Industry-standard specification templates incorporating best practices from IEEE, ISO, and modern software development methodologies. Compatible with [Constructor Studio](https://github.com/constructorfabric/studio) for validation and traceability.
...
CPT IDs (Canonical Provenance Trace IDs) enable traceability across all specification artifacts.

### CPT ID Definition

A CPT ID **defines** a unique identifier for a specification element (actor, requirement, feature, etc.). Each ID must be **globally unique** within the module, subsystem or global project depending on where it's defined
...
A CPT ID covers the **markdown section where it's defined and all its subsections**:
...
A CPT ID **reference** links to an element defined elsewhere. References create traceability between documents — for example, a Feature can reference Actors from PRD, or an ADR can reference Requirements it addresses.

**Placement**: Use backtick notation `` `cpt-...` `` when referencing an ID defined in another section or file.

### Validation

CPT IDs must be unique. When Constructor Studio tooling is connected, `cfs validate` will:
...
When Constructor Studio is connected:
...
For more details on Constructor Studio taxonomy and artifact relationships, see [`TAXONOMY.md`](https://github.com/constructorfabric/studio/blob/main/guides/TAXONOMY.md).
