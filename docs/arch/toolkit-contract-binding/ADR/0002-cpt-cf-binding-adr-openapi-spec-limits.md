...
### Amendment: multipart/mixed response streaming is now in scope

This ADR scoped out "multipart, URL-encoded-form, and octet-stream request or
response bodies" wholesale. **`multipart/mixed` response streaming is now
supported**; everything else in that line remains out of scope.

The exclusion conflated two very different things under one word. General
multipart is a document-assembly format: heterogeneous parts, per-part content
types, `Content-Disposition` naming, file uploads. Modelling that in attributes
is what the exclusion was protecting against. Streaming a *homogeneous* sequence
of one JSON item per part is not that — it is the same shape SSE already has, in
a second wire format, and the item type is the contract's existing return type
rather than anything new to declare.

In scope as of the streaming-framing work:

* **`multipart/mixed` response streaming**, one JSON item per body part, with
  the item type taken from the method's declared `Result<Item, E>` exactly as
  SSE's is;
* the framing selected per method by `#[streaming(sse)]` /
  `#[streaming(multipart_mixed)]`, where a bare `#[streaming]` keeps meaning
  SSE;
* the `boundary=` parameter generated by the framer at runtime, so it is never
  author-declared and never appears in the spec — the OpenAPI media-type key is
  the bare `multipart/mixed`.

Still out of scope, unchanged:

* **`multipart/form-data` requests** and multipart *request* bodies generally;
* **mixed per-part content types** — every part is a JSON document decodable as
  the one item type;
* **`Content-Disposition`** and per-part naming;
* **octet-stream and URL-encoded-form** bodies, either direction;
* **author-declared response headers**. The `boundary=` is a parameter on the
  one platform-standard header, not an exception to this.

**Why Option B's combinatorial-hazard argument does not apply.** That argument
(below) names "`#[multipart]` with `#[response_header]` with `#[streaming]`" as
the shape that produces combinatorial edge cases — independent attributes whose
pairwise interactions must each be specified. This change creates no such pair:
the framing is an *argument to* `#[streaming]`, not an attribute beside it.
Which wire format a stream speaks is intrinsic to what the streaming marker
means, so the two cannot be written apart, cannot be written without each other,
and cannot combine into a state that needs its own rules. There is no
`#[multipart]` attribute, and a framing selector is rejected at compile time
anywhere an HTTP media type does not exist — on the base contract trait and on a
gRPC projection.

This is also the change the consequences below pre-authorise: "extending the
macro's scope later … is a non-breaking change provided the attribute is
optional and the default behavior remains the Phase 1 semantics". The argument
is optional and its absence is SSE, so every existing `#[streaming]` method is
unchanged.

Streaming **server-handler** generation remains out of scope for both framings;
a streaming route is still registered by hand via `#[server_manual]` and
`OperationBuilder`.
