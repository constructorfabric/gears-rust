...
For v0, two clearly-marked, non-production roles are stubbed in the static auth plugins: distinct dev tokens resolve to identities marked platform-admin or tenant-admin (using `subject_type` and/or token scopes, which the static plugins already support). A dedicated admin-context endpoint returns the principal, resolved tenant, admin mode, enabled gears, and a capabilities list, derived server-side from the security context and the static role marker. The panel calls it at startup to drive mode selection, capability-gated navigation, and tenant scope, with the backend remaining the final authority on every action.
