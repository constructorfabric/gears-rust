...
- `cpt-cf-component-directory-rest` — enumerates gear endpoints + specs (`ListAllInstances`) for edge discovery
...
| `ListAllInstances`   | `ListAllInstancesRequest {}`      | `ListAllInstancesResponse { instances[] }` | Enumerate every registered instance (gear + REST endpoint + OpenAPI spec) for edge discovery |
...
sequenceDiagram
    ...
    loop Periodic
        ...
        Dir-->>Edge: instances[] (gear, rest_endpoint, openapi_spec)
