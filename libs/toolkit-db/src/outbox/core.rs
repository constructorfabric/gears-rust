...
use super::flush_handle::FlushHandle;
...
impl Outbox {
    ...
    ///
    /// Returns a [`FlushHandle`]: the write does not wake the sequencers on its
    /// own. Call [`FlushHandle::flush`] once the enclosing transaction has
    ...
    pub async fn enqueue(
        ...
        db: &(impl crate::secure::DBRunner + Sync + ?Sized),
        msg: Record<'_>,
    ) -> Result<FlushHandle, OutboxError> {
        ...
        Ok(FlushHandle::new(
            ...
            vec![partition_id],
            self.prioritizer.read().await.clone(),
        ))
    }
