...
//! validation trait. The Axum middleware, the tonic interceptor, and the
...
pub enum PlatformIdentity {
    ...
    /// (`spiffe://<trust_domain>/gear/<gear>/<version>`). Reserved — not
    /// populated in the first phase.
    Spiffe {
        /// SPIFFE trust domain.
        trust_domain: String,
        /// Gear name component of the SPIFFE ID.
        gear: String,
        /// Gear version component of the SPIFFE ID.
        version: String,
    },
    ...
    /// unrecognised value. Never constructed directly; `peer_gear` returns
...
impl PlatformIdentity {
    /// The caller's gear name, distilled for workload-policy decisions.
    ///
    /// For a [`PlatformIdentity::ServiceAccount`] this is the `ServiceAccount`
    /// name; for [`PlatformIdentity::Spiffe`] it is the gear component of the
    /// SPIFFE ID.
    #[must_use]
    pub fn peer_gear(&self) -> &str {
        match self {
            Self::ServiceAccount {
                service_account, ..
            } => service_account,
            Self::Spiffe { gear, .. } => gear,
...
///
/// Distilled to the caller's gear name and consumed **only** by workload-policy
...
pub struct PeerAuthenticated {
    /// The authenticated caller's gear name.
    pub gear: String,
}
...
/// Validates a raw platform-plane credential and resolves the caller's
/// [`PlatformIdentity`].
///
...
pub trait InternalAuthenticator: Send + Sync {
    /// Validate the raw `X-ToolKit-Internal-Token` value and resolve the
    /// caller's [`PlatformIdentity`].
    ///
    /// # Errors
    ///
    /// Returns [`InternalAuthError`] if the credential is invalid, the backend
    /// is unavailable, or validation otherwise fails.
    fn validate(
        &self,
        token: &str,
    ) -> impl Future<Output = Result<PlatformIdentity, InternalAuthError>> + Send;
}
...
mod tests {
    ...
    fn platform_security_context_roundtrips_serde() {
        let ctx = PlatformSecurityContext::new(PlatformIdentity::ServiceAccount {
            namespace: "toolkit".to_owned(),
            service_account: "directory-service".to_owned(),
            pod: None,
        });
