...
//! inherent, `.await`-able method `wire_<contract_snake>(&self, &GearCtx)`
//! that:
...
/// Emits the `wire_<contract>` method, in one of two shapes depending on
/// whether the gRPC arm is compiled in.
///
/// `wire_body` awaits only inside the gRPC arm (`#grpc_client_path::connect(...)
/// .await`), and that arm is only emitted when `enable_grpc` is true. So a gear
/// declared with `transports = [local]` or `transports = [local, rest]` used to
/// get a generated `async fn` whose body never awaited anything, and
/// `clippy::unused_async_trait_impl` (new in Rust 1.98.0) fired on the caller's
/// own `#[toolkit::provides(...)]` attribute — a lint finding the gear author has
/// no way to fix, since the body is entirely macro-generated.
///
/// So the method is emitted in one of two shapes:
///   - `enable_grpc == true`: the public method stays `async fn` and awaits, as
///     before.
///   - `enable_grpc == false`: the body moves into a private, `#[doc(hidden)]`
///     `wire_impl_method` that returns `::anyhow::Result<()>` directly (so its
///     `?` and `::anyhow::bail!` keep working unchanged), and the public
///     `wire_method` becomes a thin non-async wrapper returning
///     `::core::future::Ready<::anyhow::Result<()>>`. Every existing call site
///     (the one `#[toolkit::gear]` generates, and hand-written calls) keeps
///     compiling unchanged because the returned value is still `.await`-able.
///     This can't be `fn wire_method(..) -> impl Future<..> { async move { .. } }`
///     (that trips `clippy::manual_async_fn`, denied via `pedantic`), and the
///     body can't be inlined into an immediately-invoked closure passed to
///     `ready(...)` (that trips `clippy::redundant_closure_call`, warn-by-default
///     and fatal under `-D warnings`) — hence the separate named method.
fn build_wire_method_item(
    wire_method: &Ident,
    wire_impl_method: &Ident,
    enable_grpc: bool,
    wire_body: &TokenStream,
) -> TokenStream {
    let wire_method_doc = "Auto-generated by `#[toolkit::provides]`. Validates the \
        contract IR, reads wiring config, builds the requested transport, and \
        registers the resulting client in the `ClientHub`.";
    ...
    if enable_grpc {
        ...
        quote! {
            ...
            pub fn #wire_method(
                &self,
                ctx: &::toolkit::GearCtx,
            ) -> ::core::future::Ready<::anyhow::Result<()>> {
                ::core::future::ready(self.#wire_impl_method(ctx))
            }
        }
    }
}
...
pub fn generate(attr: &ProvidesAttr, item: &ItemStruct) -> SynResult<TokenStream> {
    ...
    // Only used when gRPC is not enabled — see the comment above `wire_method_item`
    // below for why the body then lives in a separate, non-async method.
    // Named so a contract cannot produce it: `to_snake_case` of a type name never
    // yields a leading double underscore. The previous `wire_{contract_snake}_sync`
    // was not actually unsafe — `parse.rs` requires every contract trait to end in
    // `Api`/`Embedded`/`Backend`/`Extension` (PRD #1536 D6), so one contract's public
    // `wire_{snake}` can never equal another's `_sync`-suffixed helper — but it read
    // as a public sibling of `wire_{snake}`, which it is not, and in this branch the
    // public method is not async either, so "sync" said nothing.
    let wire_impl_method = format_ident!("__wire_{contract_snake}_impl");
    let config_key = attr
        .config_key
        .clone()
    ...
    let wire_method_item =
        build_wire_method_item(&wire_method, &wire_impl_method, enable_grpc, &wire_body);
