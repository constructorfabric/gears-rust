...
//!
//! Applied on the **module struct** in the provider crate. Generates an
//! inherent async method `wire_<contract_snake>(&self, &ModuleCtx)` that:
//!   1. validates the contract IR (fail-fast at startup),
//!   2. reads typed wiring config from
//!      `modules.<module>.config.client_wiring.<contract_snake>`,
...
//! injected — so the attribute composes freely with `#[toolkit::module]`
            ...
            ::toolkit_contract::wiring::ClientWiring::Local => {
                ::anyhow::bail!(
                    concat!(
                        ...
                        "`: local transport is not enabled for module `{}` ",
