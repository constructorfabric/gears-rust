...
/// Upstream echo handler: reflects the received method, path, `Authorization`
/// header, and body back as JSON so the test can assert what the gear saw.
...
async fn start_upstream() -> Endpoint {
...
async fn proxies_get_and_post_and_propagates_bearer() {
    let endpoint = start_upstream().await;
    let registry = Arc::new(ProxyRegistry::new());
    registry.register(GearName::from("calculator"), endpoint, templates());
...
async fn deregister_stops_proxying() {
    ...
    registry.register(gear.clone(), endpoint, templates());
    let forwarder = Forwarder::new(Arc::clone(&registry), HttpClient::new().expect("client"));
