...
//! [`API_VISIBILITY_PUBLIC`]) from `OperationSpec.is_public`; this crate mirrors
...
pub use registry::{ProxyRegistry, RouteMatch, RouteTemplate};
pub use toolkit_provider::ToolKitGatewayProvider;
pub use types::{Endpoint, GearName, OpenApiSpec};

/// The `OpenAPI` vendor-extension key marking an operation as externally visible
/// (registered at the edge gateway).
///
/// Mirrors the key emitted by `cf-gears-toolkit` from `OperationSpec.is_public`.
pub const API_VISIBILITY_EXTENSION: &str = "x-cf-api-visibility";
