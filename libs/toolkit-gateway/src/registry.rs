...
impl State {
    ...
    ///
    /// Gears are visited in a stable, deterministic order (sorted by gear name),
    /// so a path template exposed by more than one gear always resolves to the
    /// same winner across rebuilds. Each distinct path template is inserted once
    /// (multiple methods on the same path share a single matchit entry, since
    /// forwarding keys off the path only); the first gear in that order wins a
    /// duplicate path, and a duplicate rejected from a different gear is logged.
    fn rebuild(&mut self) {
        let mut router = matchit::Router::new();
        // Maps each claimed path to its winning gear, for deterministic
        // first-winner resolution and conflict logging.
        let mut seen: HashMap<String, GearName> = HashMap::new();

        // Deterministic gear ordering: HashMap iteration order is unspecified, so
        // sort by gear name to make duplicate-path resolution stable.
        let mut gears: Vec<(&GearName, &GearEntry)> = self.gears.iter().collect();
        gears.sort_by(|(a, _), (b, _)| a.as_str().cmp(b.as_str()));

        for (gear, entry) in gears {
            ...
            for (path, authenticated) in by_path {
                ...
                seen.insert(path.to_owned(), gear.clone());
