...
impl Forwarder {
    ...
    /// body exceeds the proxy limit, `501` for an unsupported method, and
    ...
    pub async fn forward(&self, req: Request) -> Response {
        ...
        let Ok(body) = axum::body::to_bytes(req.into_body(), MAX_PROXY_BODY_BYTES).await else {
            return problem(
        ...
        match builder.send().await {
            Ok(resp) => upstream_to_response(resp).await,
            Err(err) => {
...
async fn upstream_to_response(resp: HttpResponse) -> Response {
    ...
    let body = match resp.bytes().await {
        ...
        Err(err) => {
            ...
            return problem(
                ...
                "reading upstream response failed".to_owned(),
            );
        }
    };
